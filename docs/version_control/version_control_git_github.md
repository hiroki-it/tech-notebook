...
（１）条件文で```var_dump```メソッド を行う。

（２）どういった入力がされた時に```true``` として判断しているのかを確認。例えば、受注済区分値が設定されている時に、それが失注区分値に変わった場合、値を返却しているならば、そこから、『失注』のビジネスルールが垣間見える。

（３）定数に添えられたコメントから、仕様を理解。
