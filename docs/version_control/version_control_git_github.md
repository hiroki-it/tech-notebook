...
2. どういった入力がされた時に```true``` として判断しているのかを確認。

例えば、受注済区分値が設定されている時に、それが失注区分値に変わった場合、値を返却しているならば、そこから、『失注』のビジネスルールが垣間見える。
...
Gitでソフトウェアを開発する場合、役割を持たせたブランチを作成し、ルールに沿ってコミットする。

Git-flowを簡略化したものに、GitHub-flowやGitlab-flowがある。
