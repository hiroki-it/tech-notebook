...
これに対して、以下のようなカテゴリパーサーのルールを定義する。各 Lambda の `aws.invoked_function_arn` 属性の ARN に応じて、`service` 属性にサービス値 (`foo-apigateway`、`bar-apigateway`、`baz-apigateway`) を付与する。この属性を使用する理由は、さまざまな AWS リソースの構造化ログが持っているためである (`owner` 属性でもよい。ただし、おそらく S3 からログを収集する場合はこれがない？) 。元の構造化ログにすでに `service` 属性があるため、この値が上書きされる。
