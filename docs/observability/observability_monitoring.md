...
| ログステータス | 説明                                             | アラートレベルの例    | アラートするかどうかの判断例 |
| -------------- | ------------------------------------------------ | --------------------- | ---------------------------- |
| emergency      | システムが使用できない状態にある緊急事態         | critical              | する                         |
| alert          | 早急に対応すべき事態                             | critical              | する                         |
| critical       | 対処すべき重要な問題                             | critical              | する                         |
| fatal          | 対処すべき重要な問題                             | critical              | する                         |
| error          | 何かが失敗している                               | criticalまたはwarning | どちらでも                   |
| warn           | 普通ではないことが起こったが、心配する必要はない | criticalまたはwarning | どちらでも                   |
| notice         | いたって普通だが、注意すべきことが起こっている   | notice                | しない                       |
| info           | 知っておくといいかもしれない情報                 | info                  | しない                       |
| debug          | 問題が起こっている場所を知るのに有効な情報       | info                  | しない                       |

#### ▼ 異常値がステータスコードの場合

アラートするステータスコードとログステータスへの変換の目安は以下の通りである。各ステータスコードをバラバラに扱うことは大変なので、系ごとにまとめて扱えるように、レベルを割り当てる。

| ステータスコード | 本番環境のアラートの目安 | アラートレベルの例 | アラートするかどうかの判断例 |
| ---------------- | ------------------------ | ------------------ | ---------------------------- |
