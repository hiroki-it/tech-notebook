...
| ヘルスチェック名 | ターゲット             | 方法                                                         |
| ---------------- | ---------------------- | ------------------------------------------------------------ |
| L3チェック       | ネットワーク層まで     | サーバー/コンテナのIPアドレスにPINGリクエスト（ICMP echo）を送信し、レスポンスを検証する。正しいPINGレスポンスが返信されれば、サーバー/コンテナまでのネットワークが正しく動作していると判断できる。<br>ℹ️ 参考：https://milestone-of-se.nesuke.com/nw-basic/ip/icmp/ |
