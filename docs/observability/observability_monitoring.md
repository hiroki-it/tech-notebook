...
SLIとして採用した指標の目標値のこと。ユーザーの視点で策定する必要があるが、ブラウザやユーザー起因の問題を加味したメトリクスを使用することは大変なので、基本的には自身のシステムのみを対象としたメトリクスを使用することが多い。SLOは```99.9```%の成功率を目標とすることが多い。その一方で、SLOだけがユーザーの満足度を決めるわけではなく、SLOを達成するための業務のせいで機能変更ができないような場合、SLOが厳しすぎるので、下方修正しても良い終わってしまう。また、サイトの一部の機能が社内部署向け（例：マーケティング部）の場合は、社内向けのSLOとなる。
...
年月当たりで許容するSLO違反（エラー、ダウンタイム、など）の程度のこと。エラーやダウンタイムの発生によって、システムの信頼性に影響を与える可能性があったとしても、SLOの閾値を超えない限りはこれを許容し、技術を優先する。ビジネスより技術を優先する場合に素早く意思決定できる。
