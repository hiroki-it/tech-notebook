...
| -------------------- | -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `L3`チェック         | `L1`から`L3` (ネットワーク層) まで     | サーバー/コンテナのIPアドレスにPing (ICMPエコーリクエスト) を送信し、レスポンスを検証する。Pingのレスポンスが正しく返信されれば、サーバー/コンテナまでのネットワークが正しく動作していると判断できる。<br>・https://milestone-of-se.nesuke.com/nw-basic/ip/icmp/ |
