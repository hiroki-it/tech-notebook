...
| メトリクスの種類 | 説明                                                                          |
|----------|-----------------------------------------------------------------------------|
| 割合系      | 割合を単位とするアプリケーションのフロントエンド領域のメトリクス                                            |
| 時間系      | 時間を単位とするアプリケーションのフロントエンド領域のメトリクス |
...
特にサイト訪問後のユーザーエンゲージメントのデータポイントを収集し、監視する。リアルユーザー監視の一種ともみなせるが、パフォーマンスの監視が主目的ではなく、リアルユーザー監視と補完し合う監視方法である。
...
| メトリクスの種類 | 説明                                                                    |
|----------|-----------------------------------------------------------------------|
| 割合系      | 割合を単位とするアプリケーションのバックエンド領域のメトリクス                                       |
| 時間系      | 時間を単位とするアプリケーションのバックエンド領域のメトリクス（例：SQLにかかる時間、ビルドまたはデプロイの開始/完了時間、外部APIコールにかかる時間） |
| カウント系    | 数を単位とするアプリケーションのバックエンド領域のメトリクス（例：リクエストの受信数、ログイン数）                              |

#### ▼ パフォーマンス（APM）

『APM（アプリケーションパフォーマンス監視）』ともいう。特にパフォーマンスに関わるメトリクスのデータポイントを収集し、監視する。
...
アプリケーションのログを収集し、監視する。多くの場合、サーバー/コンテナ内にエージェントを
...
| メトリクスの種類 | 説明                                      |
|----------|-----------------------------------------|
| 割合系      | 割合を単位とするサーバー/コンテナのメトリクス（例：CPU使用率、メモリ使用率、ストレージ使用率） |
| 時間系      | 時間を単位とするサーバー/コンテナのメトリクス                           |
