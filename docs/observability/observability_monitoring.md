...
| `L4`チェック         | `L1`から`L4` (トランスポート層) まで   | サーバー/コンテナのポートにTCPハンドシェイクを実行し、TCPコネクションを確立できるかを検証する。TCPコネクションの確立が成功すれば、サーバー/コンテナの開放ポートまでのネットワークが正しく動作していると判断できる。                                           |
...
ビジネス指標の監視に特化したメトリクス監視バックエンドを、特に『BIツール (例：Redash、Metabase、GCP Looker、など) 』ともいう。
