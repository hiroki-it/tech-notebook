...
保管したデータ（ログ、メトリクス）を集計し、新しいデータ（例：意味付けされた新しいメトリクス、ビジネス指標）として扱う。

#### ▼ データの可視化

分析したデータを監視ダッシュボードに表示し、目視できるようにする。
...
|----------|--------------------|-----------------------------------------------------------------------|
| 割合系      | データポイントのタイムスタンプ、パーセンテージ | 割合を単位とするアプリケーションのバックエンド領域のメトリクス                                       |
| 時間系      | データポイントのタイムスタンプ、処理時間 | 時間を単位とするアプリケーションのバックエンド領域のメトリクス（例：SQLにかかる時間、ビルドまたはデプロイの開始/完了時間、外部APIコールにかかる時間） |
