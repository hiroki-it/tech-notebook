...
#### ▼ コンテキスト情報伝播 (分散コンテキスト伝播)

サービスメッシュのリバースプロキシは、分散トレースのためのコンテキスト情報 (例：トレースID、マイクロサービスの属性情報、など) を生成するが、マイクロサービスに対するインバウンド通信とそれのアウトバウンド通信を紐付ける機能を持たない。

そのためマイクロサービス (クリーンアーキテクチャを採用している場合は、アプリのインフラストラクチャ層) で、受信したインバウンド通信のヘッダーから分散トレースのコンテキスト情報を取得し、アウトバウンド通信のヘッダーにコンテキスト情報を渡すような、実装が必要である。

各分散トレースの収集ツール (例：OpenTelemetry、LightStep、Jaeger、Zipkin、Datadog、AWS X-Ray) ごとに、ヘッダーからコンテキスト情報を簡単に取り出せるパッケージを使用すると良い。
