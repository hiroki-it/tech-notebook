...
とある分析にて、一定期間に発生した同じ種類のデータポイントの集約 (例：CPU 使用率、メモリ使用率など) のこと。

メトリクス名と数値のセット (例：Prometheus であれば `foo{x=1, y=2}`) で表されたデータポイントを集約し、メトリクスとする。

メトリクスは、データポイントの形式にあわせていくつかの形式 (例：Count、Gauge、Histogram、Summary など) がある。
...
分析対象から得られる最小単位の数値のこと。

メトリクス名と数値のセット (例：Prometheus であれば `foo{x=1, y=2}`) で表す。

データポイントにはいくつかの形式 (例：Gauge、Histogram、Count など) がある。
...
反対に収集間隔を拡大した場合、データポイント数が少なくなるため、データサイズは小さくなる。

注意点として、収集間隔を拡大した場合はより飛び飛びのメトリクスを描画してしまうため、欠損があったとしてもそれを検出できない可能性がある (例：3 分間隔だとしたら、3 分に満たない間で起こった欠損は描画できない)。

#### ▼ データポイントのダウンサンプリング

期間内にさまざまなタイムスタンプのデータポイントがある場合に、期間を区画に分け、区画内のタイプスタンプを数学的に集約し、単一の値に変換する。
...
冗長化されたデータポイント収集ツールのインスタンスが単一の監視バックエンドやストレージツールにメトリクスを送信する場合、特定の期間には、同じデータポイントが重複して存在することになる。
...
Histogram 型や Summary 型のメトリクスに付与できるメタ情報（`# {...}`）のこと。
...
| 暗黙的エラー | SLO に満たない `200`/`300` 系のレスポンス、API 仕様に合っていないレスポンス |

#### ▼ サチュレーション

システム利用率 (CPU 利用率、メモリ利用率、ストレージ利用率など) の飽和度のこと。
...
CPU、メモリ、ストレージ、ネットワークなどに関する `USE` メトリクス (例：CPU 使用率、CPU サチュレーションなど) を含む。
...
Exemplar を Histogram 型や Summary 型のメトリクスに付与すると、メトリクスとトレース間を相関できる。

また、監視ダッシュボード上でトレースの属性をメトリクスのラベルに変換しても、同様にメトリクスとトレース間を相関できる。
