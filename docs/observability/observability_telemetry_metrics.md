...
とある分析にて、一定期間に発生した同じ種類のデータポイントの集約 (例：CPU使用率、メモリ使用率など) のこと。

メトリクス名と数値のセット (例：Prometheusであれば `foo{x=1, y=2}`) で表されたデータポイントを集約し、メトリクスとする。

メトリクスは、データポイントの形式にあわせていくつかの形式 (例：Count、Gauge、Histogram、Summaryなど) がある。
...
メトリクス名と数値のセット (例：Prometheusであれば `foo{x=1, y=2}`) で表す。

データポイントにはいくつかの形式 (例：Gauge、Histogram、Countなど) がある。
...
同じ種類のメトリクスを特定のグループ (例：Amazon CloudWatchならば、ディメンション、名前空間) に集約する。
...
システム利用率 (CPU利用率、メモリ利用率、ストレージ利用率など) の飽和度のこと。
...
CPU、メモリ、ストレージ、ネットワークなどに関する `USE` メトリクス (例：CPU使用率、CPUサチュレーションなど) を含む。
...
ExemplarをHistogram型やSummary型のメトリクスに付与すると、メトリクスとトレース間を相関できる。
