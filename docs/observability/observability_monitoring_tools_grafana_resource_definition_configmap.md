...
Amazon CloudWatchの場合、`cloudwatch` タイプを指定する。

Amazon CloudWatch-APIからメトリクスの元になるデータポイントを収集するために、自前のクエリを送信する必要がある。
...
ローカルやリモートにあるダッシュボードのJSONを動的に読み込み、ダッシュボードを作成する用途で必要になる。
