...
その一方で、SLOを達成するための業務のせいで機能変更業務が全くできないような場合やリリースを完全止める必要がある場合、SLOが厳しすぎるため、下方修正しても良い終了すってしまう。
...
しかし、このようなヒストグラムを使用すれば、各リクエストがどの程度のレイテンシーに、どのくらい分布しているか、を正確に知られる。
