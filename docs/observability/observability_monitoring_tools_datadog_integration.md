...
言語/フレームワーク/ツールなどについて、専用のメトリクスの元になるデータポイントを収集できるようになる。

アプリケーションとして使用される言語/フレームワークの場合、datadog エージェントがインテグレーション処理を持つため、仮想環境へのインストールは不要である。

Datedog エージェントが稼働する言語/フレームワーク/ツールを自動的に認識してくれる。
...
言語/フレームワーク/ツールなどについて、専用の分散トレースを収集できるようになる。
...
AWS リソースが作成したメトリクス/ログ/分散トレースを Datadog へフォワーディングできるようにする。

> - https://docs.datadoghq.com/integrations/amazon_web_services/?tab=roledelegation#setup

`(1)`

: Datadog の AWS アカウントの登録画面で、CloudFormation による自動セットアップを選択する。
...
: CloudFormation を実行し、メトリクス/ログ/分散トレースをフォワーディングする Lambda や IAM ロールを作成する。

     このIAMロール (`DatadogIntegrationRole`) には、DatadogのIAMユーザー (`464622532012`) に委譲できるように、AWS STSの設定がなされている。

     Datadogの設定画面にアカウントIDとロール名を入力することで、設定が完了する。
...
: 収集したデータポイントを可視化できるように、各 AWS リソースのインテグレーションをインストールする。
...
: ログを収集できるように、Lambda のトリガーとして Amazon CloudWatch Logs や Amazon S3 を設定する。
...
: トリガーとする AWS リソースの命名によって、ログの `service` 属性の値が変わる。
...
Amazon EC2、Amazon ECS、Lambda で料金が発生し、それ以外の AWS リソースは無料である。

> - https://docs.datadoghq.com/account_management/billing/aws/

<br>

## 05. PHP-FPM インテグレーション

PHP-FPM インテグレーションをインストールした場合、収集したデータポイントを可視化できるようになる。
...
Nginx インテグレーションをインストールした場合、収集したログとメトリクスを可視化できるようになる。
