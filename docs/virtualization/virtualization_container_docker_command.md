...
キャッシュ無しで、指定の Dockerfile を基に、コンテナイメージをビルドする。失敗したときは削除するように、`--force-rm` オプションを有効化するか否かを設定する。
...
`--push` オプションを使用すると、ビルド後にコンテナレジストリへプッシュできる。
...
`--platform` オプションで、任意の CPU アーキテクチャに対応するコンテナイメージをビルドできる。
...
Dockerfile の `COPY` コマンドでコンテナ内に配置したファイルは、変更のたびにコンテナイメージをビルドするのが面倒である。そのため、ホストからコンテナにコピーし、再度読み出す。
...
json-file ドライバーを使用しているときに、ログファイルの出力先を確認する。
...
ホストで作成したコンテナイメージを、指定した DockerHub のユーザーにアップロードする。
...
ホストで作成したコンテナイメージを、指定した ECR にアップロードする。

ECR はタグ名がやや特殊のため、事前にタグを付け替える必要がある。
...
コンテナ内の `/etc/hosts` ファイルで、コンテナのプライベート IP アドレスを確認できる。
...
これは Dockerfile で EXPOSE 命令として設定してもよい。
...
コンテナの起動時に `bash` プロセスを実行すると、以下のようなエラーが発生する。
...
Dockerfile でボリュームマウントを実行する場合、コンテナ側のマウントポイントを設定する。

docker エリアのマウントポイントは、自動的に作成される。

Docker Compose で実行することが推奨である。
