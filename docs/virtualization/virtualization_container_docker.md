...
本番環境で，ホストからコンテナにバインドマウントを実行することは技術的に可能である．しかしバインドマウントでは，ホストとコンテナ間が依存し合うため，仮にホストとコンテナのどちらかのアプリケーションのコードが変化してしまった時に，もう一方も影響が出る．対して，バインドマウントでは，コンテナ側のコードはホストにあるdockerエリア内のボリュームを変更できず，仮にコンテナ側のアプリケーションのコードが変化してもホスト側に影響がないため，より安全である．
...
アプリケーションが処理を実行する時，大量のパッケージが読み出される．この時，ホスト上で稼働するコンテナのパフォーマンスは，```COPY```，NFS > ボリュームマウント > バインドマウント，の順で高くなる．
