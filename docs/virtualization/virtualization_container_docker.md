...
各イメージレジストリ (例：DockerHub、Amazon ECR、Google Container Registry、Artifactory、Harborなど) には、カスタマイズするうえでのベースとなるベースイメージが提供されている。
...
Amazon ECSコンテナのawsfirelensドライバーは、fluentdドライバーをラッピングしたものである。
...
標準出力/標準エラー出力に出力されたログをAmazon CloudWatch-APIに送信する。
