...
本番環境で，ホストからコンテナにバインドマウントを実行することは技術的に可能である．しかしバインドマウントでは，ホスト側のコードがコンテナと密に結合しており，コンテナのアプリケーションのコードが何らかの理由（悪意ある攻撃も含む）で変化してしまった時に，ホスト側のコードにも影響が出る．対して，バインドマウントでは，ホスト側のコードはdockerエリアとしてホストから切り離されており，コンテナは『root権限』でなければホストにあるdockerエリア内のボリュームを変更できないようになっている．これにより，仮にコンテナ側のアプリケーションのコードが変化してもホスト側に影響がない．そのため，バインドマウントより安全である．
