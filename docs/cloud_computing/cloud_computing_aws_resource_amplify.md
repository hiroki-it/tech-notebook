...
ただし、Amazon Route 53の設定は表示されており、AWS Amplifyが追加したDNSレコードをユーザーが編集できるようになっている。
...
プルリクごとに実行環境を作成できると、例えば、`1` 個のプルリクをデザイナーサイドやビジネスサイド (例：SEOなど) がレビューできるようになる。

ただし、App Runnerを使用したほうがよいかもしれない。
...
SSGの場合、裏側ではAmazon S3、Amazon CloudFront、Amazon Route 53などが作成され、静的ホスティングが実行される。
