...
（４）パーティションを管理するファイルシステムのタイプを確認する。今回は```ext4```タイプである。
...
（５）今回、EBSボリューム内にパーティションがある。そのため、```growpart```コマンドでパーティションと、パーティションの番号を指定し、パーティションのサイズを拡張する。パーティションがなければ、この手順は不要である。改めて```lsblk```コマンドを実行すると、パーティションのサイズが拡張されている。
...
（６）もし、パーティションを管理するファイルシステムのタイプが```ext4```タイプであった場合、```resize2fs ```コマンドでファイルシステムを指定し、ファイルシステムのサイズを拡張する。

```bash
# 空き領域の100%を使用して拡張する。
$ sudo resize2fs <ファイルシステム>
```
...
（７）改めて```df```コマンドを実行すると、パーティションを管理するファイルシステムを拡張できたことを確認できる。
...
（６）もし、パーティションを管理するファイルシステムのタイプが```xfs```タイプであった場合、```xfs_growfs```コマンドでマウントポイントを指定し、ファイルシステムを拡張する。もし、```xfs_growfs```コマンドがない場合は、インストールする。
...
（７）改めて```df```コマンドを実行すると、パーティションを拡張できたことを確認できる。
