...
（６）もし```ext4```タイプであった場合、```resize2fs ```コマンドでEBSボリュームに紐づくデバイスファイルを指定し、デバイスファイルのサイズを拡張する。
...
Filesystem  Type  Size  Used  Avail  Use%  Mounted on
/dev/xvda1  ext4   16G  1.9G    14G   12%  /var/lib     # EBSボリュームに紐づくデバイスファイル
...
（６）もし```xsf```タイプであった場合、```xfs_growfs```コマンドでEBSボリュームに紐づくマウントポイントを指定し、デバイスファイルのサイズを拡張する。もし、```xfs_growfs```コマンドがない場合は、インストールする。
