...
| 名前                | 種類         | 役割                                                                                                           | 配置例                  |
| ------------------- | ------------ |--------------------------------------------------------------------------------------------------------------| ----------------------- |
| publicサブネット    | パブリック   | 非武装地帯として機能する。Internet Gatewayを介して、パブリックネットワークからのインバウンド通信を待ち受ける。                                                 | ALB、NAT Gateway        |
| protectedサブネット | プライベート | 内部ネットワークとして機能する。パブリックネットワークからアクセスできず、同じVPC内からのインバウンド通信のみを待ち受ける。また、publicサブネットのNAT Gatewayを介してアウトバウンド通信を送信する。 | アプリケーション、Nginx |
| privateサブネット   | プライベート | 内部ネットワークとして機能する。パブリックネットワークからアクセスできず、前述のprotectedサブネット内からのインバウンド通信のみを待ち受ける。また、アウトバウンド通信を送信しない。                               | RDS、Redis              |
...
| 名前                  | 種類         | 役割                                          | 配置例                  |
| --------------------- | ------------ |---------------------------------------------| ----------------------- |
| frontendサブネット    | パブリック   | 非武装地帯として機能する。ロードバランサー、ルーター、踏み台サーバー、を配置する。   | ALB、NAT Gateway        |
| applicationサブネット | プライベート | 内部ネットワークとして機能する。appサーバー、リバースプロキシサーバー、を配置する。 | アプリケーション、Nginx |
| datastoreサブネット   | プライベート | 内部ネットワークとして機能する。dbサーバーを配置する。                | RDS、Redis              |
