...
| 名前                | 種類         | 役割                                                                                                           | 配置例                  |
| ------------------- | ------------ |--------------------------------------------------------------------------------------------------------------| ----------------------- |
| publicサブネット    | パブリック   | 非武装地帯として機能する。インターネットゲートウェイを介して、パブリックネットワークからのインバウンド通信を待ち受ける。                                                 | ALB、NAT Gateway        |
...
| applicationサブネット | プライベート | 内部ネットワークとして機能する。アプリケーション、リバースプロキシ、を配置する。         | アプリケーション、Nginx |
| datastoreサブネット   | プライベート | 内部ネットワークとして機能する。データベースを配置する。                                 | RDS、Redis              |
...
| ------------- | -------- | ------------------------------------------------------------ | ------------------------------------------------ |
| ***LogMetrics | カウント | ログステータスの検出数をデータポイントとする。メトリクスと検出ルールはフィルターパターンで作成できる。 | ・統計 : 期間内合計数<br>・期間 : ```5```分<br>・閾値 : ```>= 1``` |
