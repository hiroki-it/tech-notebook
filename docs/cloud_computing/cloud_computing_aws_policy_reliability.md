...
| ------------- | -------- | ------------------------------------------------------------ | ------------------------------------------------ |
| ***LogMetrics | カウント | ログステータスの検出数をデータポイントとする。メトリクスと検出ルールはフィルターパターンで作成できる。 | ・統計 : 期間内合計数<br>・期間 : 5分<br>・閾値 : >= 1 |
...
| TargetConnectionErrorCount     | カウント | ターゲットグループ内のターゲットへの通信でエラーが発生した数をデータポイントとする。 |                                                              |
| TargetTLSNegotiationErrorCount | カウント | ターゲットグループ内のターゲットへのHTTPS通信でエラーが発生した数をデータポイントとする。 |                                                              |
...
参考：https://docs.aws.amazon.com/ja_jp/AmazonElastiCache/latest/red-ug/CacheMetrics.WhichShouldIMonitor.html
...
| SwapUsage         | バイト数    | 仮想メモリのスワップ領域の使用量をデータポイントとする。 | ・統計 : 期間内最大サイズ<br/>・期間 : ```5```分<br/>・閾値 : ```>= 50```GB | 使用可能な最大メモリを超えると、Redisはスワップ領域を使用する。<br>参考：https://zenn.dev/dehio3/scraps/710a9714ce9496 |
