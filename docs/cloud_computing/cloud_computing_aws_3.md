...
以下に、EC2インスタンスの読み出しのみ権限（```AmazonEC2ReadOnlyAccess```）を紐付けできるポリシーを示す。このIAMポリシーには、他のAWSリソースに対する権限も含まれている。
...
以下のインラインポリシーが紐付けられたロールを持つAWSリソースは、任意のSystems Managerパラメーターストアを取得できるようになる。
