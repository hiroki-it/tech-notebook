...
ECS/EKSのパフォーマンスに関するメトリクスを収集する。ECSクラスター/EKS Cluster、ECSサービス、ECSタスク、ECSコンテナ、単位で監視できるようになる。また、コンテナ間の繋がりをコンテナマップで視覚化できるようになる。ECS/EKSのアカウント設定でContainerインサイトを有効化する必要がある。
