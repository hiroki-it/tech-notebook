...
| プレビュー             | GitHubのプルリク上にAmplify環境のURLを通知する。 | 執筆時点（2021/02/07）では、プルリクを新しく作成しても、これは自動で登録されない。そのため、その都度手動で登録する必要がある。                                              |
...
| CloudWatchログの設定     | API GatewayがCloudWatchログにアクセスできるよう、ロールを設定する。                         | 1つのAWSアカウントにつき、1つのロールを設定すれば良い。            |
