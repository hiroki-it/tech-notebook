...
| 分類群         | 説明                                                                        |
| -------------- |---------------------------------------------------------------------------|
| ディメンション | インスタンスの設定値を単位とした収集グループ（例：インスタンス別、スペック別、AZ別、など）のこと。 |
...
| 欠落データの処理                 | データポイントが発生しないことをどう判定するかを設定する。 | データポイントが一切発生しないことを正常と見なす場合（例：エラーイベント、など）は『```notBreaching```』とし、継続的に発生することを正常とする場合（例：CPU使用率、メモリ使用率、など）は、『```breaching```』とする。<br>ℹ️ 参考：https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/AlarmThatSendsEmail.html#alarms-and-missing-data |
