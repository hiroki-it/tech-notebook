---
title: 【IT技術の知見】コスト最適化＠AWS
description: コスト最適化＠AWSの知見を記録しています。
---
...
ECRに保存されたイメージの合計サイズが```500MB```を超えると、請求が発生するため、旧いイメージを定期的に削除する必要がある。
...
Fargateでは、FargateワーカーNode内でPodが使用したリソースサイズによって、料金が決定する。そのため、冗長化するPod数を減らすことにより、料金を抑えられる。
