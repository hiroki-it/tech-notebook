...
資格情報ファイルを使用して、ServiceAccount にログインする。

資格情報ファイルは使用後に削除したほうがよいらしい。
...
Google Cloud CLI による Google Cloud リソースへのアクセスを認証するために使用する。

`~/.config/gcloud/application_default_credentials.json` ファイルを作成し、資格情報を定義する。

また、これ使用して Google Cloud にログインする。
...
Google Cloud SDK による Google Cloud リソースへのアクセスを認証するために使用する。
...
これにより、Google Cloud CLI のための資格情報 (`~/.config/gcloud/application_default_credentials.json` ファイル) と Google Cloud SDK のための資格情報が更新される。
...
Configuration に設定されているプリンシパルを削除する。
...
複数の Configuration がある場合に、これらを切り替える。
...
すべての Configuration と、アクティブプリンシパルとを一覧で取得する。
...
現在、非アクティブになっている Configuration の特定の名前を変更する。
...
Google Cloud Logging からログを読み出す。
...
Google Cloud Logging にログを送信する。

アプリケーションの代わりに特定の条件に合致するログを送信したい場合に、使用するとよい。
