...
![Amazon SESとは](https://raw.githubusercontent.com/hiroki-it/tech-notebook-images/master/images/Amazon SESとは.png)
...
Amazon SESは連携するAWSリソースと同じリージョンに作成しなければならない。

### Sandboxモードの解除

Amazon SESはデフォルトではSandboxモードになっている。
...
アプリケーションがAmazon SESを経由してメールを送信するとき、アプリケーションに対して、Amazon SESがユーザー名とパスワードを使用した認証する。
...
Amazon SESのIPアドレスも例外ではなく、スパムとして判定されてしまうことがある。

一度スパムとして判定されてしまうと、そのAWSアカウントのAmazon SESをしばらく使用できなくなってしまう。

これの対策として、メインのAmazon SES (特に本番環境) がスパム判定された場合の予備として、Amazon SESのみを持つAWSアカウント (本番環境Amazon SES) を追加で作成しておくとよい。
