...
Amazon API Gatewayのインポートに当たり、OpenAPIの `yaml` ファイルにキーを新たに実装する必要がある。
...
- Amazon API Gatewayが `security` キーのルート定義に非対応のため、冗長ではあるが、各HTTPメソッドに個別に定義する。
...
Amazon API Gatewayのエンドポイントに対して、以下のパラメーターでリクエストを送信すると、レスポンスを確認できる。
