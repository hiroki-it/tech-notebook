...
パブリックアクセスが無効化されたAWS S3に対して、ALBへのアクセスログを保管したい場合、バケットポリシーを設定する必要がある。

バケットポリシーには、ALBからAWS S3へのログ書き込み認可スコープを実装する。『`"AWS": "arn:aws:iam::582318560864:root"`』では、`582318560864`はALBアカウントIDと呼ばれ、リージョンごとに値が決まっている。
...
パブリックアクセスが無効化されたAWS S3に対して、CloudFrontからのルーティングで静的ファイルを読み出したい場合、バケットポリシーでCloudFrontの識別情報を設定する必要がある。
...
執筆時点 (2020/10/08) では、パブリックアクセスが無効化されたAWS S3に対して、CloudFrontへのアクセスログを保管できない。
