...
Amazon EventBridgeでは、どのようなJSONのイベントをターゲットに送信したかを確認できない。

そこで、デバッグ時はAmazon EventBridgeのターゲットにAWS Lambdaを設定し、イベント構造をログから確認する。
...
対象のAWSリソースで任意のイベントが発生したときに、Amazon EventBridgeからAWS Lambdaに送信するように設定する。
...
その後、入力テンプレートにて送信するJSONを定義する。例えばここでは、Slackに送信するJSONを出力する。
