...
他のAWSリソース (例：SMオートメーション、Amazon EventBridgeなど) を定期的に実行するCronとして使用する。
...
IaCのように柔軟なプロビジョニングを実行するというより、決まりきったプロビジョニング (例：AWS上で稼働するロードテストツールの実行からレポート作成まで) を実行するのに向いている。
...
パラメーターのタイプは全て『SecureString』としたほうが良い。
...
パラメーターストア上で変数は暗号化されており、EC2 (Amazon ECSやAmazon EKSのコンテナのホストを含む) で参照するときに復号される。
...
SMパラメーター名は、『`/<リソース名>/<変数名>`』とすると、わかりやすい。
...
EC2 (Amazon ECSやAmazon EKSのコンテナのホストを含む) へ通信できるようにする。

SSH公開鍵認証とは異なり、Internet Gateway経由ではなく、ssmmessagesエンドポイント経由でインスタンスへリクエストを送信できる。
