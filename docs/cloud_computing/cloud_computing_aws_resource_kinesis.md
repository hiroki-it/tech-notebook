...
| IAM ロール         | Kinesis Data Firehose がAWS リソースにデータをフォワーディングできるように、認可スコープを設定する。                                                                                                                 | Kinesis では IAM ロールの細やかな設定が正しく動作しないことがあり、最小認可スコープを諦め、FullAccess 権限のロールを付与してしまうほうがよい。最低限、Amazon CloudWatch Logs とS3 の認可スコープが必要である。 |
