...
| クライアント証明書       | SSL証明書をAWS API Gatewayに割り当てる。                                                                                           | APIが、AWS API Gatewayからルーティングされたリクエストであること識別できるようになる。                                                                                  |
| AWS CloudWatchログの設定 | AWS API GatewayがAWS CloudWatchログにリクエストを送信できるよう、ロールを設定する。                                                | `1`個のAWSアカウントにつき、`1`個のロールを設定すれば良い。                                                                                                             |
...
AWS CloudWatchログにAWS API Gatewayの実行ログを送信するか否かを設定できる。
...
AWS CloudWatchログにAWS API Gatewayのアクセスログを送信するか否かを設定できる。
...
| エッジ最適化 | AWS API Gatewayのエンドポイントに対するリクエストを、CloudFrontのエッジサーバーで受け付ける。 |
