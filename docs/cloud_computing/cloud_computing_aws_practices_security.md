...
社内の送信元IPアドレスのみ許可した状態で、CIツール (例：GitHub Actions、CircleCI、GitLab CI、など) が社内サービスにリクエストできるようにする。
...
アプリケーションでは、特定のURLパスにリクエストできる送信元IPアドレスを社内だけに制限する。
...
アプリケーション全体にリクエストできる送信元IPアドレスを、特定のIPアドレスだけに制限する。
