...
| 名前                  | 種類         | 役割                                                                                                                                                                                        |
| --------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frontendサブネット    | パブリック   | 非武装地帯として動作する。`L7`ロードバランサー (AWS ALB) 、ルーター (AWS NAT Gateway) 、`ssh`コマンド用の踏み台サーバー (AWS EC2) を配置する。                                              |
| applicationサブネット | プライベート | 内部ネットワークとして動作する。Appサーバー (AWS EC2、AWS ECS、AWS EKS) 、リバースプロキシサーバー (AWS EC2、AWS ECS、AWS EKS) 、`aws ssm`コマンド用の踏み台サーバー (AWS EC2) を配置する。 |
...
`ssh`コマンドを実行してポートフォワーディングを実施する場合、踏み台サーバー (AWS EC2) をパブリックサブネットに配置する必要がある。
...
`aws ssm start-session`コマンドを実行してポートフォワーディングを実施する場合、踏み台サーバー (AWS EC2) をプライベートサブネットに配置できる。
