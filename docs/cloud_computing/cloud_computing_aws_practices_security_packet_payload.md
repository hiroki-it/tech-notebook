...
| 名前                  | 種類         | 役割                                                                                 | 配置例                   |
| --------------------- | ------------ | ------------------------------------------------------------------------------------ | ------------------------ |
| frontendサブネット    | パブリック   | 非武装地帯として動作する。`L7`ロードバランサー、ルーター、踏み台サーバーを配置する。 | AWS ALB、AWS NAT Gateway |
| applicationサブネット | プライベート | 内部ネットワークとして動作する。appサーバー、リバースプロキシサーバーを配置する。    | AWS EC2、Fargate         |
...
リクエストを受信するAWSリソース (例：AWS ALB、Aurora AWS RDS、AWS CloudFront、AWS EC2/AWS ECS/AWS EKSなど) に紐づけられる。
