...
| DB認証                  | DBに接続するための認証方法を設定する。                                                                                                                             | 各DBインスタンスに異なるDB認証を設定できるが、全てのDBインスタンスに同じ認証方法を設定すべきなため、AWS AuroraのDBクラスターでこれを設定すれば良い。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 削除保護                | AWS AuroraのDBクラスターの削除を防ぐ。                                                                                                                                         | AWS AuroraのDBクラスターを削除するとクラスターボリュームも削除されるため、これを防ぐ。補足として、AWS AuroraのDBクラスターの削除保護になっていてもDBインスタンスは削除できる。DBインスタンスを削除しても、再作成すればクラスターボリュームに接続されて元のデータにリクエストを送信できる。<br>https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_DeleteCluster.html#USER_DeletionProtection                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
コンソール画面にて、AWS AuroraのDBクラスター内の全てのDBインスタンスを削除すると、AWS AuroraのDBクラスターも自動的に削除される。

一方で、AWS-APIをコールして全てのDBインスタンスを削除する場合、AWS AuroraのDBクラスターは自動的に削除されずに、空の状態になる。
...
### クラスターボリュームのみのセカンダリーAWS AuroraのDBクラスター (ヘッドレスセカンダリーAWS AuroraのDBクラスター)

『ヘッドレスセカンダリーAWS AuroraのDBクラスター』ともいう。

コスト削減などを目的として、クラスターボリュームのみのセカンダリAWS AuroraのDBクラスターがある。

セカンダリーAWS AuroraのDBクラスターのリードレプリカを削除すると、ヘッドレスセカンダリーAWS AuroraのDBクラスターを作成できる。
...
マイナーバージョンのアップグレードの場合は、セカンダリーAWS AuroraのDBクラスターからアップグレードする。
...
AWS AuroraのDBクラスター内の全てのDBインスタンスが同じAZに配置されている場合、あらかじめ異なるAZにリードレプリカを新しく作成する必要がある。
