...
1つのVPC内には複数のサブネットが入る。そのため、サブネットのIPアドレス範囲は、サブネットの個数だけ含めなければならない。また、VPCが持つIPアドレス範囲から、VPC内の各AWSリソースにIPアドレスを割り当てていかなければならない。VPC内でIPアドレスが枯渇しないように、以下の手順で、割り当てを考える。
