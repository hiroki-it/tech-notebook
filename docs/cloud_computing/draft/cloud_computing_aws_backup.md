...
- 専用のリポジトリを作成し、CircleCIコンテナ内で実行できるようにする。認証情報を環境変数として登録できるため、シェルスクリプトが漏洩しても、社内リソースにはアクセスできない。この時、日付などの引数は環境変数としてその都度登録する。
...
この時、日付を引数として、指定した過去日のバックアップをクローンできるようにする。
