...
IAMユーザーに対して、一時的にロールをAssume (委譲) するサービスのこと。

これにより、AWSリソースへリクエストできるようになる。
...
aws-cliを活用して、aws-cli用IAMユーザーが特定のAWSリソースへリクエストできるようにする。
...
- リクエスト制限の仕組みが単純であり、AWSに関わりが少ないエンジニアにとって理解しやすい。
...
- 専用のリポジトリを作成し、CircleCIコンテナ内で実行可能にする。資格情報を環境変数として登録できるため、シェルスクリプトが漏洩しても社内リソースにはアクセスできない。この時、日付などの引数は環境変数としてその都度登録する。
- もし興味を持つ人がいれば勉強会を開く
