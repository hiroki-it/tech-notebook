...
- テスト環境に Redis、RDS、ElasticSearch を作成し、ストレステストを実施したい。
...
現在の欠点は、AWS リソースに対する『**アクセス**』+『**作成**』+『**削除**』という一連の操作を自動化することで解決できると考えた。
...
## IAM ポリシー、 aws-cli、シェルスクリプトによる自動化
