...
| 属性                                                                       | 標準ではインスタンスタイプに備わっていない機能を表す。       | `a` (AMD CPU)、`g` (Graviton CPU) 、`i`(インテルCPU) など     |
...
| -------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ---------- | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| ほとんど全てのアプリケーションサーバー                                                             | 全般                                                    | `M`系      | 世代数は大きいほど高性能で値段も安い。そのため、その時点で一番新しい世代を選ぶ。 | サイズが一つ大きくなると、ハードウェアリソーススペックと料金が`2`倍になる。ネットワーク帯域はあまり変わらない。アプリケーションが必要とするサイズを選ぶ。 | ★        |
| ほとんど全てのアプリケーションサーバー                                                             | バースト系 (ハードウェアリソース要求がスパイクする場合) | `T`系      | 同上                                                                             | 同上                                                                                                                                                      | ★★       |
