...
つまりEC2が作成されると、ボリューム内に保存されたファイルは、ブロックデバイスを経由して、マウントポイントのディレクトリ内に作成される。

また反対に、マウント先ディレクトリ内に保存されたファイルは、ルートデバイスボリューム内に保存される。
...
EBSで保存されているルートデバイスボリュームで、推奨の方法である。
...
インスタンスストアで保存されているルートデバイスボリュームで、非推奨の方法である。
