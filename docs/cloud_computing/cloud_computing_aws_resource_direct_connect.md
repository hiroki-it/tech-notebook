...
専用線方式のWANとして動作し、AWS側の単一のプライベートネットワーク (Amazon VPC) と、ユーザー側のプライベートネットワーク (例：オンプレミス) の間を接続する。

専用線方式では、帯域を占有でき、ネットワーク間は安定した速度で通信できる。

注意点として、DirectConnectは、それ専用の中継Amazon VPC内に作成する。
...
DirectConnectと複数のプライベートネットワーク (Amazon VPC) をアタッチメントで接続する。

TransitGatewayが必要になるため、各Amazon VPC上にそのためのルートテーブルを作成する必要がある。
