...
この時、必要以上にコストがかかっているAWSリソースを見つければ、次週のミーティングまでに改善する。
...
- Amazon EC2、Amazon EC2ベースのAWSリソース (例：AWS Fargate)
- Amazon RDS、Amazon RDSベースのAWSリソース (例：Amazon Aurora)
- AWS ALB
...
マイクロサービスアーキテクチャでは、ALBが増えがちであり必要経費であるが、地味に高い。
...
また、複数のAWS EBSボリュームをAmazon EC2に紐付けている場合は、その合計サイズで料金が発生する。
...
Amazon ECRに保管したイメージの合計サイズが `500MB` を超えると、請求が発生するため、古いイメージを定期的に削除する必要がある。
...
Auto Scalingグループを使用している場合、予定されたアクションで起動と停止のスケジュールを設定できる。
