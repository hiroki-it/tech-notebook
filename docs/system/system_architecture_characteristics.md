...
ただ、非機能要件や品質特性だとその性質を正しく表現できないとして、アーキテクチャ特性と表現したほうがよい。
...
もし、`1` 台当たりの負荷を `60`%未満にすると、『`60`% × `3` = `180`%』になる。

これであれば、1 台停止したとしても、`180`% > `200`%であるため、システム全体として稼働し続けられる。

同様にして、通常時 `4` 台であれば、負荷を `70`%未満にするとちょうどいい。
...
また、いずれかのコンポーネントで障害が起こった場合は、正常なコンポーネントへルーティングする。
...
インバウンド通信のルーティング先を待機中インスタンスに切り替えるとき、手動で切り替えることを『スイッチオーバー』、自動的に切り替えることを『フェイルオーバー』という。
...
スプリットブレイン対策として、STONITH (問題があるほうのインスタンスを強制的に再起動する) がある。
...
システムに割り当てられたハードウェアリソースのなかで、十分に処理できるか、どれだけ効率的に処理できるかどうできるかの、程度を表す。
...
インスタンス当たりの CPU やメモリの並列処理数を向上させることにより、システム全体としての性能を高める。
...
システムの利用ユーザーが特定の機能 (例：何らかの登録/参照/変更/削除) を使用するときにどれだけ満足 (例：目的達成まで至りやすい、エラーがわかりやすい) できるかの、程度を表す。
...
| 拡張性     | 将来的に予想される DB レコード数増加率など                                       | ...              | 将来的に予想される初期データを DB に用意したうえで、処理速度に問題が起こらないかどうかを検証する。 |
| 安全性     | DoS 攻撃など                                                                     | ...              | 脆弱性診断を実施する。                                                                             |
| 保守性     | 機能修正、障害対応、アップグレード規約など                                       | ...              | 初期データを、追加/変更/削除できるかどうかを検証する。                                             |
...
## 11. システムのドメイン分野の非機能要件

システムのドメイン分野に応じて非機能要件の程度が異なる。

非機能要件の程度は法規制やユーザー体験に特に影響を受ける。
...
| ドメイン     |    留意点    | 可用性 | 性能 | 拡張性 | 安全性 (セキュリティ) | 保守性 | 運用性 | 可搬性 | 信頼性 | 使いやすさ |
| ------------ | :----------: | :----: | :--: | :----: | :-------------------: | :----: | :----: | :----: | :----: | :--------: |
| フィンテック |    法規制    |   高   |  中  |   中   |         極高          |   中   |  厳格  |   中   |  極高  |     中     |
| 医療         |    法規制    |  極高  |  中  |   中   |         極高          |   中   |  厳格  |   中   |  極高  |     中     |
| 公共         |    法規制    |  極高  |  中  |   中   |         極高          |   中   |   高   |   中   |  極高  |     中     |
| ゲーム       | ユーザー体験 |   高   | 極高 |   高   |          中           |   中   |   高   |   中   |   中   |     高     |
| 動画配信     | ユーザー体験 |   高   | 極高 |   高   |          中           |   中   |   高   |   中   |   高   |     高     |
| EC           |     なし     |  極高  | 極高 |   高   |          高           |   中   |   高   |   中   |   中   |     高     |
| HR           |     なし     |   中   |  中  |   中   |          高           |   中   |   中   |   中   |   高   |     中     |
| 物流         |     なし     |   高   |  中  |   高   |          高           |   中   |   高   |   中   |   高   |     中     |
| 製造         |     なし     |  極高  |  高  |   高   |          高           |   中   |   中   |   中   |  極高  |     中     |
| 教育         |     なし     |   中   |  中  |   中   |          中           |   中   |   中   |   中   |   中   |     高     |
| 広告         |     なし     |   高   | 極高 |  極高  |          高           |   中   |   高   |   中   |   中   |     高     |
