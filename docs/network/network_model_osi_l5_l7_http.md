...
リクエストがプロキシを経由して他のシステムにリダイレクトされる場合に、プロキシまではリクエストが送信されている。
...
リトライすると解決する可能性があり、リクエストを繰り返しても状態が変わらずに冪等性がある (二重処理にならない) があるステータスコードは、リトライしてもよい。

- `408`
- `429`
- `500`
- `502`
- `503` (POSTリクエストで二重処理になる可能性を捨てきれない)
- `504`
