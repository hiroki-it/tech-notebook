...
『IPマスカレード』ともいう。パブリックネットワークとプライベートネットワークの双方向への通信時に、IPアドレスだけでなく、ポート番号も変換すること（```プライベートIPアドレス``` ⇄ ```グローバルIPアドレス```）。また、プライベートIPアドレス同時を相互変換することにも使用される（```プライベートIPアドレス``` ⇄ ```プライベートIPアドレス```）。1つのグローバルIPアドレスに対して、複数のプライベートIPアドレスを紐付けられる。AWSやGCPなどで使用されているようなNATルーターはこちらであり、VPC内のプライベートIPアドレスに単一のパブリックIPアドレスを割り当てるようになっている。
