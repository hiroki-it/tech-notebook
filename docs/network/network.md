...
| ---------- |--------------------------------------------------------------------------------|-------------------------------------------------------|---------------------------------------------------------------------------|
| DNATの場合 | 外部IPアドレスをネットワークに公開する。外部IPアドレスを宛先IPアドレスとしたパケットを受信した場合に、そのパケットをシステム内部にルーティングする。 | NATルーターを通過した後、パケットの宛先IPアドレスを内部IPアドレスに書き換える。          | 無し                                                                        |
| SNATの場合 | NATルーターを通過した後、パケットの送信元IPアドレスを外部IPアドレスに書き換える。                                   | NATルーターが受信したパケットのうちで、指定した『送信元IPアドレス』を持つパケットのみルールを適用し、そうでないパケットはそのまま通過させる。 | NATルーターが受信したパケットのうちで、指定した『宛先IPアドレス』を持つパケットのみルールを適用し、そうでないパケットはそのまま通過させる。 |
