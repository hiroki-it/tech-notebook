...
OSI 参照モデルの各層と通信機器の間の対応関係は以下の通りである。

`(1)`

: プライベートネットワークにクライアント PC がある。

`(2)`

: クライアント PC にて、Web ブラウザのアプリケーションのプロセスが、OSI アプリケーション層、OSI プレゼンテーション層、OSI セッション層で稼働している。

     ここで、パケットを作成する。

`(3)`

: パケットは、クライアント PC の OSI トランスポート層、OSI ネットワーク層、OSI データリンク層、OSI 物理層を経る。各層で、パケットにヘッダー情報が追加される。
...
: ルーターは OSI ネットワーク層に属する。

そのため、パケットはより上層へ一度戻ることになる。
...
: パケットは、OSI データリンク層、OSI 物理層 (NIC：Network Interface Card (例：LAN アダプタ、LAN ボード、LAN カード) ) 、リピータ、LAN ケーブル、OSI ネットワーク層、OSI トランスポート層を経る。
...
パケットは、パケットペイロードとヘッダー情報から構成される。

パケットペイロードは、IP ヘッダー、トランスポートヘッダー、アプリケーションデータからなる。
...
パケットペイロードは、OSI の層を経るたびにヘッダーを追加していく。
...
: プレゼンテーション層 (`L6`) にて、パケットペイロードを文字コード変換し、また暗号化/復号する。さらに `L6` ヘッダーを追加する。

    例えば、HTTPであればWebページやリクエストメッセージを文字コード変換し、また暗号化/復号する。
