...
ハードウェア仮想化 (仮想サーバー) や OS 仮想化 (コンテナ) を採用する場合、これらにもポートが存在することになる。
...
アプリケーションのプロセスへのパケット送受信で、プロセスを区別する目的で各プロセスに割り当てられる番号。

アプリケーションには、それぞれポート番号が割り当てられている。

トランスポート層は、ポート番号を元にして、特定のプロセスにパケットを送信する。
...
| DNS                                       | `53`                   |
| FTP                                       | `21`                   |
| HTTP/1.1、HTTP/2 (例：gRPC、GraphQL など) | `80`、`591`            |
| HTTPS                                     | `443`                  |
| POP2                                      | `109`                  |
| POP3                                      | `110`                  |
| SSH                                       | `22`                   |
...
IANA が登録申請を受けて公開しているポート番号。

企業が作成した独自のアプリなどに対して割り当てられる。

| 企業例             | `L7` (アプリケーション層) のプロトコル    | よく使用するポート番号 |
| ------------------ | ----------------------------------------- | ---------------------- |
| 企業が共有している | HTTP/1.1、HTTP/2 (例：gRPC、GraphQL など) | `8080`                 |
| ...                | ...                                       | ...                    |
...
もちろん、そのポート番号で待ち受ける必要がなければ、脆弱性から閉じておくほうがよい。
...
サーバー内の特定のポート番号のアプリケーションに対して、パケットが送信されてきたとき、これを異なるポート番号のアプリケーションにフォワーディングすること。

SSH と組み合わせると、SSH ポートフォワーディングを実現できる。
...
ポートスキャナを使用することで、各ポート番号に接続し、応答があるか否かを調べて一覧表示できる。

また、どのようなソフトウェアが応答するかも確認できる。
