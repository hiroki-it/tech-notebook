...
処理が `var_dump` 関数を通過していないことが原因。任意の場所に `var_dump("文字列")` を記述し、どこに記述したときに文字列が出力されるか否かを確認する。
...
任意の場所に `exit` 関数を記述し、どこに記述したときに、`500` ステータスが起こらずに処理が完了する (レスポンス無し) か否かを確認する。
