...
アーティファクトの出力先を指定しない場合は、以下の場所に出力する。
...
ビルド対象パスを指定しない場合、ルートディレクトリの go ファイルをビルドの対象とする。
...
また、指定したパス内の go ファイルをビルド対象として、指定したパスにビルドのアーティファクトを作成もできる。
...
コードと外部モジュールに対して `build` コマンドを実行することで、`$GOPATH` 以下の `bin` ディレクトリまたは `pkg` ディレクトリにインストール (配置) する。
...
`HEAD` を指定すると、最新のコミット ID を指定できる。

複数のブランチがある場合は、時系列的に最新のコミット ID である。
...
デフォルトではテストコードがあるモジュールしか解析しないため、すべてのモジュールを含めて解析するために `./...` を指定する。
...
モジュール単位、関数単位 (`-func`) 、全体のカバレッジを指定できる。
...
バイナリで使用している Go のバージョンと、モジュールのバージョンを出力する。
...
Go では、思想的にモジュールのバージョンを固定して運用せず、常に新しいバージョンを強制的に利用させるような仕組みがある。
...
`go get` コマンドは不用意に `go.mod` ファイル上のほかのモジュールの定義も更新してしまうため、非推奨である。
...
`go.mod` ファイルに記載の Go のバージョンを上書きしつつ、`go.sum` ファイルを更新する。
...
(Go の思想的にも) できるだけ新しいバージョンを強制しようとするため、想定するバージョンよりも新しいモジュールをインストールしてしまう可能性がある。

そのため、特定のバージョン (特にコミット ID での指定) は `go get` コマンドでインストールする必要がある。
...
基本的には、モジュールの URL やディレクトリ構成と同じにする。
...
コミット ID やバージョンタグを使用して、パブリックリポジトリから開発中のモジュールをインポートする。

この場合、`go get` コマンドで特定のコミット ID やバージョンタグを指定し、モジュールをインストールする。
...
`GOPRIVATE` 変数にプライベートリポジトリの URL を設定することで、インポートできるようになる。
...
自前共有の全モジュールでモジュール名を置換する必要はなく、プロジェクトのルートパスについてのみ定義すればよい。
...
PHP における `composer.lock` ファイルに相当する。
