...
JSON 型データのリストから複数のオブジェクトを取得する場合、取得したオブジェクトを再びリストに入れる。
...
リストへのパスを表す。もし JSON 型データが起点からリストだった場合は、『`.[]`』になる。
...
パスによる取得結果のなかから、特定のキーや値を持つオブジェクトを取得する。
