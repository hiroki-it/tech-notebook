...
これ使用して、汎用的な UI レンダリングロジックと状態管理ロジックのクラスまたは関数を定義できる。
...
React では、関数コンポーネントで `async` 宣言は使用できない仕様になっている。

非同期関数を `useEffect()` 関数に渡したうえで、これを react コンポーネントにする必要がある。
...
処理負荷を軽減するため、該当の state 変数の状態が変わらない限り処理結果を再利用する。

`useEffect` で複数の state 変数の状態を管理している場合に、一方の変数の状態だけを再利用できる。
