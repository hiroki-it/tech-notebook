...
JavaScript の実行環境にあらかじめ組み込まれたオブジェクト。
...
他の標準ビルトインオブジェクトすべての継承元になっているため、標準ビルトインオブジェクトは、Object が持つ振る舞いと状態を使用できる。
...
以下の関数宣言、関数式、アロー関数による関数式省略の記法が推奨される。
...
オブジェクトの作成時、`constructor()` 関数でオブジェクトを初期化する。
...
オブジェクトからプロパティや関数をコールしたとき、そのオブジェクトにこれらが存在しなければ、継承元まで辿る仕組みを『プロトタイプチェーン』という。
...
コンストラクタ関数内の this は、自身がコールされたオブジェクトを指す。
...
今まで、JavaScript では、`this` の参照先が文脈によって変わることに批判が集まっていた。
