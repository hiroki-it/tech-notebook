...
オブジェクト内のデータにアクセスするには、直接的にデータを処理することはできず、オブジェクト内の関数をコールし、アクセスしなければならない。
...
Setterを持たせずに、`__construct` 関数のみで値の設定を行えば、ValueObjectのような、『イミュータブル』なオブジェクトを実現できる。
...
そこで、そのような関数では、一度コールされて集計を行った後、データに返却値を保持しておく。
...
この定数がコールされたファイルが配置されたディレクトリのパスが、ルートディレクトリ基準で格納されている。
