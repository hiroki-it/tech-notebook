...
Getterでは、状態を取得するのみではなく、何かしらの処理を加えたうえで取得すること。
...
Setterを持たせずに、`__construct` 関数のみを持たせれば、ValueObjectのような、『イミュータブル』なオブジェクトを実現できる。
...
Test01クラスインスタンスの `$property01` に値を設定するためには、インスタンスからSetterをコールする。

Setterは何度でも呼び出せ、その度にデータの値を上書きできる。
...
一方で、Test02クラスインスタンスの `$property02` に値を設定するためには、インスタンスを作り直さなければならない。

つまり、以前に作ったインスタンスの `$property02` の値は上書きできない。

Setterを持たせずに、`__construct` 関数のみを持たせれば、『イミュータブル』なクラスとなる。
...
Entityは、ミュータブルであるため、Setterと `__construct` 関数の両方を持てる。

ValueObjectは、イミュータブルのため、`__construct` 関数しか持てる。
...
共通の処理を実行するGetter (例：値を取得するだけのGetterなど) を無闇に増やしたくない場合に使用することにより、コード量の肥大化を防げる。

PHPには最初からマジック関数は組み込まれているが、自身で実装した場合、オーバーライドされてコールされる。
...
共通の処理を実行するSetter (例：値を設定するだけのSetterなど) を無闇に増やしたくない場合に使用することにより、コード量の肥大化を防げる。

PHPには最初からマジック関数は組み込まれているが、自身で実装した場合、オーバーライドされて呼び出される。
...
Setterを持たせずに、`__construct` 関数のみで値を設定すれば、ValueObjectのような、『イミュータブル』なオブジェクトを実現できる。
...
Setterは何度でもコールでき、その度にデータの値を上書きできる。
...
Setterを持たせずに、`__construct` 関数のみを持たせれば、『イミュータブル』なオブジェクトとなる。
...
どちらも、new演算子と組み合わせて、自身のインスタンスを返却する関数であるが、作成の対象になるクラスが異なる。
...
自身のインスタンス化処理が継承される場合は、`static` 関数を使用したほうが良い。
...
以下の様に、返却値のオブジェクトを使用して、より深い層に連続してアクセスしていく場合…
...
DBレコード区分名をそのまま保存してもよいが、区分名が変わったときにアプリケーションの実装だけでなくDBレコードを変更するのが大変である。

その一方で、DBは区分名ではなく区分値として管理しておけば、DBレコードを変更せず、アプリケーションだけを変更すればよい。
