...
オブジェクト内のデータにアクセスするには、直接的にデータを処理する事はできず、オブジェクト内の関数をコールし、アクセスしなければならない。
...
オブジェクトに対して特定の操作が行われた時に自動的にコールされる特殊な関数のこと。
...
定義されていないデータや、アクセス権のない状態を取得しようとした時、代わりに呼び出される。
...
定義されていない関数や、アクセス権のない関数を取得しようとした時、代わりにコールされる。
...
定義されていない静的関数や、アクセス権のない静的関数を取得しようとした時、代わりに呼び出される。
...
定義されていないデータや、アクセス権のないデータに値を設定しようとした時、代わりにコールされる。
...
自身のインスタンス化処理が継承される場合は、`static`関数を使用した方が良い。
...
特定のイベント名が渡された時、それに対応づけられた関数をコールする。
...
無名関数をコールしたい時は、`call_user_func`関数を使用する。
