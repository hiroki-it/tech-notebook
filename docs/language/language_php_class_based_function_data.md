...
本資料の以降では、振る舞い (関数) と状態 (プロパティ) に分けて、説明していく。
...
オブジェクトのすべてのデータが `public` であるなら、オブジェクト指向における隠蔽のメリットは、特にないかもしれない。
...
カプセル化とは、ソフトウェアの実装方法を外部から隠すこと。

オブジェクト内のデータにアクセスするには、直接的にデータは処理できず、オブジェクト内の関数をコールし、アクセスしなければならない。
...
別ファイルでの関数の呼び出しにはインスタンス化が必要である。

しかし、static 修飾子をつけることにより、インスタンス化しなくともコールできる。

状態は使用せず (静的) 、引数の値のみを使用して処理を実行する関数に対して使用する。
...
Getter では、状態を取得するのみではなく、何かしらの処理を加えたうえで取得すること。
...
#### ▼ マジック関数の `__construct()` 関数

マジック関数の `__construct()` 関数を持たせることにより、このデータを持っていなければならないとい制約を明示できる。

Setter を持たせずに、`__construct()` 関数のみを持たせれば、ValueObject のような、『イミュータブル』なオブジェクトを実現できる。
...
Test01 クラスインスタンスの `$property01` に値を設定するためには、インスタンスから Setter をコールする。

Setter は何度でも呼び出せ、そのたびにデータの値を上書きできる。
...
一方で、Test02 クラスインスタンスの `$property02` に値を設定するためには、インスタンスを作り直さなければならない。

つまり、以前に作ったインスタンスの `$property02` の値は上書きできない。

Setter を持たせずに、`__construct()` 関数のみを持たせれば、『イミュータブル』なクラスとなる。
...
Entity は、ミュータブルであるため、Setter と `__construct()` 関数の両方を持てる。

ValueObject は、イミュータブルのため、`__construct()` 関数しか持てる。

<br>

### マジック関数 (Getter系)

オブジェクトに対して特定の操作が行われたときに自動的にコールされる特殊な関数のこと。

自動的に呼び出される仕組みは謎。

共通の処理を実行する Getter (例：値を取得するだけの Getter など) を無闇に増やしたくない場合に使用することにより、コード量の肥大化を防げる。

PHP には最初からマジック関数は組み込まれているが、自身で実装した場合、オーバーライドされてコールされる。

#### ▼ `__get()` 関数

定義されていないデータや、アクセス権のない状態を取得しようとしたとき、代わりに呼び出される。

関数は定義しているが、データは定義していないような状況で使用する。
...
定義されていない関数や、アクセス権のない関数を取得しようとしたとき、代わりにコールされる。

データは定義しているが、関数は定義していないような状況で使用する。
...
定義されていない静的関数や、アクセス権のない静的関数を取得しようとしたとき、代わりに呼び出される。

自動的にコールされる仕組みは謎。

共通の処理を実行する Setter (例：値を設定するだけの Setter など) を無闇に増やしたくない場合に使用することにより、コード量の肥大化を防げる。

PHP には最初からマジック関数は組み込まれているが、自身で実装した場合、オーバーライドされて呼び出される。

#### ▼ `__set()` 関数

定義されていないデータや、アクセス権のないデータに値を設定しようとしたとき、代わりにコールされる。

オブジェクトの不変性を実現するために使用される。
...
インスタンス化時に自動的に呼び出される関数。

インスタンス化時に実行したい処理を記述できる。

Setter を持たせずに、`__construct()` 関数のみで値を設定すれば、ValueObject のような、『イミュータブル』なオブジェクトを実現できる。
...
Setter は何度でもコールでき、そのたびにデータの値を上書きできる。
...
Setter を持たせずに、`__construct()` 関数のみを持たせれば、『イミュータブル』なオブジェクトとなる。
...
#### ▼ `static()` 関数 と `self()` 関数の違い

どちらも、new 演算子と組み合わせて、自身のインスタンスを返却する関数であるが、作成の対象になるクラスが異なる。
...
以下の通り、`self()` 関数は定義したクラスをインスタンス化する。

一方で、`static()` 関数はコールされたクラスをインスタンス化する。

自身のインスタンス化処理が継承される場合は、`static()` 関数を使用したほうがよい。
...
以下のような、オブジェクト A を最外層とした関係が存在しているとする。

【オブジェクト A (オブジェクト B を状態に持つ) 】
...
以下のように、返却値のオブジェクトを使用して、より深い層に連続してアクセスしていく場合…
...
自プログラムから自身をコールし、実行できるプログラムのこと。

**＊例＊**

ある関数 `f` の定義のなかに `f` 自身を呼び出している箇所がある。
...
関数がコールされた場所に値を返却した後、その処理が終了する。

#### ▼ `yield`

関数がコールされた場所に値を返却した後、そこで終了せず、`yield` の次の処理が続く。
...
特定の名前と関数を紐付け、名前を渡すことにより、関数をコールするオブジェクトを Dispatcher という。
...
// 文字列から関数をコール。ついでに、引数を渡す。
$dispatcher->dispatch("foo", "test");
```

#### ▼ イベント名と関数の紐付け

名前としてイベント名を定義し、これに関数を紐付ける。

特定のイベント名が渡されたとき、それに対応づけられた関数をコールする。
...
大サイズのデータを集計するは、その処理に時間がかかる。

そこで、そのような関数では、一度コールされて集計した後、データに返却値を保持しておく。

そして、再びコールされたときには、返却値をデータから取り出す。
...
class ResultCacher
{
    ...
    {
        // call_user_funcで関数を実行
        return call_user_func(
        // 登録されている関数名から、関数をDispatch.
...
// 最初の括弧を使用しないことにより、普段よくやっている値渡しの関数を定義しているのと同じになる。
// use()に、親関数 ($optionName) のスコープの$itemを渡す。
...
// 最初の括弧を使用しないことにより、普段よくやっている値渡しの関数を定義しているのと同じになる。
// 親関数 ($optionName) のスコープの$itemを、use()に渡す。
// $paramは、コール時に使用する変数。
$optionName = function ($para) use ($item) {
    $item->optionName() . $para;
};

// コール時、$paramをfunction()に渡す。
echo $optionName("BC");
...
### クロージャー (無名関数) の定義と即コール

定義したその場でコールされる無名関数を『即時関数』と呼ぶ。

無名関数をコールしたいときは、`call_user_func()` 関数を使用する。
...
class Foo
{
    ...
    public function Shiborikomi($callback)
    {
        ...
        foreach ($this->properties as $property) {
            ...
            // 例えば、判定でtrueのもののみを返す関数を渡すと、自データを絞り込むような処理を行える。
...
区分名（`A_TYPE`、`B_TYPE`、`C_TYPE`）を integer 型の定数として扱う。

DB レコード区分名をそのまま保存してもよいが、区分名が変わったときにアプリケーションの実装だけでなく DB レコードを変更するのが大変である。

その一方で、DB は区分名ではなく区分値として管理しておけば、DB レコードを変更せず、アプリケーションだけを変更すればよい。
...
この定数がコールされたファイルの配置ディレクトリパスは、ルートディレクトリ基準で格納されている。

**＊実装例＊**

以下の実装を持つファイルを、`/var/www/foo` ディレクトリ下に配置しておき、`/vendor/autoload.php` ファイルと結合してパスを通す。
...
この定数がコールされたクラス名と関数名が、`{クラス名}::{関数名}` の形式で格納されている。
...
文字列のなかで、変数の中身を取り出すことを『変数展開』と呼ぶ。

※Paiza で検証済み。

#### ▼ シングルクオーテーションによる変数展開

シングルクオーテーションの中身はすべて文字列として認識され、変数は展開されない。
...
波括弧を使用すると、明示的に変数として扱える。

これによって、変数の前後に半角スペースを配置しなくとも、変数は展開される。
...
変数の `$b` には、`$a` の１行目の格納によって 2 が格納される。
...
一度にすべてのデータを入出力するのではなく、少しずつ入出力する処理のこと。

#### ▼ php://stdin

stdin：standard in (標準入力) を意味する。

PHP のプロセスが、標準入力に対して、読み出し処理を送信できるようになる。
...
PHP のプロセスが、標準出力に対して、書き込み処理を送信できるようになる。
...
PHP のプロセスが、標準エラー出力に対して、書き込み処理を送信できるようになる。
...
『`a` から `z` のいずれか』または『A から Z のいずれか』のアルファベットを意味する。
