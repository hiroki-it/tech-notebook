...
インストールされるバージョンをチーム内で固定するため、プロジェクトが正しく動作するバージョンのセットを記載した `composer.lock` ファイルを作成しておく必要がある。
...
プロジェクトで `git clone` コマンドを実行することは、プロジェクト内で `composer install` コマンドを実行することと同じである。
...
一方で、互換性はあるものの、大きな変更がなされている可能性がある。
