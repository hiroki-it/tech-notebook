...
RUN go mod tidy \
  ...
  `# Goでリクエストを受信しないアプリケーションであれば、パッケージのインストールはコンテナ起動後に実行しても良い。` \
  && go mod download -x \
...
ファイル名とパッケージ名は合わせる必要はないが、独自ルールを設けても良い。
...
アプリケーション内で構造体名の頭文字が重複すると、同じレシーバ名の構造体が乱立してしまうため、これを防ぐために二文字を取ると良い。
