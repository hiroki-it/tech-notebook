...
クラスからデータや関数をコールした時、そのクラスにこれらが存在しなければ、継承元まで参照しにいく仕組みを『クラスチェーン』という。
...
再利用したい関数や状態を部品化し、利用したい時にクラスに取り込む。
...
そこで、各クラスでtraitを使用して、マジック関数をコールした時に、開発者向けにエラーが発生させる。
...
: Repositoryのインターフェース (抽象クラス) を、より上位のエンティティ層に配置する。また、Repositoryの実装クラスを、より下位のインフラ層に配置する。
...
: ２と３により、インフラ層とユースケース層の両方が、エンティティ層のインターフェース (抽象クラス) に依存することになる。これは、原則１を満たす。
...
エンティティ層にインターフェースリポジトリを配置する例を考える。
