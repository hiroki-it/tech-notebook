...
汎化の時、子クラスで関数の処理内容を再び実装すると、処理内容は上書きされる。
...
この時、『働く関数』は部署ごとに異なってしまうが、どう実装したら良いのか…
...
: `1` 個の社員オブジェクトの中で、働く関数に部署ごとで変化する引数を設定
...
そこで、各クラスでtraitを使用して、マジック関数をコールしたときに、開発者向けにエラーが発生させる。
...
PHPでは、集約や合成の関係性を作り、委譲先クラスの関数では委譲元のコールして、処理を追加実装することに相当する。
...
そこで、チームでの開発では、記述ルールを設けて、`require_once` 関数で読み込んで良いものを決めておくと良い。
...
Composerのオートロード機能を有効化したうえで、外部ファイルのクラスの `namespace` を `use` で指定するのみで、そのクラスのみを読み込める。
...
そのため、ビジネス上ありえないオブジェクトを作成できなくなり、インジェクションの中で、ソフトウェアの安全性の観点でもっとも優れている。

**＊実装例＊**

依存先のSupplierクラスを、`construct` 関数の引数として、Clientクラスに注入する。
...
依存先のUserNameクラスを、`construct` 関数の引数として、Userクラスに注入する。
...
上記2つ以外の関数の引数として、サプライヤー側のインスタンスを注入する。

サプライヤー側を状態として保持せず、読み込んで関数を使用する。

**＊実装例＊**

依存先のUserNameクラスを、関数の引数として、Userクラスに注入する。
...
そのため、実装リポジトリではなくインターフェースリポジトリを確認することにより、そのリポジトリがどの集約に対してCRUD処理を実行するのかを認識しやすい。
...
LCOM4は、クラスの各関数内で、保持する全てのデータにアクセスしているほど、凝集度が高いと見なす方法である。
