...
Remix の 90%は react-router とのこと。

ラッパーを減らし、ブラウザがデフォルトで提供する関数やオブジェクト、HTML 要素をそのまま使用できるようになっている。
...
SSR のアプリケーションで以下の順に処理を実行し、データの取得からレンダリング（サーバーレンダリング／クライアントレンダリング）までを実施する。
...
レンダリング前に API からデータを取得し、またブラウザ操作に応じてアクションからデータを取得する。

各エンドポイントごとに定義できる。

DB にクエリを送信し、データを取得できる。
...
export const loader = async () => {
  // バックエンドでデータを処理する
  // ローダーの段階でデータを用意しておき、フロントエンドではデータ表示ロジックだけを実装すると、ブラウザの描画の性能が上がる
  const data = {
    posts: [
      {
        slug: "my-first-post",
...
機能内にある URL を表すようなコンポーネント名にする。

例えば、Foo という機能があり、その機能内にレポート表示 URL があれば、FooReport というコンポーネント名にする。

```typescript
// レポートのURLで使用する
export default function FooReport() {
  // 何らかの実装
}
```
...
親コンポーネントから子コンポーネントにわたす値を特別に props という。
...
// 親のremixコンポーネントで、ローダーからデータを取得する
export default function Posts() {
  const {posts} = useLoaderData<typeof loader>();
...
// 子のremixコンポーネントで、親コンポーネントからpropsとしてpostsを受け取る
...
レンダリング後のブラウザ操作に応じて、デザインパターンのコントローラーのようにクエリストリングやリクエストのコンテキストを受信し、DB のデータを変更する。

また、レスポンスのデータを remix コンポーネントに渡す。

component を同じファイルへ実装する以外に、`.server` ディレクトリへ切り分ける方法もある。
...
各 route ファイルが独立した小さな MVC になっている。
...
CSR モードと SSG モードは厳密な区分ではなく、Remix 独自の擬似的なモードである。
...
レイヤードアーキテクチャに沿った依存方向やディレクトリ構成でアプリケーションを設計したいが、本アプリケーションが採用している Remix にはフレームワークとしての設計ルールがあり、これも尊重する必要がある。

レイヤードアーキテクチャに沿った依存方向で設計できるが、Remix は 3 つの要素（ローダー、アクション、コンポーネント）を使用した設計方法を強制する点で、ディレクトリ構成は完全にはレイヤードアーキテクチャに沿うことはできない。

そこで、Remix を尊重しつつ、ディレクトリ構成をレイヤードアーキテクチャに可能な限り近づけることとする。
...
`routes` ディレクトリには、API ルーティングまたはページング処理、状態管理、UI レンダリング、CSS スタイリング、API 入力値バリデーションに関するロジックを置く。

`routes` ディレクトリのアーキテクチャは Remix の仕組み（ローダー、アクション、コンポーネント）の影響を特に受ける。

フロントエンドまたはバックエンドのプレゼンテーション層のロジックがローダー、アクション、コンポーネントに配置される。

```typescript
// 例：app/routes/foo.tsx
...
export default function FooComponent() {
  // 中略
  // フロントエンド領域のプレゼンテーション層に相当
  // 状態管理、UIレンダリング、CSSスタイリングのロジック
  // 中略
}
```
...
- 下書き系：`getDraftXxx` (UI 特有のユースケースで、API では不要)
...
`entry.server` ファイルの `default` エクスポート関数は、Remix の SSR モードのエントリーポイントになる。
...
Remix の内部で実行され、JSON データを作成し、Remix のフロントエンド処理に渡す。
...
Remix v2 を `@remix-run/node` パッケージからインポートする。
...
Remix の仕様ではディレクトリ構造やファイル名がエンドポイントに影響してしまう。
...
バックエンドからデータの取得に時間がかかる場合、defer によるレスポンスストリーミングを使用する。

```ts
import {defer} from "@remix-run/node";

export const loader = async () => {
  const heavyProceccingData = heavyProceccing();

  return defer({
    critical: "data",
    slowPromise: heavyProceccingData,
  });
};
```
...
- 特定の値を二重管理していないか
- コンポーネントにそのまま出力すればいい値を状態としてわざわざ管理していないか
...
`action` 値を省略した場合、フォームの入力データは他に送信されず、その remix コンポーネント内のみで処理される。
...
Link コンポーネントが URL クエリストリングのページ値を書き換えると、Remix のローダーがそれを検知し、新しい URL クエリストリングで再リクエストを送信することで別のページが表示される。
...
SSR では、Web Storage API と通信できず、ブラウザの LocalStorage や SessionStorage を操作できない。
