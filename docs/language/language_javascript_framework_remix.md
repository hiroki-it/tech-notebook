...
| ---- | ------------------- | ---------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | ブラウザ上          | -                | -                                                           | ブラウザからRemixにリクエストを送信する                                                                                                                                                                                           |
...
| 3    | remixコンポーネント | `app/components` | UIロジック、CSSスタイリングロジック、状態・振る舞いロジック | レンダリング処理を実行する。reactコンポーネントとは区別する。また、`app/components`にあるreactコンポーネント（UIレンダリングロジック、状態・振る舞いロジック）を呼び出す。 生成したページをブラウザに返信する。                   |
...
| 6    | ローダー            | `app/routes`     | UIロジック、CSSスタイリングロジック、状態・振る舞いロジック | ブラウザ操作に応じて、アクションからデータを取得する。                                                                                                                                                                            |
| 7    | remixコンポーネント | `app/components` | UIロジック、CSSスタイリングロジック、状態・振る舞いロジック | ２番に同じ                                                                                                                                                                                                                        |
...
Model
# modelsなどが相当
# エンティティ層のモデルとインフラ層の永続化処理が結合している
```
