...
| ---- | ------------------- | ------------ | ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | ローダー            | `app/routes` | UI + 状態・振る舞い | レンダリング前、APIからデータを取得する。                                                                                                                                                           |
| 2    | remixコンポーネント | `app/routes` | UI + 状態・振る舞い | レンダリング処理を実行する。reactコンポーネントとは区別する。また、`app/components`にあるreactコンポーネント（UIロジック）を呼び出す。                                                              |
...
| 5    | remixコンポーネント | `app/routes` | UI + 状態・振る舞い | ２番に同じ                                                                                                                                                                                          |
...
│   ├── root.tsx
│   ├── routes/             # フロントエンド (例：レンダリング処理) とバックエンド (例：APIエンドポイント処理) の関数。関数はローダー、remixコンポーネント、アクションに分類できる。
