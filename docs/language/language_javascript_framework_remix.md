...
機能内にあるURLを表すようなコンポーネント名にする。

例えば、Fooという機能があり、その機能内にレポート表示URLがあれば、FooReportというコンポーネント名にする。
...
親コンポーネントから子コンポーネントにわたす値を特別にpropsという。
...
レイヤードアーキテクチャに沿った依存方向やディレクトリ構成でアプリケーションを設計したいが、本アプリケーションが採用しているRemixにはフレームワークとしての設計ルールがあり、これも尊重する必要がある。

レイヤードアーキテクチャに沿った依存方向で設計できるが、Remixは3つの要素（ローダー、アクション、コンポーネント）を使用した設計方法を強制する点で、ディレクトリ構成は完全にはレイヤードアーキテクチャに沿うことはできない。

そこで、Remixを尊重しつつ、ディレクトリ構成をレイヤードアーキテクチャに可能な限り近づけることとする。
...
`routes` ディレクトリには、APIルーティングまたはページング処理、状態管理、UIレンダリング、CSSスタイリング、API入力値バリデーションに関するロジックを置く。

`routes` ディレクトリのアーキテクチャはRemixの仕組み（ローダー、アクション、コンポーネント）の影響を特に受ける。
...
`routes` ディレクトリには、APIルーティングのためのapiルートとページング処理のためのuiルートがある。
...
バックエンドからデータの取得に時間がかかる場合、deferによるレスポンスストリーミングを使用する。
...
Remixはhookを同じ順番、同じ回数でコールする必要がある。
