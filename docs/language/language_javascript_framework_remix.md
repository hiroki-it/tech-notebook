...
| 2    | ローダー            | `app/routes`     | ルーティング                                          | Controller               | レンダリング前にバックエンドでデータを処理し、`return json()` でフロントエンドにデータを渡す。ローダーの段階でデータを用意しておき、フロントエンドではデータ表示ロジックだけを実装すると、ブラウザの描画のパフォーマンスが上がる。 |
...
レイヤードアーキテクチャに沿った依存方向で設計することはできるが、Remixは3つの要素（ローダー、アクション、コンポーネント）を使用した設計方法を強制する点で、ディレクトリ構成は完全にはレイヤードアーキテクチャに沿うことはできない。
