...
| 3    | remixコンポーネント | `app/components` | UI + 状態と振る舞い | レンダリング処理を実行する。reactコンポーネントとは区別する。また、`app/components`にあるreactコンポーネント（UIロジック + 状態と振る舞いのロジック）を呼び出す。 生成したページをブラウザに返信する。                            |
