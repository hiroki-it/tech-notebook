...
| 3    | remixコンポーネント | `app/components` | UI + 状態／振る舞い | レンダリング処理を実行する。reactコンポーネントとは区別する。また、`app/components`にあるreactコンポーネント（UIレンダリングロジック + 状態／振る舞いのロジック）を呼び出す。 生成したページをブラウザに返信する。                |
...
| 6    | ローダー            | `app/routes`     | UI + 状態・振る舞い | ブラウザ操作に応じて、アクションからデータを取得する。                                                                                                                                                                            |
| 7    | remixコンポーネント | `app/components` | UI + 状態・振る舞い | ２番に同じ                                                                                                                                                                                                                        |
