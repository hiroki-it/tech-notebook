...
gRPC に関するミドルウェアパターン (例：認証、ロギング、メトリクス、分散トレーシングなど) を持つ。

なお、gRPC はリモートプロシージャーコールであるため、ミドルウェアパターンにルーティングは含まれない。
...
特定の条件のときに `true` を返却する `match()` 関数を定義し、これを `runtime.WithIncomingHeaderMatcher()` 関数に渡す。
...
`DialContext()` 関数のラッパーであり、新しいコンテキストで gRPC サーバーとの接続を作成する。
...
既存コンテキストを使用して、gRPC サーバーとの接続を作成する。
...
gRPC サーバーへの通信でサーバー認証を実施するかどうかを設定する。
...
リクエストの宛先で、Carrier からトレースコンテキストを抽出する。
...
クライアント側で、トレースコンテキストを Carrier に注入する。
...
HTTP ヘッダーを Carrier として使用できるようにする。
...
`Header()` 関数を `HeaderCarrier` に変換することで、HTTP ヘッダーを Carrier として使用する。
...
各永続化関数でスパンを作成したり、SQL ステートメントを設定する必要がなくなる。
...
gRPC のメタデータを Carrier として使用できるようにする。
...
執筆時点 (2024/03/31) で ClientInterceptor 系関数は非推奨であり、`NewClientHandler()` 関数が推奨である。
...
執筆時点 (2024/03/31) で ServerInterceptor 系関数は非推奨であり、`NewServerHandler()` 関数が推奨である。
...
gRPC の場合、リモートプロシージャーコールなため、スパン名は関数名とするとよい。
...
HTTP リクエストの場合、スパン名は URL にするとよい。
...
これは、gRPC による HTTPS プロトコルで監視バックエンド (デフォルトでは `https://127.0.0.1:4317`) に送信する。
...
これは、HTTP プロトコルで監視バックエンド (デフォルトでは `https://127.0.0.1:4318/v1/traces`) に送信する。
...
Carrier のインターフェースである。

さまざまな計装ツールの Carrier がこのインターフェースの実装になっている。
...
`ote/propagation` パッケージの `NewCompositeTextMapPropagator` のラッパーであり、Composite Propagator を作成する。

デフォルトでは、W3C Trace Context と Baggage の Composite Propagator になる。

また、`OTEL_PROPAGATORS` 変数 (`tracecontext`、`baggage`、`b3`、`b3multi`、`jaeger`、`xray`、`ottrace`、`none`) で Propagator 名をリスト形式 (`tracecontext,baggage,xray`) で指定していれば、上書きできる。
...
プロセスの終了時に、バッファーに保管されているログをすべてフラッシュする。
