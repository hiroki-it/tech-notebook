...
index.jsの `methods:` 内には、イベントハンドラ関数として `result` 関数を定義する。
...
View の `input` タグでは、一文字でも値が入力された時点で発火し、ViewModelバインディングされたイベントハンドラ関数をコールする。`v-on:change` とは、イベントが発火するタイミングは異なるため、共存できる。
...
`data` オプションは、Vueインスタンス作成後に有効化される。beforeCreate フックでは参照できない。
...
ただし、全ての子コンポーネントでマウントが完了したことを待つために、`nextTick` 関数を使用する必要がある。
