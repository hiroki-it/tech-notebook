...
: View では、`foo.html`、`/foo.twig`、`foo-component.vue` の `template` タグ部分)

`(2)`

: ViewModel では、`index.js`、`foo-component.vue` の `script` タグ部分

`(3)`

: Model では、Vuex (`store.js`)や JavaScript からなるモデル (`foo.js`) を配置する。
...
この仕組みを、Props Down, Events Up という。
...
データが `js` ファイルに渡される状況としては、イベント発火時である。

**＊実装例＊**

例えば、テンプレートの親コンポーネントタグでクリックイベントが発火したとき、親コンポーネントから、イベントに紐付いたイベントハンドラ関数がコールされる。
...
Vue コンストラクタ関数を使用して、インスタンス化することによって、ルート Vue インスタンスが作成される。

インスタンスの変数名 `vm` は VIewModel の意味である。

インスタンス化時、すべてのコンポーネントのデータが初期化される。
...
コンポーネントは、View に相当する `template` タグ、ViewModel に相当する `script` タグと `style` タグを使用して、単一ファイルコンポーネントとする。

**＊実装例＊**

例えば、親コンポーネントの子コンポーネントタグでクリックイベントが発火したとき、子コンポーネントから、イベントに紐付いたイベントハンドラ関数がコールされる。
...
View (`template` タグ部分) のイベントを、ViewModel (`script` タグ部分) のイベントハンドラ関数 (`methods:` 内にある関数) やインライン JS ステートメントにバインディングし、イベントが発火した時点でイベントハンドラ関数をコールする。
...
View のフォーム送信イベントが起きた時点で、ViewModel にバインディングされたイベントハンドラ関数をコールする。
...
index.js の `methods:` 内には、イベントハンドラ関数として `result()` 関数を定義する。
...
: 『検索ボタンを押す』という `submit` イベントの発火によって、`form` タグでイベントに紐付けられているイベントハンドラ関数 (`search()` 関数) がコールされる。
`(2)`

: イベントハンドラ関数内の `emit()` 関数が、親コンポーネントの `search` イベントを発火させる。これに紐付くイベントハンドラ関数 (`result()` 関数) がコールされる。
...
View でクリックイベントが起きた時点で発火し、ViewModel でバインディングされたイベントハンドラ関数をコールする。

#### ▼ `v-on:change="<イベントハンドラ関数>"`

View で `input` タグや `select` タグで、値の入力後にマウスフォーカスがタグから外れた時点で発火し、ViewModel でバインディングされたイベントハンドラ関数をコールする

#### ▼ `v-on:input="<イベントハンドラ関数>"`

View の `input` タグでは、一文字でも値が入力された時点で発火し、ViewModel バインディングされたイベントハンドラ関数をコールする。`v-on:change` とは、イベントが発火するタイミングは異なるため、共存できる。
...
条件分岐するタグであり、`v-show` または `v-if` を使用して、プロパティ名を指定する。親テンプレートから渡す `props` 内のプロパティ名の値が `TRUE` の場合は表示し、`FALSE` のときは非表示にする。もし頻繁に表示と非表示の切り替えを実行するようなら、`v-if` のほうが、描画コストが重たくなるリスクは高くなるため、`v-show` 推奨である。
...
実装方法は、`v-on:input="<イベントハンドラ関数>"` と同じである。例えば、以下の 2 つは同じである。
...
Vue インスタンスの作成から破棄までの間に実行される関数のこと。
...
ブラウザ上のリアル DOM の要素を、Vue.js の処理によって作成される仮想 DOM の要素で置き換えること。
...
Vue インスタンスの作成前に実行される。

**＊検証例＊**

beforeCreate フックの動作を検証する。

`data` オプションは、Vue インスタンス作成後に有効化される。beforeCreate フックでは参照できない。
...
`data` オプションは、Vue インスタンス作成後に設定されるため、created フックでコールでき、処理結果が表示される。
...
Vue インスタンスがマウントされる前に実行される。

**＊検証例＊**

beforeMount フックの動作を検証する。

`data` オプションから `name` 変数に対する展開は、マウントによって実行される。

そのため、beforeMount フックの段階では要素自体が作成されておらず、何も表示されない。
...
Vue インスタンスがマウントされた後に実行される。

要素を操作する処理を実装する。

まず、Vue インスタンスに `el` オプションが設定されているかを識別し、これに応じて処理の流れが分岐する。

SSR の場合には使用できない。

**＊実装例＊**

Vue.js 以外の外部パッケージの読み出し、検索実行時のイベントハンドラ関数、ページング実行時のイベントハンドラ関数など

**＊検証例＊**

beforeMount フックの動作を検証する。

`data` オプションから `name` 変数に対する展開は、element に対するマウント中に実行される。

そのため、`mounted()` 関数が実行され、空文字が上書きされる。
...
ただし、すべての子コンポーネントでマウントが完了したことを待つために、`nextTick()` 関数を使用する必要がある。
...
データが更新されるときの再レンダリング前に実行される。

再レンダリング前に要素を操作する処理を実装する。

**＊実装例＊**

Window オブジェクトや Document オブジェクトの関数による要素の取得など
...
データが更新されるときの再レンダリング後に実行される。

再レンダリング後に DOM を更新する処理を実装する。
...
Vue インスタンスが削除された後に実行する。

インスタンスを削除した後の TearDown 処理を実装する。
