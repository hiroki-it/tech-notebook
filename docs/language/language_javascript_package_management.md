...
NPM に登録されていないパッケージは、『`git+<GitHubリポジトリURL>`』を指定する。
...
すべての npm パッケージのなかで、一意の名前である必要がある。
...
Composer の `composer.lock` ファイルに相当する。
...
パッケージをインストールし、ディレクトリの実行権限不足でインストールが停止する可能性もある。
...
エイリアスのなかで、実行するコマンドのセットを定義する。

補足として、実行するコマンドのなかで、再び `npm run` コマンドも定義できる。
...
`package.json` ファイルに定義があり、実際にはフレームワークの起動コマンド (例：Next.js なら `next build` コマンド) を実行している。
...
`package.json` ファイルに定義があり、実際にはフレームワークの起動コマンド (例：Next.js なら `next dev` コマンド) を実行している。
...
`package.json` ファイルに定義があり、実際にはフレームワークの起動コマンド (例：Next.js なら `next start` コマンド) を実行している。
...
すべてのパッケージを、`package.json` ファイルを無視して最新までアップグレードする。
...
`js` ファイルを読み込む script タグを 1 つにまとめる。

`html` ファイルがブラウザにレンダリングされると、JavaScript のファイルに対するリクエスト数が減るため、Web ページの読み出しが早くなる。
...
モジュールバンドルは、script タグでのパッケージの読み出しをまとめる。
