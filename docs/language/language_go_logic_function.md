...
なお、`init()` 関数は `init.go` ファイルとして切り分けたほうが良い。
...
goのプログラムが起動したときに、各パッケージの `init()` 関数が実行された後、`main()` 関数が実行される。
...
値レシーバによって紐付けられると、その関数は構造体の状態を変えられなくなるため、構造体をイミュータブルにしたい場合は、値レシーバを使用すると良い。
...
ポインタレシーバによって紐付けられると、その関数は構造体の状態を変えられるようになるため、構造体をミュータブルにしたい場合は、ポインタレシーバを使用すると良い。

構造体を初期化する処理を持つコンストラクタ関数のみをポインタレシーバとし、他の関数を全て値レシーバとすると、最低限にミュータブルなプログラムを実装できる。
...
パラメーターと `append()` 関数の返り値の両方のsliceを、アンパック (`...`) する必要がある。
...
4. 最後に、必要な処理を実行しつつ、終了ステータスコード `0` (`main()` 関数の通常の終わり方) や `1` (`Fatal` 系) でプロセスを終えるGraceful Shutdown処理を実行する。
...
時間のかかる処理（例：`for`文、HTTPリクエスト、ループ、ポーリング、DB操作、ファイル操作、計算、暗号化、圧縮解凍など）と相性がいい。
...
そのため、Goroutineの処理結果を使用したい場合は、WaitGroupではなくchannelを使用するほうが良い。

一方で、channelでは `select()` 関数が必要になって、処理が複雑になる点は注意する。
...
エラー処理を含む関数でGoroutineを宣言したいときに使用する。
...
例えばPHPでは、エラーをキャッチし、ソフトウェア開発者がわかる言葉へ変換した例外としてスローする。
...
xerrorsパッケージによって作成される `error` インターフェースには、返却された行数がスタックトレースとして含まれている。
...
Goのコメントの前に不要な改行が挿入されないように、`{{-` とするほうが良い。
