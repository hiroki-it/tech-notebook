...
因果関係がありそうなデータに対して、横軸を原因、縦軸を結果とし、もっとも当てはまりのよい線形モデルを推定する分析手法。

モデルの精度が高ければ因果関係の証明になり、モデルに原因を代入することにより結果を予測できる。

### 単回帰分析

原因と結果が 1 つずつと仮定したときに、もっとも当てはまりのよい線形モデルを推定できる。
...
原因が 2 つ以上で結果が 1 つと仮定したときに、もっとも当てはまりのよい線形モデルを推定できる。
...
因果関係がありそうなデータに対して、横軸を原因、縦軸を結果とし、もっとも当てはまりのよい非線形モデルを推定する分析手法。
...
説明変数が質的変数の場合、もっとも当てはまりのよい非線形モデル (ロジスティック分布) を推定する。
...
データに対して、もっとも当てはまりのよい決定木モデルを推定する分析手法。
...
: `ggplot()` 関数：グラフのキャンバスを準備

`(2)`

: `geom_XXX()` 関数：グラフをプロット
