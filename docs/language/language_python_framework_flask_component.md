...
WSGI アプリケーションの実行に関する関数を持つ。

クラスの引数に、グローバル変数の『`__name__`』あるいはエントリーポイントのパスを直接的に設定する。

環境変数の `FLASK_APP` で指定したエントリーポイントでは、必ず Flask クラスのインスタンスを作成する必要がある。
...
| `host`         | 受信する通信の IP アドレスを設定する。すべての IP アドレスの場合は、`0.0.0.0` とする。                |
...
```yaml
# リクエスト
# レスポンスの Set-Cookie ヘッダーによって、Cookie ヘッダーをつける必要がある
GET /foo/
---
Cookie: session=*****
```
...
Python のコードを配置するディレクトリに `__init__.py` ファイルを配置し、ここで Flask クラスのインスタンスを作成する関数を定義する。
...
名前空間を判定する条件分の外で `create_app()` 関数を実行しないと、uwsgi が app 変数を見つけられない。
...
代わりに、uWSGI や gunicorn を使用して、エントリーポイントの関数を直接的にコールする。
...
ホスト名とデフォルトのプロトコル名 (HTTP) のある URL を作成する。
