...
現在のセッションのユーザーが認証済みであれば、ユーザーの Eloquent モデルを取得する。
...
現在のセッションのユーザーが認証済みであれば、`true` を返却する。
...
DB アクセスが、そのアカウントに設定された認可スコープの範囲内か否かを検証する。
...
: ヘッダーにアクセストークンを含めて、認証ガードの設定されたバックエンド側のルーティングへリクエストを送信する。
