...
SQL の種類が異なっていても、共通のロジックでクエリを送信できる。
...
読み出したレコードをに一度にすべて取得してしまうと、サーバー側のメモリを圧迫してしまう。
...
プリペアードステートメントを使用して SQL を定義する。

#### ▼ `fetch()` 関数

読み出されたすべてのレコードのうち、最初のレコードのすべてのカラムを取得し、1 次元の連想配列で返却する。

#### ▼ `fetchAll()` 関数

読み出されたすべてのレコードの、すべてのカラムを取得し、2 次元の連想配列で返却する。
...
読み出されたすべてのレコードのうち、最初のレコードの一番左のカラムのみを取得し、混合型で返却する。
