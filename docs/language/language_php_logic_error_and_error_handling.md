...
いずれも Throwable インターフェースを実装している。
...
エラーの種類に合わせて、`Exception` クラスを継承した自前例外クラスを実装し、使い分けるとよい。

`__construct()` 関数に、メッセージやエラーコード (例外コード) などを渡せる。
...
ステータスコードはコントローラーのレスポンス処理で割り当てる。

**＊実装例＊**

『Foo 変数が見つからない』というエラーに対応する例外クラスを定義する。
...
if-throw 文を使用して、エラー検出と例外スローを実行する。

ランタイムエラーは検出できないことに注意する。特定の処理のなかに、想定できる例外があり、それを例外クラスとしてするために使用する。

ここでは、すべての例外クラスの親クラスである `Exception` クラスのインスタンスを投げている。
...
ただし、if-throw では、都度例外を検証するがあり、さまざまな可能性を考慮しなければいけなくなる。
...
: のいずれかで返却されるときは、返却の直前に finally 句が実行されるとわかる。
...
例外を `try-catch` でキャッチした後、別の新しい例外をスローしてもよい。
...
想定外のエラーも含めて、すべてのエラーを検出できるように、コントローラーまたはミドルウェアに try-catch 文を実装する。
...
コントローラーの実装をより単純にするべく、より下位のユースケースに try-catch を実装する。
...
`try-catch` に伴うロギングの場合、catch 句のなかでこれを実行する。

そのため、ロギングを実行するレイヤーは try-catch のレイヤーと同じになる。
...
例えば、メッセージアプリケーションの API に対してメッセージ作成のリクエストを送信するとき、例外の種類に合わせて、外部 API との接続失敗によるエラーログを作成することと、自社ソフトウェアなどその他原因によるエラーログを作成する必要がある。
