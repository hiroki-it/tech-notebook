...
ただし、if-throwでは、都度例外を検証するがあり、さまざまな可能性を考慮しなければいけなくなる。
...
try-catch-finallyでは、特定の処理の中で起こる想定できない例外を捉えられる。
...
例外を `try-catch` でキャッチした後、別の新しい例外をスローしても良い。
...
`try-catch` に伴うロギングの場合、catch句の中でこれを実行する。

そのため、ロギングを実行するレイヤーはtry-catchのレイヤーと同じになる。
...
例えば、メッセージアプリケーションのAPIに対してメッセージ作成のリクエストを送信するとき、例外の種類に合わせて、外部APIとの接続失敗によるエラーログを作成することと、自社ソフトウェアなどその他原因によるエラーログを作成する必要がある。
