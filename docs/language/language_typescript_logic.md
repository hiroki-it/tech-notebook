...
TypeScript では、 `return` の型のみを指定すればよい。
...
Typescript では `throw` は、Error オブジェクトだけでなく、どんな型でも返却できる。

ちなみに、よくある型は Error オブジェクト型、object 型、string 型である。
...
TypeScript の `try-catch` で捕捉したエラーの型に応じて処理を実行し分ける場合、`catch` ブロックを複数書くのではなく、`if` 文を使用する必要がある。
...
変数がオブジェクト型の場合に、その変数の階層にある値のみをコピーする。
...
与えられた変数の値が falsy だった場合に、値を設定する。
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      di.metrics.increment("user_fetch_error", {
        status:
          error instanceof Response || error instanceof RetryFailedError
            ? error.status
            : undefined,
      });
...
ループ内で線形探索すると処理量が多くなるため、`Set()` 関数や `Map()` 関数で事前準備しておく。

#### ▼ 悪い例

ループのなかで線形探索の関数 (`some`、`includes`、`find`) を実行してしまっている。
