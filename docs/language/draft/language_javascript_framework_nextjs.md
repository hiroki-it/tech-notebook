...
API Routesモードとほかのモード（CSR、SSR、SSG）はディレクトリを分けて共存させられるため、一つのアプリケーションでフロントエンドアプリケーションとBFFを兼ねることができる。
...
| SSG                                       | サーバーのビルド時         |         ✅         |                  ✅                  |
| CSR                                       | ブラウザ上                 |         ✅         |                  ❌                  |
| API Routesモード (厳密にはモードではない) | サーバーへのリクエストごと |         ✅         | ✅ (`api`ディレクトリ配下で読み込む) |
