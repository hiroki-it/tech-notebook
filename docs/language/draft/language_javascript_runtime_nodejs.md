...
JavaScript をクライアント側で実行する場合、ブラウザ (例：Chrome) を実行環境として使用できる。

一方で、サーバー側で実行する場合に、Node.js を実行環境として使用する必要がある。
...
cgroup v2 を使用し、コンテナのハードウェアリソースの割り当てを決定している。
...
JavaScript や TypeScript は、ブラウザよりも Node.js で実行したほうが高性能である。

つまり、フロントエンドの処理をバックエンドに寄せることで、性能向上の可能性がある。

| 比較項目    | ブラウザ（例：Chrome）                       | Node.js                            |
| ----------- | -------------------------------------------- | ---------------------------------- |
| CPU、メモリ | ユーザーのローカル PC やスマートフォンに依存 | サーバーに依存                     |
