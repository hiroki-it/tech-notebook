...
初期データ (`input`、`payload`など) と期待値 (`expected`、`want`など) をファイル (例：`json`、`yaml`) として用意しておく。
...
func TestFoo_ShouldThrowError(t *testing.T) {
	...
	input_foo_failed_status, _ := ioutil.ReadFile("../testdata/data/foo_succeed_status.json")
