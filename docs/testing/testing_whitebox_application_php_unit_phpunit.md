...
Composerの実行時にメモリ不足にならないよう、メモリを拡張する。
...
データ型を検証できないため、`assertSame` 関数を使用するほうが良い。
...
事前処理として、全てのテスト関数の前にコールされる関数である。
...
事後処理として、全てのテスト関数の後にコールされる関数である。
...
補足として、PHPUnitの場合、モックの関数は `null` を返却する。
