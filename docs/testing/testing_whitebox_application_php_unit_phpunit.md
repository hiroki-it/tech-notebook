...
データ型を検証できないため、`assertSame`関数を使用する方が良い。
...
リクエストにて、チャンネルとメッセージを送信した時、レスポンスとして`TRUE`が返信されるかを検証する。
...
リクエストにて、メッセージのみを送信しようとした時、例外を発生させられるかを検証する。
