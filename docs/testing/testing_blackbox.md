...
| -------------- | ---------------------------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Istio          | UT                                             | -                         | コントロールプレーンのPod (`istio-deployment`というDeployment配下のPod) が正常である。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行することにより、正常か否かを確認する。 |
...
| コンポーネント | テスト種別<br>(UT：単体テスト、IT：結合テスト) | 組み合わせ (ITの場合のみ)                         | テストケース                                                                                                                                                                      |
| :------------- | ---------------------------------------------- | ------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Prometheus     | UT                                             | -                                                 | Podが正常である。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行することにより、正常か否かを確認する。      |
...
| コンポーネント | テスト種別<br>(UT：単体テスト、IT：結合テスト) | 組み合わせ (ITの場合のみ)                      | テストケース                                                                                                                                                                                                                           |
| -------------- | ---------------------------------------------- | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Alertmanager   | UT                                             | -                                              | Podが正常である。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行することにより、正常か否かを確認する。                                                           |
...
| コンポーネント | テスト種別<br>(UT：単体テスト、IT：結合テスト) | 組み合わせ (ITの場合のみ)                 | テストケース                                                                                                                                                                 |
| -------------- | ---------------------------------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Grafana        | UT                                             | -                                         | Podが正常である。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行することにより、正常か否かを確認する。 |
...
| コンポーネント     | テスト種別<br>(UT：単体テスト、IT：結合テスト) | 組み合わせ (ITの場合のみ) | テストケース                                                                                                                                                                                                                                  |
| ------------------ | ---------------------------------------------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PrometheusOperator | UT                                             | -                         | PodMonitorの既存の設定が正常に動作しており、異常を検知するとアラートを発火できる。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行することにより、正常か否かを確認する。 |
...
| コンポーネント | テスト種別<br>(UT：単体テスト、IT：結合テスト) | 組み合わせ (ITの場合のみ)                           | テストケース                                                                                                                                                                                                                                                                                         |
| -------------- | ---------------------------------------------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Fluentd        | UT                                             | -                                                   | Podが正常である。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行することにより、正常か否かを確認する。                                                                                                                         |
|                | IT                                             | Fluentd + <br>ログ監視バックエンド + <br>Kubernetes | Node内の任意のPodのログを正しい形式でログ監視バックエンドに送信できている。例えば、Containerdには、ログメッセージ部分がテキスト形式 (例：metrics-server)　の場合とJSON形式 (例：alertmanager) の場合がある。これらの両方の形式のログを正しく解析し、監視バックエンドに送信できていることを確認する。 |

ハードウェアリソース管理系コンポーネントのテストケース例を示す。

| コンポーネント     | テスト種別<br>(UT：単体テスト、IT：結合テスト) | 組み合わせ (ITの場合のみ)                                | テストケース                                                                                                                                                                                                                                                                                                                                                    |
| ------------------ | ---------------------------------------------- | -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| cluster-autoscaler | UT                                             | -                                                        | Podが正常である。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行することにより、正常か否かを確認する。                                                                                                                                                                                    |
|                    | IT                                             | cluster-autoscaler + <br>metrics-server + <br>Kubernetes | cluster-autoscalerが、新しいPodのスケジューリングの可否を条件として、Nodeをスケーリングすることを確認する。例えば、Node数を手動で増やし、cluster-autoscalerのスケールインによってNode数が元に戻ることを確認する。cluster-autoscalerは10分ごとくらいNodeグループをwatchしており、`10`分後くらいにNode希望数が元に戻る。                                          |
| descheduler        | UT                                             | -                                                        | Podが正常である。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行することにより、正常か否かを確認する。                                                                                                                                                                                    |
|                    | IT                                             | descheduler + <br>Kubernetes                             | CronJobが、deschedulerのJobを実行することを確認する。例えば、`kubectl create job test-job --from=cronjob/descheduler -n descheduler`を実行し、検証用Jobが実行されることを確認する。検証後は、`kubectl delete job test-job -n deschedulerコマンドで削除しておく。                                                                                                |
| metrics-server     | UT                                             | -                                                        | Podが正常である。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行することにより、正常か否かを確認する。                                                                                                                                                                                    |
|                    | IT                                             | metrics-server + <br>Kubernetes                          | `kubectl`コマンドの実行者が、kube-apiserverを介して、metrics-serverからNodeとPodのメトリクスを取得できる。例えば、`kubectl top node`コマンドや`kubectl top pod`コマンドを実行することにより、CPU列とメモリ列に消費量が表示されているかを確認する。                                                                                                              |
|                    | IT                                             | metrics-server + <br>Kubernetes                          | Kubernetesリソース (特に、HorizontalPodAutoscaler、VerticalPodAutoscaler) が、kube-apiserverを介して、metrics-serverからNodeとPodのメトリクスを取得できる。例えば、`kubectl get hpa`コマンドを実行することにより、DeploymentのTarget列の値が`<unknown>`になっていないことを確認する。なお、いずれのDeploymentでTarget列の値が表示されていることを確認しておく。 |
