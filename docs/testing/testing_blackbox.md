...
|                    | IT                                              | cluster-autoscaler +<br>Kubernetes | Nodeの空きリソースに応じて、Nodeをスケーリングできる。例えば、Node数を手動で増やし、cluster-autoscalerのスケールインによってNode数が元に戻ることを確認する。                                                |
| descheduler        | UT                                              | -                                  | Podが正常である。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行し、正常か否かを確認する。                                            |
|                    | IT                                              | descheduler +<br>Kubernetes        | Podを新しくスケジューリングするための空きリソースが既存のNodeに無い場合、Nodeがスケールアウトできる。例えば、Node数を手動で増やし、cluster-autoscalerのスケールインによってNode数が元に戻ることを確認する。 |
| metrics-server     | UT                                              | -                                  | Podが正常である。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行し、正常か否かを確認する。                                            |
|                    | IT                                              | metrics-server +<br>Kubernetes     | `kubectl`コマンドの実行者が、kube-apiserverを介して、metrics-serverからNodeとPodのメトリクスを取得できる。例えば、`kubectl top`コマンドを実行し、確認する。                                                 |
|                    |                                                 | metrics-server +<br>Kubernetes     | Kubernetesリソース (特に、HorizontalPodAutoscaler、VerticalPodAutoscaler) が、kube-apiserverを介して、metrics-serverからNodeとPodのメトリクスを取得できる。例えば、を意図的に発火させる。                   |
...
|                | IT                                              | Fluentd +<br>ログ監視ツール +<br>Kubernetes | Node内の任意のPodのログをログ監視ツールに送信できている。例えば、構造化ログの`cluster`ラベルから、実行環境名 (dev、stg、prd) を確認する。                        |
