...
| -------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| UT                                                 | -                                                   | Podが正常である。『Podが`Running`フェーズかつ`Ready`コンディションであること』をPodの正常とみなす。例えば、`kubectl get`コマンドを実行することにより、正常か否かを確認する。                                                                                                                                                                |
| IT                                                 | Fluentd + <br>ログ監視バックエンド + <br>Kubernetes | Node内の任意のPodのログを正しい形式でログ監視バックエンドに送信できている。例えば、Containerdには、ログメッセージ部分がテキスト形式 (例：metrics-serverなど)　の場合と`json`形式 (例：alertmanager、istio-proxyなど) の場合がある。<br>これらの両方の形式のログを正しく解析し、監視バックエンドがそれを正しく受信できていることを確認する。 |
