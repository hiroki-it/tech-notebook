...
Prometheus のメトリクスから各コンテナに最適な CPU/メモリの設定値を算出できる。
...
krr では、CPU の `.spec.containers[*].resources.limits` キー値の設定はアンチパターンとして扱っている。

そのため、未設定を推奨している。

理由として、CPU を制限してしまうと、コンテナがどんな状況であってもコンテナに CPU を割り当てない。
...
CPU に関して、`99`%の CPU 処理が余裕を持てるように数値を算出する。
...
シンプル戦略に基づいて、最適な CPU/メモリの設定値を算出する。
