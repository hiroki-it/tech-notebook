...
| テストの種類                | 検証内容                                                                     |
| --------------------------- | ---------------------------------------------------------------------------- |
| 正常系 (成功で終わる操作)   | 操作の種類 (登録、参照、更新、削除) 、画面遷移、状態遷移、セキュリティ、など |
...
一定時間内に、ユーザーが一連のリクエスト (例：ログイン、参照、登録、ログアウト) を行った時に、ネットワークの性能指標 (スループット、レスポンスタイム、など) にどのような変化があるかを検証する。
...
リクエスト数に応じてネットワークの性能指標 (スループット、レスポンスタイム、など) の目標値を決める。
...
例えば、`1`秒間に`50`人のユーザーからリクエストがあり、それらのレスポンスタイムは`3`秒以内である必要があるとする。

この場合、スループットの目標値は`50` (件/秒) 、レスポンスタイムの目標値は`3`秒以内となる。
...
様々な実地的 (例：平常時、ピーク時、障害時) な負荷を再現し、これに対する性能が非機能的な品質を満たしているかを検証する。
