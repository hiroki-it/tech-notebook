...
本番環境にて実際のユーザーの手を借り、非機能的な品質を実地的に検証する (例：該当のメトリクスが基準値を満たすか) 。
...
カオスエンジニアリングは、実地的なテストであり (例：該当のメトリクスが基準値を満たすか) 、今まさに実際のユーザーが使用しているソフトウェアに対して実施することになるため、ビジネスサイドの理解が必要になる。
