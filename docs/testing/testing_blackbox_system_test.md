...
該当のエンドポイントにおいて、レスポンスの `95`%がSLOを満たすことを確認する。

| エンドポイント | トランザクション回数／秒間 | SLO                                                               |
| -------------- | -------------------------- | ----------------------------------------------------------------- |
| `/api/v1/foo`  | `10` tps                   | `10` tps の負荷時に、`99` パーセンタイルが `1` 秒以内に完了する   |
| `/api/v1/bar`  | `20` tps                   | `20` tps の負荷時時に、`99` パーセンタイルが `1` 秒以内に完了する |
| `/api/v1/baz`  | `15` tps                   | `15` tps の負荷時時に、`99` パーセンタイルが `1` 秒以内に完了する |
