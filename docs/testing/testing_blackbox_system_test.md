...
一定時間内に、ユーザーが一連のリクエスト (例：ログイン、参照、登録、ログアウト) を行った時、あらかじめ定めた指標にどのような変化があるかを検証する。
...
実地を再現した (許容の限界値を含む) 負荷をシステムに与えた時、どのように動作するかを確認する。
...
性能の限界値に達するほどのリクエスト数が送信された時、障害回避処理 (例：アクセスが込み合っている旨のページを表示) が実行されるかを検証する。
...
許容以上の負荷をシステムに与えた時、どのように動作するかを確認する。
...
長時間の大量リクエストが送信された時、短時間では検出できないどのような問題が存在するかを検証する。
...
障害が発生した時、どの時点まで遡ってデータを回復させるかの目標値のこと。
...
障害が発生した時、障害後どの時間以内にデータを回復させるかの目標値のこと。
