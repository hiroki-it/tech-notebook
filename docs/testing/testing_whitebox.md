...
ソースコードの実行前に、字句解析、構文解析、型解析、を行い、ソースコードの文法上の問題を検証する方法。ソースコードの機械語翻訳の仕組みの違いで、コンパイラ方式言語の場合はアプリケーションの実行前に文法の誤りを検出できるが、インタプリタ方式言語は実行後でしか検出できない。そのため、特にインタプリタ方式言語では実施した方が良い。
...
ソースコードのベストプラクティス違反を検証する。言語によって、ビルトインのコマンドがある場合（例：```go vet```コマンド）や、外部ツールが必要な場合がある。
...
マイクロサービスのコントローラーがコールされてから、DBの操作が完了するまでを、検証する。下流マイクロサービスのコールはモック化またはスタブ化する。
