...
Datadog社のサーバーからリクエストが送信される。サーバー自体はAWSやAzureによって管理されており、用いるサーバーのリージョンを選択できる。リージョンごとに数個ずつサーバーが存在しているため、もし合成監視対象のアプリケーションでIP制限が行われている場合は、これらのサーバーのIPからのリクエストを許可する必要がある。
