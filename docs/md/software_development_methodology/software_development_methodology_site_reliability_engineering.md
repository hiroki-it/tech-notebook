...
| インシデントの原因をヒューマンエラーにすること           | インシデントが起こった時に、ヒューマンエラーを原因と見なし、責任を追及しないようにする。責任の所在を決めて、インシデントを解決した気になってはならない。あくまで、インシデントを解消することだけに専念する。例えば、Backlogなどのタスクツールに用いると、インシデントだけに集中できる。 |
