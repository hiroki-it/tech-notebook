---
title: 【知見を書きなぐるサイト】テレメトリー間の紐付け@Datadog
---
...
トレーシングライブラリを導入したコンテナの環境変数で値を設定する．
...
| 規則                                               | 例                                    |
| -------------------------------------------------- | ------------------------------------- |
| ```<サービス名>-<フレームワーク名/ライブラリ名>``` | ```order-pdo```，```order-phpredis``` |

ただし，フレームワークのインテグレーション名は接尾辞を付けずに，単に『```<サービス名>```』とする．なぜ接尾辞を付けないかというと，分散トレースと他のテレメトリーを紐づける時に，同じサービス名をタグに使用していないといけないためである．
...
| 規則               | 例                                         |
| ------------------ | ------------------------------------------ |
| ```<サービス名>``` | ```order-service```，```payment-service``` |
...
| 規則               | 例                                                           |
| ------------------ | ------------------------------------------------------------ |
| ```<サービス名>``` | ```order```，```order-service```，```payment```，```payment-service``` |
