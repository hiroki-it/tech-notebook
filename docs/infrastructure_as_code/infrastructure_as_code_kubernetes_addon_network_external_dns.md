...
ExternalDNS Controller は、ネットワークからのリクエストに DNS レコードを必要とする Kubernetes リソース (例：Ingress、Service など) の設定値に応じて、DNS プロバイダー (例：Amazon Route 53) に DNS レコードを自動的に作成する。

Ingress Controller (例：aws-load-balancer-controller、glb-controller) と合わせて使用し、パブリックネットワークからのリクエストを ArgoCD のダッシュボード (argocd-server) へルーティング可能にする。
...
すると ExternalDNS アドオンは、Ingress の `.spec.rules[*].host` キーに応じて、Amazon Route 53 に A レコードと TXT レコードを作成する。
...
なお、Ingress のアノテーションに `external-dns.alpha.kubernetes.io/hostname: <ホスト名>` を明示的に指定してもよい。
...
ExternalDNS アドオンは、Deployment (external-dns) 、Service などのマニフェストから構成される。
...
ExternalDNS Controller が DNS プロバイダー (例：Amazon Route 53) にリクエストを送信できるように、ExternalDNS Controller には ClusterRole に基づく認可スコープを持つ。
...
別途、Ingress や Service の作成が必要である。
