...
一時的な認証/認可サービス（例：AWS STS、GCP条件付きロールバインディング）を使用して、ArgoCDに紐づく実行ユーザー（ログインユーザー）を一時的に認証し、また同じく一時的な認可スコープを付与する（例：AWS STS）。一時的な認証/認可としないと、認証情報が漏洩した場合、認証情報の所有者が常にデプロイできるようになってしまう。
