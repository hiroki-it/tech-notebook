...
- `data`ブロックは、`tfstate`ファイルが自身以外 (例：コンソール画面) で作成されたリソースの状態を参照するために使用でき、異なるtfstateファイルから状態を参照することにも使用できる。

一方で以下のデメリットがある。

- 依存先のリソースごとに`data`ブロックを定義する必要がある。
...
ここでいうプロダクトの粒度は、アプリを動かす EKS/ECS/AppRunner/EC2 とそれを取り巻くリソースである。

そもそも、プロダクトごとにVPCを分けるべきとは思っているが、実際にそういう構成に遭遇するので…😭
