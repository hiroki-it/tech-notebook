...
Kubernetesでは、稼働する可能性のあるPod数から、CIDRブロックを算出すると良い。アプリケーションのPodがスケーリングすることや、Istioなどのリソースを導入することも考慮して、尤もらしいIPアドレス数を算出できる。削除されるPodと作成されるPodが別のIPアドレスになるようにするために（IPアドレスの再利用を防ぐために）、Podの最大数の2倍のIPアドレスを持つCIDRブロックを設定すると良い。

参考：https://cloud.google.com/kubernetes-engine/docs/how-to/flexible-pod-cidr

| Node当たりの最大Pod数 | Node当たりのCIDRブロック | IPアドレスの数 |
| ------------------------- | ------------------------ | --------------- |
| ```1```                   | ```/32``` | ```2```         |
| ```8```                   | ```/28```                | ```16```        |
| ```9``` ～ ```16```       | ```/27```                | ```32```        |
