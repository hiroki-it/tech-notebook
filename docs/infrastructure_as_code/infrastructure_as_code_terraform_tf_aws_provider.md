...
### （１）Systems Managerパラメータストア

Terraformに値をハードコーディングしたくない場合は、Systems Managerパラメータストアで値を管理し、これを```data```ブロックで取得する。
...
| Systems Managerパラメータストア          | 全て                                   | セキュリティを含むAWSリソースでは、Terraformのリポジトリで機密な変数を管理するわけにはいかず、また```.tfstate```ファイルに書き込まれてしまうため。                                                                                                                                                                                  |
