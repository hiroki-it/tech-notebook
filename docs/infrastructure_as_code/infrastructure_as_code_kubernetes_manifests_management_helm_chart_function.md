...
テンプレートや `values` ファイルを出力するときに、特定のキーパスにリクエストを送信する。
...
Helm のテンプレート内に、アクションや変数以外の理由で `{}` を出力する場合 (例：Alertmanager のアラートで変数出力を定義する場合) 、これらとして認識されないようにエスケープする必要がある。
