...
IaaS環境でNode数をスケールさせることができる余地がある環境では動的にNode数をスケールさせることで処理量の増加に対して柔軟に対応できるようになる。
...
DaemonSet配下のPodは、各Nodeの最低1つずつスケジューリングすることが望ましく、NodeからDaemonSet配下のPodが退避させられると、障害が起こる可能性がある。
...
また、暗号化キーで暗号化したうえで、Secretストアで管理するようにする。
...
補足として、Guaranteed QoSのPodはスケジューリングの優先度がもっとも高く、Node-pressure Evictionが発生した場合には、他のQoS (Burstable、BestEffort) よりも後に退避する。
...
なお、Dockerfileの `USER` や `GROUP` でも同様にUIDとGIDを設定できるが、マニフェスト側でもDockerfileと同様にrunAsUserとrunAsGroupを設定したほうが可読性が高くなる。
