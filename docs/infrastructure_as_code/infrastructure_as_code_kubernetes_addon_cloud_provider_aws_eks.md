...
AWS CoreDNSがAmazon EKS Cluster内に無い場合、外部のツール (例：SSOのIDプロバイダーなど) を名前解決できなくなるため、必須である。
...
AWS kube-proxyがAmazon EKS Cluster内に無い場合、Pod内のコンテナのライフサイクルを何も管理できなくなるため、必須である。
...
Nodeに関するヘルスチェック (例：Amazon EC2内のkubeletの正常性) を実施する。

なお、Amazon EC2に関するヘルスチェック (例：Amazon EC2の正常性) は、AWS Auto Scalingグループで設定できる。
