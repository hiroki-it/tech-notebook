...
| 観点             | 説明                                                                                                                                                                                                                                           |
|----------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 文法の誤りテスト       | Dockerの```build```コマンド（```docker build```コマンド）を使用して、Dockerfileの文法の誤りを検証する。                                                                                                                                                                   |
| ベストプラクティス違反テスト | 外部のベストプラクティス違反テストツール（例：hadolint）を使用して、Dockerfileのベストプラクティス違反を検証する。                                                                                                                                                                           |
| 脆弱性テスト （イメージスキャン）  | 外部の脆弱性テストツール（例：trivy）を使用して、Dockerfileの脆弱性を検証する。                                                                                                                                                                                              |
| コンテナ構造テスト | 外部の脆弱性テストツール（例：container-structure-test）を使用して、Dockerfileのコンテナの構造を検証する。（例：期待するファイルが存在するか、コンテナ起動時の```ENTRYPOINT```が正しく機能するか、など）<br>ℹ️ 参考：https://qiita.com/tsubasaogawa/items/d41807d368e7b2635e77#container-structure-test-%E3%81%A8%E3%81%AF |
