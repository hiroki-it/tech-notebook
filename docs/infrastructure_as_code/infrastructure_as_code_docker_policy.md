...
コンテナのログインユーザーにルート権限の認可スコープを付与すると、全てのファイルが操作されうる。これを防ぐために、コンテナの実行ユーザーを別途作成し、これに非特権な認可スコープを付与する。
...
# ログインユーザーを作成し、必要最低限の認可スコープを付与する。
...
外部の静的解析ツール（例：hadolint）を使用して、Dockerfileの静的解析を実施する。

#### ▼ 脆弱性テスト

外部の脆弱性テストツール（例：trivy）を使用し、Dockerfileの脆弱性テストを実施する。
