...
そのため、`helm destroy` コマンド時には CRD を削除しない仕様になっている。
...
チャートをインストールするときに、OpenAPI を使用したマニフェストの静的解析を無効化する。

特に、CRD のファイルサイズが大きすぎてインストールできない場合に使用する。
...
Kubernetes に作成されるリソースのマニフェストを取得する。
...
なお、指定した values ファイルとは別に、チャートのルートディレクトリの `values` ファイルも暗黙的に読み込まれていることに注意する。
...
同様にして、指定した values ファイルとは別に、チャートのルートディレクトリの `values` ファイルも暗黙的に読み込まれていることに注意する。
...
作成した Pod が Ready 状態になるまで、`helm install` コマンドの完了を待機する。
...
warning でも終了コード `1` (失敗) で終えるようにする。

また、`values` ファイルの値が Helm テンプレートで使用されていない場合に、これを警告する。
...
チャートは、バージョンによって中身の `yaml` ファイルに差があるため、ここでチャートのバージョンを確認するとよい。
...
チャートを特定のバージョンへアップグレードする前に、リポジトリの情報を更新しておく必要がある。
...
CI 上でマニフェストの静的解析をしたい場合、CI の実行環境へ暗号化キーの使用許可を付与する必要はない。

`helm template` コマンドに暗号化された YAML ファイルをそのまま渡すと、暗号化ツール (例：SOPS) を介さない。
...
`helm template` コマンドは、暗号化された YAML ファイルを単なる YAML ファイルとして認識し、暗号化されたままマニフェストを展開する。
...
新しいリビジョンを作成し、インストール済の Helm リリースをアップグレードする。
...
これにより、`helm upgrade` コマンド時に CRD のインストールをスキップし、非 CRD のみをインストールできる。

なお、CRD のアップグレードは Helm の仕様上必ずスキップする。
...
`helm upgrade` コマンド時に、CRD の作成をスキップし、非 CRD のみをインストールする。
...
作成した Pod が Ready 状態になるまで、`helm upgrade` コマンドの完了を待機する。
