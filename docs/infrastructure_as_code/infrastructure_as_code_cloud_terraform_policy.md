...
Terraformの各コマンドをCI/CD上で実行している場合、tfnotifyなどの通知ツールを使用し、GitHub上に```terraform plan```コマンドの結果が通知されるようにする。

#### ▼ CI/CDがない場合

Terraformの各コマンドを手動で実行している場合に、```terraform plan```コマンドの結果をテキストファイルに出力し、これをプルリクに添付する。

````bash
$ terraform plan -var-file=foo.tfvars -no-color > plan.txt
````
