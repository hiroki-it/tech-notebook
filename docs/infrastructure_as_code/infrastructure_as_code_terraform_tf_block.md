...
可読性の観点から、`resource` ブロック一括で出力するのではなく、`resource` ブロックの特定の `attribute` 値を出力するようにしたほうが良い。
...
あらかじめ `for_each` 引数に格納したmap型の `key` の数だけ、`resource` ブロックを繰り返し実行する。
...
例として、AWS Certificate Managerのサーバー証明書を示す。

AWS Certificate Managerのサーバー証明書は、ALBやCloudFrontに紐付いている。新しい証明書に紐付け直した後、既存のものを削除する必要がある。
