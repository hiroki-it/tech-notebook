...
複数のアプリケーションのために、同様の設定で同様のインフラを作成する場合がある。また、1つのアプリケーションのためにインフラを冗長化する場合もある。いずれも手動で作成する必要があり、労力がかかる。
...
そのため、例えばAWSの場合、運用時に変更頻度が高いインフラ (例：API Gateway (VPCリンクを含む) 、IAMユーザ (紐付けるロールやポリシーを含む) ) はコード化せず、あえて画面上から作成する。
...
ただし、OSとミドルウェアのプロビジョニングが得意ではない。そのため、サーバー系やコンテナ系のツールを併用することがある。
...
異なるDockerfileを基に、コンテナイメージのビルド、コンテナレイヤーの作成、コンテナの作成、コンテナの起動ができる。
...
どのホスト上のdockerデーモンに対して、どのコンテナに関する操作をするのかを選択的に命令できる。
...
アプリコンテナと同じPod内や、Amazon ECSタスク内に、アプリケーションの一部の機能のみを持つコンテナを配置する。
...
アプリコンテナと同じPod内や、Amazon ECSタスク内に、リバースプロキシコンテナ (Envoy、Linkerdなど) を配置する。
