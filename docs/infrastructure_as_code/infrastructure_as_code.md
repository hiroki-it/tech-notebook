...
複数のアプリケーションのために、同様の設定で同様のインフラを作成する場合がある。また、1 つのアプリケーションのためにインフラを冗長化する場合もある。いずれも手動で作成する必要があり、労力がかかる。
...
コードをバージョン管理すれば、Issue と紐付けて、履歴を残せる。
...
そのため、例えば AWS の場合、運用時に変更頻度が高いインフラ (例：API Gateway (VPC リンクを含む) 、IAM ユーザ (紐付けるロールやポリシーを含む) ) はコード化せず、あえて画面上から作成する。

#### ▼ クラウドプロバイダーの機能追加に追従しにくい

クラウドプロバイダーは日々をオプションを追加している。

画面上からの操作であればすぐにオプションを使用できるが、コード化した場合、ツールのバージョンをアップグレードしなければ、これを使用できない。
...
OS とミドルウェアのプロビジョニングが得意であり、クラウドインフラのプロビジョニングでは、クラウドインフラ系のツールと組み合わせることがある。
...
ただし、OS とミドルウェアのプロビジョニングが得意ではない。そのため、サーバー系やコンテナ系のツールを併用することがある。
...
コンテナを最終的な状態に至らせるまでに実行する一連の作業のこと。
...
クラウドインフラを最終的な状態に至らせるまでに実行する一連の作業のこと。
...
異なる Dockerfile を基に、コンテナイメージのビルド、コンテナレイヤーの作成、コンテナの作成、コンテナの起動ができる。
...
どのホスト上の docker デーモンに対して、どのコンテナに関する操作をするのかを選択的に命令できる。
...
アプリコンテナと同じ Pod 内や、Amazon ECS タスク内に、アプリケーション機能の一部のみを持つコンテナを配置する。
...
アプリコンテナと同じ Pod 内や、Amazon ECS タスク内に、リバースプロキシコンテナ (Envoy、Linkerd など) を配置する。
