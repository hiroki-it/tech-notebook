...
ビルドに失敗したイメージからコンテナを構築し、接続する。```rm```オプションを設定し、接続の切断後にコンテナを削除する。Dockerfileで、イメージのプロセスの起動コマンドを```ENTRYPOINT```で設定している場合は、後から上書きできなくなるため、```docker run```コマンドの引数として新しいコマンドを渡せずに、デバッグできないことがある。
