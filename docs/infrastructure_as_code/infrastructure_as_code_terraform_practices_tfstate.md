...
ローカルモジュールは、リポジトリ内で汎用的に再利用するユースケースに限って作成するべきである。

そのため、ローカルモジュールは以下のようにする。
...
ローカル/リモートモジュールのディレクトリ構成について、**`tfstate` ファイルは粒度に関係ないため、これといった目安はない**が、参考として今までに観測したことのある例をあげた。

<br>

### 依存先 AWS リソース別

依存先の多い AWS リソースは、依存先の AWS リソース別にローカルモジュールを分割する。

依存先 AWS リソース別の分割方法は、Terraform の公式リモートモジュールに一番多い構成である。

**＊例＊**

ローカルモジュールの場合

ただ前述の通り、ローカルモジュールの使用するかどうかは考え直したほうがよい。
...
またスクリプトを使用する AWS リソース (例：Lambda) では、そのソースコードをモジュール下で管理する。
...
AWS リソースのプロビジョニング先のリージョン別にローカルモジュールを分割する。

例えば、AWS Certificate Manager は同じリージョンの AWS リソースにしかアタッチできない制約があるため、AWS リソースによっては複数リージョン必要になる。
...
環境ごとに差分が大きい AWS リソースは、実行環境別にローカルモジュールを分割する。
...
モノリスな `tfstate` ファイルとは、例えば特定の AWS アカウント内の AWS リソースをすべて 1 つの `tfstate` ファイルで管理している場合である。

AWS リソース値を参照しない関係であれば、これらは別の `tfstate` ファイルに分割できる。
...
: bar 側では、foo 側の `tfstate` ファイルからリソース値を取得しつつ、
