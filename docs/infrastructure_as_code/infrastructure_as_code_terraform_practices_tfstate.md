...
ローカルモジュールに関して、これをリポジトリ内で汎用的に再利用するユースケースのみ作成するべきである。
...
ローカル/リモートモジュールのディレクトリ構成に関して、**`tfstate` ファイルは粒度に関係ないため、これといった目安はない**が、参考として今までに観測したことのある例をあげた。
...
環境ごとに差分が大きい AWS リソースに関して、実行環境別にローカルモジュールを分割する。
