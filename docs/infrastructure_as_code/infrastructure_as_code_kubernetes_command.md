...
そのため、`-f` オプションで、マニフェストを指定したほうが良い。
...
Podと同じNamespaceに所属する必要があるため、作成時にNamespaceの指定を忘れないようにする。
...
大文字の `-L` オプションとは異なり、特定のラベルの値を持つKubernetesリソースを絞り込める。
...
異なるPodやServiceの場合、`&` (1つだけ) でつなぐと、同時にポートフォワーディングできる。
