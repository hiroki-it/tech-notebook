...
`~/.kube/config` 配下にあり、Cluster の資格情報が定義されている。
...
kube-apiserver に送信するマニフェストを指定する。`-R` オプションでディレクトリ内のファイルを再帰的に指定もできる。
...
ServerSideApply を実行する。

```bash
$ kubectl apply --server-side -f manifests.yaml
```

ServerSideApply ではない通常の Apply は、ClientSideApply という。

ServerSideApply と ClientSideApply の間で、マニフェストの最終的な状態を決める主体が異なる。
...
一方で ServerSideApply では、kube-apiserver 側でマニフェストを変更する。

ClientSideApply では、クライアント側の制約により `metadata.annotations` キーのサイズ制限でエラーになる。
...
`kubectl` コマンドの宛先になっている kube-apiserver を取得する。
...
`kubectl` コマンドの向き先を、指定した Kubernetes の実行環境の kube-apiserver に変更する。

**＊例＊**

宛先を Minikube の kube-apiserver に変更する。

```bash
$ kubectl config use-context minikube
```

宛先を Docker for Desktop の kube-apiserver に変更する。

```bash
$ kubectl config use-context docker-desktop
```

宛先を Amazon EKS Cluster の kube-apiserver に変更する。
...
`kubectl drain` コマンドとは異なり、Pod を退避させることはない。
...
そのため、`-f` オプションで、マニフェストを指定したほうがよい。

同じ識別子 (リソース名) の Kubernetes リソースが存在する場合は重複エラーになってしまう。
...
Pod 数を維持管理する ReplicaSet を作成する。

Pod を終了するためには、Deployment 自体を削除しなければならない。
...
イメージレジストリの資格情報を持つ Secret を作成する。

Pod と同じ Namespace に所属する必要があるため、作成時に Namespace の指定を忘れないようにする。
...
指定した `.env` ファイル以外から Secret を作成する。
...
Pod の場合、オプションのない `kubectl delete` コマンドが Graceful Shutdown となる。
...
簡易的な情報を参照するときは、`kubectl get` コマンドを使用する。
...
`grep` コマンドを使用して、Pod をスケジューリングさせている Node を取得する。
...
Node が他に存在すれば、その Node 上で Pod をスケジューリングさせる。
...
ただし、Pod の設定値は直接的に変更できず、代わりに Deployment や StatefulSet 上での設定値を変更する必要がある。
...
Pod の `.metadata.labels` キーではなく、Pod 名であることに注意する。
...
`grep` コマンドを使用して、`Running` フェーズの Pod のみを取得する。
...
ArgoCD の Application から、`argocd app diff` コマンドで反復的に差分を確認する。
...
指定した Kubernetes リソースを Namespace に関係なく取得する。

```bash
$ kubectl get pod -A
```

`grep` コマンドを使用して、特定の Node のみを取得する。

```bash
$ kubectl get pod -A -o wide | grep -e NAMESPACE -e <Node名>
```

すべての Pod のイメージをアルファベット順で取得する。
...
指定した Kubernetes リソースの情報でユーザー定義のカラムで取得する。

カスタムリソースの情報をいい感じに取得する際、役立つ。

**＊例＊**

Service の指定した情報をユーザー定義のカラムで取得する。
...
Application の指定した情報をユーザー定義のカラムを取得する。
...
事前に定義したユーザー定義のカラムに基づいて、Kubernetes リソースを取得する。
...
指定した Kubernetes リソースの特定の設定を出力する。

**＊例＊**

`L4` ロードバランサーの IP アドレスを取得する。
...
IstioOperator に定義した Istio のバージョンを取得する。
...
Node が複数ある場合、Node に渡って Kubernetes リソースの情報を確認できるところがよい。
...
`grep` コマンドを使用して、特定の Pod のみを取得する。
...
`grep` コマンドを使用して、特定の Service のみを取得する。
...
指定した Kubernetes リソースの設定を取得し、`yaml` 形式で出力する。

**＊例＊**

指定した Secret を `yaml` 形式で取得する。
...
特定の `.metadata.labels` キーと値を持つ Kubernetes リソースを取得する。

大文字の `-L` オプションとは異なり、特定のラベルの値を持つ Kubernetes リソースを絞り込める。
...
複数の `.metadata.labels` キーを AND 条件で指定できる。

```bash
$ kubectl get pod -l <キー>=<値>,<キー>=<値>
```

`.metadata.labels` キーの値を OR 条件で指定できる。
...
`.metadata.labels.topology.kubernetes.io/zone` キーの値が `ap-northeast-1a` である Node を取得する。
...
特定の `.metadata.labels` キーを持つ Kubernetes リソースを取得し、`kubectl get` コマンドの結果に新しい列として追加する。

小文字の `-l` オプションとは異なり、特定のラベルを持つ Kubernetes リソースを一覧で表示する。
...
Amazon EKS にて、Node グループの種類を確認するため、`eks.amazonaws.com/nodegroup` キーを取得する。
...
Node が作成された AWS リージョンを確認するため、`topology.kubernetes.io/zone` キーを取得する。
...
istio のコンテナインジェクションが有効されている Namespace を確認するため、`.metadata.labels.istio.io/rev` キーを取得する。
...
特定の Kubernetes リソースがどのように管理されているかを取得する。

公式の Helm チャートでは、Deployment、Daemonset、StatefulSet がタグを持つことが多い。
...
指定した `.spec.selector` キーを持つ Deployment を取得する。
...
指定した Kubernetes リソースの `.metadata.labels` キーの値を表示する。

**＊例＊**

すべての Kubernetes リソースの `.metadata.labels` キーの値を表示する。
...
すべての Kubernetes リソースのなかから、ArgoCD で管理していないものの `.metadata.labels` キーの値を表示する。
...
指定した Pod の情報 (フェーズ、ステータスなど) を継続的に取得し、情報に変更があれば出力を追記していく。

別ツールで時間のかかる Kubernetes リソースを作成しながら、`--watch` オプションを使用すると、作成状況を確認できる。
...
Namespace 名、Pod 名、コンテナ名を指定し、kube-proxy のログを確認する。
...
Kubernetes リソースを強制的に削除し、別のマニフェストで再作成する。
...
Pod の Volume (例：ConfigMap、Secret、PersistentVolume、persistentVolumeClaim) の設定を変更した後、Pod へ再び読み込ませるために役立つ。
...
CronJob のバッチを一時無効にする。

```bash
kubectl patch cronjobs foo-batch -p '{"spec" : {"suspend" : true }}'
```

#### ▼ pv

PersistentVolume の設定値を変更する。
...
Pod を直接的に指定する場合と、他の Kubernetes リソース (例：Service、Deployment) の情報を使用して、Pod を指定する方法がある。

このとき、通信自体は他の Kubernetes リソースを経由しているわけではないことに注意する (例えば、Kiali を確認すると、Ingress Controller を経由していないことがわかる) 。

開発環境にて、Service を介さずに直接的に Pod へリクエストを送信したい場合や、SQL クライアントを使用して Pod 内の DB コンテナへ TCP/IP 接続したい場合に使用する。
...
同じ Pod や Service で複数のポートが公開されている場合、同時にポートフォワーディングできる。
...
kube-proxy とは異なるリソースであることに注意する。
...
Node に Taint (`app=batch:NoSchedule`) を付与する。
...
コントロールプレーン Node として扱う Taint を付与する。
...
kubectl と Kubernetes のバージョンをそれぞれ取得する。

両方のバージョンに差があっても、1 つ以内のマイナーバージョンであれば許容範囲である。
