...
多くの場合、 C-Zoneのインスタンスが最初に作成されるため、インスタンスのゾーン名と配置されるA/Cゾーンが逆になる。
...
そのため、IPセットを新しく設定し直す場合は、以下の通り```2```個の段階に分けてデプロイする。

ちなみに、IPセットの名前を変更する場合は、更新処理ではなく削除を伴う再作成処理が実行されるため注意する。
