...
削除保護設定のあるAWSリソースに癖がある。削除保護の無効化と```resource```ブロックの削除を同時にデプロイしないようにする。もし同時に行った場合、削除処理を先に実行するが、削除は保護されたままなため、エラーになる。エラーになる。そのため、このAWSリソースを削除する時は、以下の通り```2```個の段階に分けてデプロイする。

（１）削除保護を無効化（```false```）に変更し、デプロイする。
