...
SSLサーバー証明書のDNS検証時に、ドメインを購入したサービスが管理するドメインレジストラに、Route53のNSレコード値を登録する。

#### `(2)`検証のためにメール再送が必要

SSLサーバー証明書のEメール検証時に、ドメインの所有者にメールが送信されないことがある。

送信されなかった場合は、メールの再送を実行する。

#### (＊) SSLサーバー証明書の検証方法を変更する

もしコンソール画面からSSLサーバー証明書の検証方法を変更する場合、検証方法の異なるSSLサーバー証明書を作成してこれに切り替えたうえで、古いSSLサーバー証明書を削除する必要がある。
