...
サーバー証明書のDNS検証時に、ドメインを購入したサービスが管理するドメインレジストラに、Route53のNSレコード値を登録する。

#### `(2)` 検証のためにメール再送が必要

サーバー証明書のEメール検証時に、ドメインの所有者にメールが送信されないこともある。
...
これに合わせて、Terraformでもリリースを2回に分ける。
...
Amazon EC2の設定値を変更しても、AWS AMIでは初回時しかAmazon EC2を検知できない。

新しいAMIを作成する場合、AMI名を変更し、AMIを再作成する。

Amazon EC2の名前に、`<バックアップするAmazon EC2のID>` をつけることでわかりやすくなる。
...
同じAWSアカウントの異なるAmazon ECSサービスを作成する場合がある。

この場合、コンテナ名が重複することになるため、Amazon CloudWatch LogsのロググループはAmazon ECSサービスをルートとして命名する必要がある。
...
Behavior画面で設定するオリジンにルーティングするパスを定義する。
...
Behavior画面で設定するオリジンにルーティングする標準パスを定義する。
...
アプリケーションのデプロイによって、実インフラのECSタスク定義のリビジョンが増加するため、これを追跡可能にする。
...
問題として、AWS ALBやAWS NLBやリスナーの作成が終了する前に、Amazon ECSサービスの作成が始まってしまう。

AWS ALBやAWS NLBの作成 (※リスナーも含む可能性) が完全に完了しない状態では、ターゲットグループはAmazon ECSサービスに紐付けらず、これが完了する前にAmazon ECSサービスがターゲットグループを参照しようとするため、エラーになってしまう。

リスナーの後にAmazon ECSサービスを作成するようにし、『ターゲットグループ ➡️ ALB/NLB ➡️ リスナー ➡️ Amazon ECSサービス』の順で `resource` ブロックを作成可能にする。
...
TerraformでECSタスク定義を更新すると、現在稼働しているECSで稼働しているECSタスクはそのままに、新しいリビジョンのECSタスク定義を作成する。
...
Amazon ECSサービスの削除には『ドレイニング』の時間が発生する。

約 `2` 分 `30` 秒かかるため、気長に待つこと。

#### (＊) ローリングアップデート

`terraform apply` コマンドで、新しいリビジョンのECSタスク定義を作成すると、これを使用してローリングアップデートが自動的に実行されることに注意する。
...
tpl形式のポリシーにlist型の値を渡すとき、JSONを文字列に変換する `jsonencode()` 関数を使用する必要がある。
...
ヘルスチェックプロトコルがHTTPまたはHTTPSリクエストの時のみ、パスを設定できる。
...
事前にインスタンスへAZを表す識別子を入れたとしても、Terraformはインスタンスの配置先AZを選択できない。
...
運用でTerraformによるエンジンバージョンのアップグレードに抵抗感がある場合、コンソール画面からアップグレードする。その後、Terraformで無視すると良い。
...
Amazon Auroraでは、クラスターにインスタンスを1つだけ紐付けると、プライマリーインスタンスとして作成される。
...
Amazon Auroraでは、紐付けられたサブネットグループが複数のAZのサブネットで構成されている場合、各インスタンスを自動的に配置するAZを決める。
...
Terraformを使用してVPCを作成したとき、メインルートテーブルが自動的に作成される。
...
Amazon S3紐付けられる、自身へのアクセスを制御するためにインラインポリシーのこと。
...
以下の理由で、個人的には、一部のAWSリソースではTerraformを使用しないほうが良い。

代わりに、コンソール画面やAWS CLIでAWSリソースを構築する。

- ビジネスロジックを持つAWSリソースでは、継続的な改善のサイクルが早い (ロジックの変更頻度が高い) ため、リリースまでに時間がかかるTerraformで管理すると、このサイクルを阻害してしまう：
  - API Gateway、IAMユーザー/グループ、IAMユーザー/グループに関連するロール/ポリシーなど

- セキュリティを含むAWSリソースでは、Terraformのリポジトリで機密な変数やファイルを管理できない。`tfstate` ファイルに書き込まれないようにもする：
  - EC2の秘密鍵、パラメーターストアなど

- Terraformによる初期作成時に必要であり、それがないとそもそも `terraform apply` コマンドできない：
  - Terraform用IAMユーザー、tfstateを管理するAmazon S3バケットなど
  - これに関しては、CloudFormationで作成しても良い。

- Terraformの誤操作で削除してはいけないAWSリソースでは、Terraformで管理しないことにより、削除を防げる：
  - tfstateを管理するAmazon S3バケットなど
...
| ----------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| AWS Certificate Manager             | 全て                                 | `terraform apply` コマンド中に承認作業が発生し、プロビジョニングが止まってしまうため。ただし、承認を自動で実行できるオプションを使用する場合には、Terraformで管理しても良い。                                                                                                                                                                                                                                |
| Amazon API Gateway、紐付くVPCリンク | 全て                                 | ビジネスロジックを持ち、ロジックの変更頻度が高い。バックエンドチームがスムーズにAPIを作成できるようになる。                                                                                                                                                                                                                                                                                                  |
| AWS Chatbot                         | 全て                                 | AWSがAPIを公開していないため、Terraformで作成できない。                                                                                                                                                                                                                                                                                                                                                      |
| AWS DynamoDB                        | 全て                                 | `state.lock` ファイルをテーブルとして管理する。                                                                                                                                                                                                                                                                                                                                                              |
| Amazon EC2                          | 秘密鍵                               | Terraformで作成するときにGitHubで秘密鍵を管理する必要があるため、セキュリティ上の理由で却下する。                                                                                                                                                                                                                                                                                                            |
| AWS ENI                             | 全て                                 | 特定のAWSリソース (ALB、セキュリティグループなど) の作成に伴って、自動的に作成されるため、Terraformで管理できない。                                                                                                                                                                                                                                                                                          |
...
| AWS IAMユーザー                     | 全て                                 | ビジネスロジックを持ち、ロジックの変更頻度が高い。                                                                                                                                                                                                                                                                                                                                                           |
| AWS IAMユーザーグループ             | 全て                                 | ビジネスロジックを持ち、ロジックの変更頻度が高い。                                                                                                                                                                                                                                                                                                                                                           |
...
| Amazon SES                          | 全て                                 | `terraform apply` コマンド中に承認作業が発生し、プロビジョニングが止まってしまうため。ただし、承認を自動で実行できる場合には、Terraformで管理しても良い。                                                                                                                                                                                                                                                    |
...
以下のコマンドを実行し、`tfstate` ファイル用のAmazon S3バケットを作成する。
...
`tfstate` ファイルを管理するAmazon S3バケットをTerraformで管理しないプラクティスが一般的ではあるが、実はTerraformで管理する裏技がある。

それは、`tfstate` ファイルのAmazon S3バケットのみを、別のローカルバックエンドの `tfsftate` ファイルで管理する方法である。

このローカルバックエンドの `tfstate` ファイルが管理するAmazon S3バケットをリモートバックエンドとして、メインの `tfstate` ファイルを配置する。

もちろん、ローカルバックエンドの `tfstate` ファイルがAmazon S3バケットに配置されたメインの `tfstate` ファイルを検知しないように、無視する必要がある。
...
そのため、このAWSリソースを削除するときは、以下の通り `2` 個の段階に分けてデプロイする。
