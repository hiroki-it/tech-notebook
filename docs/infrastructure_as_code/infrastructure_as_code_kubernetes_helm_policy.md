...
追加/変更した機能に関するドキュメントを作成する。ドキュメントの内容についてもレビューする。

#### ▼ 独自チャートのREADME

外部のドキュメント自動生成ツール（例：helm-docs）を使用して、独自のチャートを使用している場合、```values```ファイルに基づいて、```module```ブロックのREADMEを作成する。

> ℹ️ 参考：https://github.com/norwoodj/helm-docs
...
| 脆弱性テスト          | 外部の脆弱性テストツール（例：checkov）を使用して、チャートの脆弱性を検証する。                        |      |
