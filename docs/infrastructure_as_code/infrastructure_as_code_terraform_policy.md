...
外部のツール（例：Renovate）を使用し、プロバイダーやTerraformの新バージョンを自動的に検出する。
...
独自```module```ブロックを使用している場合、```variable```ブロックや```output```ブロックに基づいて、```module```ブロックの仕様書を作成しておく。外部のドキュメント生成ツール（例：Terraform-doc）を使用しても良い。
