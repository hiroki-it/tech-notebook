...
| 観点          | 説明                                                         | 補足 |
|-------------| ------------------------------------------------------------ | ---- |
| 文法の誤り       | Terraformの静的解析コマンド（```terraform validate```コマンド）を使用して、機能追加/変更を含むブロックの静的解析を実施する。外部の静的解析ツール（例：tflint）を使用しても良い。 |      |
| ベストプラクティス違反 |                                                              |      |
| 脆弱性         | Terraformの脆弱性テストを実施する。外部の脆弱性テストツール（例：tfsec）を使用しても良い。 |      |
...
通知ツール（例：tfnotify、tfcmt）を使用し、GitHub上に```terraform apply```コマンドの結果が通知されるようにする。これを確認し、差分が正しいかをレビューする。
