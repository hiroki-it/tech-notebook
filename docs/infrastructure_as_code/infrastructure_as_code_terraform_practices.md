...
そのために、現在の Terraform のバージョンで `terraform apply` コマンドを実行することにより、差分がないようにしておく。
...
ステージング環境に対して `terraform apply` コマンドを実行することにより、既存機能/追加/変更を含むすべてのブロックを組み合わせたシステムテストを実施する。
