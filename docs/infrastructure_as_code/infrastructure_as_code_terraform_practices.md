...
リソースをセットアップするうえで異なる種類のリソースが必要になる場合に、それらの `resource` ブロックを1つにまとめて管理する。
...
注意点として、`templatefile` 関数でこれを読み込むとき、`bash` ファイルではなく、tplファイルとして定義しておく必要あるため、注意する。
...
また、`# Output values` コメントの部分では、`output` ブロックをモジュールに渡すときにクラウドプロバイダーのリソースのアルファベット順で並べる。
...
Terraformまたはプロバイダーのアップグレードを別々にリリースするようにすれば、リリース時にインシデントが発生したときに、どちらが原因なのかを明確化できる。
