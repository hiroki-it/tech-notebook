...
Regionalタイプは、同じリージョンの異なるクラウドプロバイダーのリソース間で共有できるため、共通セットとしてディレクトリ分割する。
...
リソースをセットアップするうえで異なる種類のリソースが必要になる場合、それらの `resource` ブロックを1つにまとめて管理する。
...
ポリシーのために `json` 形式を定義する場合、Terraformのコードにハードコーディングせず、切り分けるようにする。

また、『カスタマー管理ポリシー』『インラインポリシー』『信頼ポリシー』も区別し、ディレクトリを分割している。

注意点として、`templatefile` 関数でこれを読み込むとき、`bash` ファイルではなく、tplファイルとして定義しておく必要があるため、注意する。
...
repositoryが2回繰り返されることになるため、`1` 個省略している。
...
Terraformを開発するうえでバージョン管理するべきではないファイルを `.gitignore` ファイルに記載する。
...
この場合、できるだけリリースは分けたほうが良い。

Terraformまたはプロバイダーのアップグレードを別々にリリースすれば、リリース時にインシデントが発生したときに、どちらが原因なのかを明確化できる。
...
| 観点                         | 説明                                                                                                                                                                                    | 補足 |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| Terraformの文法違反テスト    | Terraformの静的解析コマンド (`terraform validate` コマンド) を使用する。機能追加/変更を含むブロックの文法違反を検証する。代わりに、外部の静的解析ツール (例：tflint) を使用しても良い。 |      |
| ベストプラクティス違反テスト |                                                                                                                                                                                         |      |
...
ただコードを眺めているより、レビュー対象がコンソール画面のどこに相当するのかも並行して確認したほうが、Terraformを理解しやすい。
...
経験則で、create処理やdestroy処理よりもupdate処理のほうがエラーは少ない。そのため、ロールバックにもたつきにくい。
...
もしリリース時に問題が発生した場合、インフラのバージョンをロールバックする必要がある。経験則として、create処理やdestroy処理よりもupdate処理のほうがエラーは少ない。そのため、ロールバックにもたつきにくい。

そのため、再実行時にどのくらいのcreate処理やdestroy処理が実行されるかを考慮し、過去のリリースタグを `terraform apply` コマンドでデプロイするか否かを判断する。
