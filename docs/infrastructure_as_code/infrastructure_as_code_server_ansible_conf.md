...
管理対象ノードを設定する．```ini```形式または```yml```形式で定義する．実行環境（本番/ステージング）別にファイルを切り分けると良い．また，サーバーを冗長化している場合は，これも別々に定義しておく．プロビジョニングの実行対象はロードバランサーから一時的に切り離すようにすることで，プロビジョニングに伴ってインシデントが起こっても，ユーザーへの影響を防げる．
