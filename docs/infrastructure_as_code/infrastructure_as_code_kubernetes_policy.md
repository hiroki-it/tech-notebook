...
| GitHubリポジトリ + キーバリュー型ストレージ      | ```base64```方式エンコード値を暗号化キー（例：AWS KMS、GCP KMS、など）で暗号化した上で、リポジトリ内のキーバリュー型ストレージ（例：sops、Hashicorp Vault）で管理する。kube-apiserverへの送信前に```base64```方式エンコード値に復号化する。                                  |
