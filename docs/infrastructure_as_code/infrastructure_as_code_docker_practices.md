...
悪意のあるユーザーがコンテナに接続した場合、root権限を与えてしまうと、悪意のある操作をされてしまう。
...
その代わり、ボリュームマウントやCOPYでファイルをコンテナイメージへ詰め込むことになる。
...
なお、各コンテナイメージのアーキテクチャに割り当てられたダイジェスト値を指定できるが、DockerがホストのCPUアーキテクチャに基づいてよしなに選んでくれるので、ダイジェスト値は指定しない。
