...
そのため、一部のKubernetesリソースを別のチャートでリリースし直したい場合、```meta.helm.sh```キーを手動で書き換える必要がある。

```bash
$ kubectl annotate --overwrite crd $crd meta.helm.sh/release-namespace="<新しいNamespace>"
$ kubectl annotate --overwrite crd $crd meta.helm.sh/release-name="<新しいリリース名>"
```
