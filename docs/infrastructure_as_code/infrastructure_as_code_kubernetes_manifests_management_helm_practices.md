...
この方法であると、リポジトリが増えすぎてしまうため、少なくとも同じ種類のチャートは同じリポジトリで管理するほうが良い。
...
Helmチャートは、特定の種類のマイクロサービスを動かすための「K8sマニフェストのセット（例：Deployment、Service、Istioリソースなど）」を閉じ込めるように設計する。

つまり、Helmチャートはマイクロサービス単位で作成し、マイクロサービスと同名にする。
...
可能な限り、Kubernetesリソースごとのテンプレートを再利用するようにする。

**＊実装例＊**

このConfigMapのマニフェストが `1` 個あれば、`values` ファイルによらずConfigMapを作成できる。
...
新バージョンのチャートを使用して、あらためてHelmリリースすることにより、チャートをアップグレードする。
...
: Helmリリースのリビジョンが新しくなっていることを確認する。
...
: 補足として、もしCRDをインストールする前にカスタムリソースをインストールしようとすると、カスタムリソースを定義できずエラーになってしまう。
...
この時、存在しないAPIのマニフェストが含まれると、差分パッチを作れずエラーになる。
...
Helmのユニットテストコマンド (`helm test` コマンド) を使用して、機能追加/変更を含むチャートが単体でまさしく動作するか否かを検証する。
...
実際のCluster上でマニフェストを検証してもよいし、本番のCluster (例：Amazon EKS、Google Cloud GKE、Azure AKE、Kubeadmなど) に相当する実行環境 (例：Kind、K3SやK3D、Minikubeなど) をCI上に作成してもよい。
