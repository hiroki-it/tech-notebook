...
Helm のユニットテストコマンド (`helm test` コマンド) を使用して、機能追加/変更を含むチャートが単体でまさしく動作するか否かを検証する。
...
テスト環境に対して `helm upgrade` コマンドを実行することにより、既存機能/追加/変更を含むすべてのチャートを組み合わせたシステムテストを実施する。
