...
| IngressClassの機能による | ImplementationSpecific | IngressClass (例：Nginx) の設定に応じて、独自タイプ、Prefix、Exact、を自動的に切り替える。そのためIngressClass間で互換性が低く、Ingressのルーティングの動作がIngressClassの種類に変わるため、想定外のルーティングになる可能性がある。 |
