...
拡張apiserverは、kube-apiserverからリクエストを直接的に受信するのではなく、専用のServiceを介してリクエストを受信する。

この時、どのServiceからリクエストを受信するかを設定する。
...
```spec.readinessProbe```では、コンテナが準備済みチェックを待ち受けられるように設定する。
