...
#### ▼ Kuberneresリソースのwatch

Kubernetesのカスタムリソースから定義される。

watch対象のKubernetesリソースやカスタムリソースを設定する。
...
Application自体もカスタムリソースなため、ApplicationがApplication自身のソースの変更をwatchし、Syncできる。
...
| 操作名       | 説明                                                                                                                                                                                                     |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Sync         | watch対象リポジトリとのマニフェストの差分を確認し、差分があれば`kubectl apply`コマンドを実行する。                                                                                                       |
| Refresh      | watch対象リポジトリとのマニフェストの差分を確認する。差分を確認するのみで、applyは実行しない。                                                                                                           |
| Hard Refresh | redis-serverに保管されているキャッシュを削除する。また、watch対象リポジトリとのマニフェストの差分を確認する。差分を確認するのみで、applyは実行しない。                                                   |
...
リポジトリ (マニフェストリポジトリ、チャートリポジトリ、OCIリポジトリ) の変更をwatchし、これらからプルしたマニフェストで`kubectl apply`コマンドを実行。
...
watch対象のマニフェストリポジトリのディレクトリ構造に関して設定する。

また、リポジトリにチャートを配置しているがチャートリポジトリとして扱っていない場合、マニフェストリポジトリ内のローカルのチャートとして、watchすることもできる。
...
watch対象のマニフェストリポジトリのディレクトリを設定する。
...
マニフェストリポジトリ内のローカルのチャートもwatchできる。
...
watch対象のマニフェストリポジトリのブランチやバージョンタグを設定する。

各実行環境に、実行環境に対応したブランチを指定するマニフェストを定義しておくと良い。

これにより、各実行環境内のApplicationは特定のブランチのみをwatchするようになる。
...
watch対象のチャートレジストリ内のリポジトリにあるチャート名を設定する。
...
watch対象のリポジトリにある`values`ファイルを使用する。
...
watch対象のチャートレジストリ内のリポジトリのURLを設定する。
...
watch対象のチャートレジストリ内のリポジトリにあるチャートのバージョンタグを設定する。
...
watch対象のOCIレジストリ内のリポジトリのURLを設定する。
...
watch対象のOCIレジストリ内のリポジトリのバージョンタグを設定する。
