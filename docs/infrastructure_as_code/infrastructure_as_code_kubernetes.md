...
クライアントにマスターNodeを公開する。クライアントがリクエストを送信すると、マスターNode上のkube-apiserverがコールされ、コマンドに沿ってKubernetesリソースが操作される。存在しないリソース定義をリクエストされると、kube-apiserverはリソース定義を見つけられず、以下のエラーレスポンスを返信する。
