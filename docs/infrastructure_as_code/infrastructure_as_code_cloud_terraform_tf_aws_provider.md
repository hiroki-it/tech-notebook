...
誤って削除しないように，またコードに機密情報をハードコーディングしないように，キーペアはコンソール画面で作成した後，```key_name```でキー名を指定するようにする．
...
運用でTerraformでエンジンバージョンをアップグレードすることに抵抗感がある場合，コンソール画面からアップグレードをTerraformで無視するようにすると良い．ただし，画面からの変更後にTerraformのコードも変更しておく必要がある．Terraformを書き換えなくとも問題は起こらないが，Terraformのコードと実インフラが乖離してしまう．
