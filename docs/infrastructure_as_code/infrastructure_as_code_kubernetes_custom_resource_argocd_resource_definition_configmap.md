...
管理チーム (`app`、`infra`) 単位でAppProjectを作成した上で、AppProject配下のみ認可スコープを持つロールを定義する。
...
実行環境 (`dev`、`prd`) 別にAppProjectを作成した上で、AppProject配下のみ認可スコープを持つロールを定義する。
...
実行環境 (`dev`、`tes`、`prd`) 別にAppProjectを作成した上で、AppProject配下のみ認可スコープを持つロールを定義する。
