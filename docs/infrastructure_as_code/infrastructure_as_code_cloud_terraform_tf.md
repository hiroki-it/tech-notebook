...
リソースを新しく構築した後に削除するように、変更できる。通常時、Terraformの処理順序として、リソースの削除後に構築が行われる。しかし、他のAWSリソースと依存関係が存在する場合、先に削除が行われることによって、他のAWSリソースに影響が出てしまう。これに対処するために、先に新しいリソースを構築し、紐付けし直してから、削除する必要がある。
