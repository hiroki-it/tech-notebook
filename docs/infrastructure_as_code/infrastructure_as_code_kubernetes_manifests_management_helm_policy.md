...
最も基本的な構成。実行環境別に```values```ファイルを作成する。また、実行環境の差は設定値だけとし、```templates```ディレクトリ配下にマニフェストを配置する。
...
実行環境別に```values```ファイルを作成する。また、実行環境ごとにマニフェストの構造に差分があるとし、```templates```ディレクトリ配下に実行環境別のディレクトリを配置する。
