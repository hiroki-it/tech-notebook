...
初期化（```terraform.lock.hcl```ファイルの作成、子モジュールやプロバイダーのインストール、バックエンドの切り替えなど）を実行し、```.terraform```ディレクトリ内に格納する。```state```ファイルを書き換えることはしないため、基本的には安全である。もしプロバイダーをアップグレードした場合は、新しいバージョンのインストールするために、本コマンドを実行すつ必要がある。
