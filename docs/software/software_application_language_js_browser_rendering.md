...
1つのWebページの```.html```ファイル内で、scriptタグが複数に分散していても、Scriptingプロセスでは、1つにまとめて実行される。そのため、より上部のscriptタグの処理は、より下部のscriptに引き継がれる。
...
2. 実行時には以下の様に、まとめて実行される。ここでは、```.html```ファイルで定義した関数の外にある変数は、グローバル変数になっている。1つのページを構成する```.html```ファイルを別ファイルとして分割していても、同じである。
