...
オブジェクト図のほうが、設計の前段階の参考資料として適している。

ユースケース名は、『ソフトウェアは、ユーザーが〇を△する機能を提供する。』と考え、『機能名』をオブジェクト図に反映させる。
...
| パッケージ：<br>![usecase-diagram_package](https://raw.githubusercontent.com/hiroki-it/tech-notebook-images/master/images/usecase-diagram_package.png)             | アクターによって提供する機能に違いがある場合、その境界線を表す。                                                                                                                                                                           |                               |
...
ステートマシンをオブジェクト、各ステートをステートマシンの依存先オブジェクト、アクションをオブジェクトの関数、として実装する。
...
特定の状態を保持する具体的なオブジェクト間の関係性を基に、ソフトウェアの『静的構造』を表す。
