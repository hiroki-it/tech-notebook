...
標準出力に出力された文字列のうち、合致するものだけを取得する。文字列の表示に関する様々なユーティリティ（例：```ls```、```cat```、```find```、など）と組み合わせられる。
...
パーティション名は、```fdisk```コマンドまたは```df```コマンドで確認できる。パーティションのデバイスファイル名を確認するだけであれば、マウントされているパーティションしか表示しない```df```コマンドよりも、```fdisk```コマンドの方がよいかもしれない。
...
パーティションを指定し、パーティションを拡張する。パーティションの番号は『```1```』である。
