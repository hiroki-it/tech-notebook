...
NginxはOAuth2 Proxyにトークンバリデーションリクエストを送信し、OAuth2 ProxyはIDプロバイダーのバリデーション結果をNginxに返信する。

例えばNginxは、レスポンスのステータスコードが`200`であれば認証成功、`401`または`403`であれば認証失敗とし、アプリケーションへのリクエストを許可/拒否する。
...
http {
    ...
    location / {
        ...
        # トークンバリデーションリクエストの宛先とするパスを設定する
        # ここでは、トークンバリデーションリクエストをOAuth2 Proxyに送信する
