...
PDOでは書き込み処理に `exec()` 関数、読み出し処理に `query()` 関数を使用する。
...
一部のRDBMS (例：PostgreSQL) は、トランザクション処理を識別するためにトランザクションIDを付与する。
...
障害によって、トランザクション内の一連のSQLステートメントが1つでも実行されなかった場合、ストレージ上のジャーナルファイルの更新前ログを使用して、トランザクションの開始前の状態に戻す。
...
アプリケーションやRDBMSでは、トランザクションのロック待機時間を設定しておく (例：prismaの `maxWait` 値、MySQLの `innodb_lock_wait_timeout` 値) 。そして、これを超過した場合はエラーになるよう設定する。
...
レコードの `UPDATE` 処理を実行するときに、他者によってUPDATE/`READ` 処理の両方を実行させない場合に使用する。
...
`UPDATE` 処理のためにユーザAがDBのレコードを読み込んだときに、バージョン値を一時的に保持しておく。

ユーザーAが続けて更新する直前に、DBからバージョンの値をあらためて取得する。

保持しておいたバージョン値とDBの値を比較し、DBの値のほうがより新バージョンだった場合、`UPDATE` 処理を失敗させることにより、複数の更新処理の競合を防ぐ。
...
ユーザAがDBのレコードを読み込んだ時点でロックを起動し、ユーザBはレコードの取得すら不可能にする。

ユーザAが更新を終えてロックが解除され、そこで初めてユーザBはレコードを読み込めるようになる。
