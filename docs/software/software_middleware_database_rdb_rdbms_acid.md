...
事前に作成したベースバックアップ（例：SQLによって異なり、MySQLの場合は```mysqldump```コマンドの出力）と差分バックアップ（例：SQLによって異なり、MySQLの場合はバイナリーログ）を使用し、DBをその時点の状態に戻す。トランザクションの前後の状態に戻すロールバック/ロールフォワードとは異なり、DBを任意の時点の状態に戻せる。なお、バックアップに含まれない期間の状態には戻せない。
