...
```sql
RENAME USER "{ 旧いユーザー名 }" TO "{ 新しいユーザー名 }";
```
...
| eq_ref | ・クラスタインデックスを使用している。                       | ```1```個のカラム               | ```1```個のカラムしか```fetch```しないため、```JOIN```句を使用したアクセスの中で、実行時間が最も短い。 |
| const  | ・クラスタインデックスを使用している。<br>・```JOIN```句を使用していない。 | ```1```個のカラム               | ```1```個のカラムしか```fetch```しないため、実行時間が最も短い。  |
