...
アプリの中でも非機能に近いロジックは、マイクロサービスに依らずに、同じような実装になりがちです。
...
フロントエンドアプリのCI/CDパイプラインは、以下の要素からなります。
...
そのため、ArgoCDによるフロントエンドアプリのデプロイ後に、静的ファイルのキャッシュを削除すべきです。
