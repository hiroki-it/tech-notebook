...
コミットの結果をメモリ上に書き込むと同時に、ディスク上のジャーナルファイルには更新前ログを書き込む。データファイルに書き込むよりも書き込み量が少なく済むため、短時間で終わる。また、コミットの途中で障害が起こった場合に、メモリ上のデータが削除されてしまうため、これの

参考：https://gihyo.jp/dev/serial/01/db-academy/000202
