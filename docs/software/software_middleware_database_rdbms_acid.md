...
コミットの結果をメモリ上に書き込むと同時に、ディスク上のジャーナルファイルには更新前ログを書き込む。データファイルに書き込むよりも書き込み量が少なく済むため、短時間で終わる。また、コミットの途中で障害が発生した場合、メモリ上のデータが削除されてしまう、ジャーナルファイルの更新前ログをバックアップとして、データを復元できる。
