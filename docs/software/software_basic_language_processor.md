...
コードをバイナリ形式のコードに翻訳した後、CPU 上で命令が実行される。
...
字句解析のアルゴリズムは、Lex や Flex で実装されている。
...
構文解析のアルゴリズムは、Bison や Yacc で実装されている。
...
コードをバイナリ形式のコードに１行ずつ変換し、その都度、CPU 上で命令が実行される。
...
JVM 言語 (例：Java、Scala、Kotlin など) に当てはまる。
