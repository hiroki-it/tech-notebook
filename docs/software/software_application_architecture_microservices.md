...
ビジネスがスケーリングするとき、マイクロサービスを新規実装したり削除したりすれば良いため、ドメイン層の激しい変化に強い。
...
| historical data service          | 業務マイクロサービス | 境界づけられたコンテキスト (過去データ／ETA分析)          |
| ...                              | ...                  | ...                                                       |
...
マイクロサービスアーキテクチャのフレームワークとして、Dapr、Spring Cloud、Axon、Eventuate、MicroProfile LRAなどがある。
...
マイクロサービスアーキテクチャフレームワークは、非機能ロジックの中でもより機能ロジックに近いSDKやドライバーのようなロジック（ストレージ接続ロジック、メッセージブローカー接続ロジック）を提供する。

一方で、サービスメッシュはSDKやドライバーには対応していない。
