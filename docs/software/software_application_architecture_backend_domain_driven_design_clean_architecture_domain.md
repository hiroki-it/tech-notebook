...
バリデーションを実装できるパッケージが世の中にあるが、ビジネスロジックは外部に依存しないほうがいいため、言語のそのままのロジックで実装すると良い。

複数のエンティティをまたぐドメインルールのバリデーションはSpecificationパターンに切り分けるのがよい。
...
単一のエンティティに関する単一のドメインルールの場合は、Specificationパターンとして切り分けずに、そのエンティティにboolean値関数 (`isFoo()` 関数) をもたせる。
...
ドメインルールのバリデーションを実行するSpecificationクラスと区別するために、Criteriaオブジェクトという名前としても使用される。
...
オブジェクトが識別子 (例：DBのオートインクリメントID、UUIDなど) を持ち、他のオブジェクトと同じ属性をもっていても、区別される。

この識別子は、DBのオートインクリメントIDに対応している。
...
エンティティの識別子にはドメインルールを持たせないほうが良いため、システムで使用するIDが適する。

- DBのオートインクリメントID
- UUIDなど

例えば、ドメインルール上の識別子（例：書籍のISBN、配達物の追跡番号）はエンティティの識別子にしないほうがいい。
