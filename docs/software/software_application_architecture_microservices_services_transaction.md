...
### 実装パターン (リクエスト−レスポンスパターン、パブリッシュ/サブスクライブパターンを区別していない)
...
マイクロサービスアーキテクチャでは、トランザクションの通常のロールバック機能を使用した場合、処理に失敗したマイクロサービスだけでロールバックし、それ以前のマイクロサービスではロールバックが起こらない問題がある。
