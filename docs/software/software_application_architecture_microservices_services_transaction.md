...
### 実装パターン (リクエスト／レスポンス方式、パブリッシュ／サブスクライブ方式を区別していない)
...
マイクロサービスアーキテクチャでは、トランザクションの通常のロールバック機能を使用した場合、処理に失敗したマイクロサービスだけがロールバックする。このため、それ以前のマイクロサービスではロールバックが起こらない問題がある。
...
複数のマイクロサービスにまたがるread処理が必要な場合がある。
