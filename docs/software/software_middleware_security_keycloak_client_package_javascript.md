...
初回認証時の場合は、Keycloak に認可リクエストを送信する。

次回認証時は、任意の場所 (例：SessionStorage、LocalStorage、Cookie) に保管している資格情報をリクエストに設定する。

例えば、アクセストークンを `Authorization` ヘッダーで運搬する場合は SessionStorage や LocalStorage から取得し、`Cookie` ヘッダーの場合は Cookie から取得する。
