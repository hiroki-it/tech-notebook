...
CoreDNS で名前解決するせずに、他の DNS サーバーに名前解決を委任する。
...
例えば、`consul.local` という接尾辞をつけてドメインを指定すると、`10.150.0.1` にある DNS サーバーへ名前解決を委任できる。
...
CoreDNS のヘルスチェックを開始するまでの待機時間を設定する。
...
Cluster 内の DNS 名と Cluster 外のドメインを紐づける。
...
まずは UDP プロトコルによるルーティングを使用し、失敗した場合に TCP プロトコルを使用する。
