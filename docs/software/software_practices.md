...
アップグレードの影響は大きくなることもある (例：ダウンタイムが発生する) 。ここでは、ステークホルダーと対応日時を調整する必要がある。

そのために、アンケートを実施する。

| 実行環境 | 都合が良ければチェック | 日付      | 時間            | 補足 |
| -------- | ---------------------- | --------- | --------------- | ---- |
| `tes`    | [ ]                    | m 月/d 日 | `16:00 ~ 19:00` |      |
|          | [ ]                    | m 月/d 日 | `16:00 ~ 19:00` |      |
|          | [ ]                    | m 月/d 日 | `16:00 ~ 19:00` |      |
| `stg`    | [ ]                    | m 月/d 日 | `16:00 ~ 19:00` |      |
|          | [ ]                    | m 月/d 日 | `16:00 ~ 19:00` |      |
|          | [ ]                    | m 月/d 日 | `16:00 ~ 19:00` |      |
| `prd`    | [ ]                    | m 月/d 日 | `02:00 ~ 05:00` |      |
|          | [ ]                    | m 月/d 日 | `02:00 ~ 05:00` |      |
|          | [ ]                    | m 月/d 日 | `02:00 ~ 05:00` |      |
...
また、本番環境であればユーザーへ事前に周知しておき、その時間以内にアップグレードを完了させる必要がある。
...
| `tes`    | m 月/d 日 | `16:00 ~ 19:00` |                                                                                   |
| `stg`    | m 月/d 日 | `16:00 ~ 19:00` |                                                                                   |
| `prd`    | m 月/d 日 | `02:00 ~ 05:00` | ユーザーが実行環境を利用中のため、利用者の少ない `03:00` から実施する必要がある。 |
...
アップグレード作業中、ロードバランサーがすべてのリクエストに対してメンテナンスページ (例：`503` ステータスの静的ファイル) を返却するように、アップグレード直前にロードバランサーの設定値を変更しておく。
...
機能廃止や非推奨機能がある。ソフトウェアがこれを使用している場合、ソフトウェアへの影響が出る可能性もある。
