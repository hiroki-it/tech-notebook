...
マイクロサービス間の通信方式でパブリッシュ／サブスクライブ方式を採用する場合に使用するメッシュ。

パブリッシュ／サブスクライブ方式では、メッセージ中継システムが専用のプロトコル (例：AMQP、MQTT、Kafka独自プロトコル) を使用することが多い。

一方で、サービスメッシュツールに適切なプロトコル (例：HTTPなど) に対応するメッセージ中継システムは少ない。 (例：Amazon SQS、Amazon SNSなど)

イベントメッシュをツールを使用するほうが良い。
