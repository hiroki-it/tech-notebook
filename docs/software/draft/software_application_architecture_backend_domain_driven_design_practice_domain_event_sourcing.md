...
async function findTop3RecommendedBooks(bookId: BookId): Promise<string[]> {
  ...
  // 対象書籍に紐づく全Reviewをたどり、それぞれのイベントを読み出して再構築している。
  // 横断検索や集計では、この処理コストが大きくなりやすい。
...
| 順番 | イベント              | わかること               |
| ---- | --------------------- | ------------------------ |
| 1    | `ReviewCreated`       | 誰がレビューを作成したか |
| 2    | `ReviewCommentEdited` | いつコメントを変更したか |
| 3    | `ReviewRatingChanged` | いつ評価を変更したか     |
| 4    | `ReviewDeleted`       | いつ削除したか           |
...
async function getAuditTrail(reviewId: string): Promise<AuditLog[]> {
  // 作成・編集・削除の履歴をイベントとして保持しているため、
  // 誰がいつ何を変更したかを追跡できる。
  return eventStore.findByAggregateId(reviewId);
}
```
