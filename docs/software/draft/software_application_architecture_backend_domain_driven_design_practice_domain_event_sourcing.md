...
複数集約を横断する検索や集計では負荷が大きくなる。書籍では、人気書籍に1万件のレビューがあり、各レビューに平均3〜4件のイベントがある例を挙げている。
...
システム要件、チームスキル、運用体制を踏まえて採用判断する。まずは従来方式で構築し、必要時に移行を検討する段階的アプローチが現実的である。
...
蓄積されたイベント履歴から、後から定義した新しい観点や指標で状態を再構築し分析できる。書籍では「レビューの信頼性スコア」を例に、設計時に存在しなかった分析軸でも過去に遡って検証できる点を説明している。
...
導入初期は、ビジネス的に重要で技術的に比較的シンプルな集約から始める。従来方式とのハイブリッドアプローチも有効である。
...
イベント数が100件を超えた場合の作成フローを図で説明している。ポイントは、イベント保存とスナップショット保存をトランザクションで扱うことである。

### 21-7-4 スナップショットの導入検討

スナップショットは技術的最適化である。最初から導入せず、性能要件を満たせない場合に検討する。

## 21-8 多様な状態導出とビジネス価値の創出

### 21-8-1 イベントソーシングの実装課題

イベント履歴だけでは、横断的な読み取り要求をうまく扱えない。書籍では、`BookRecommendationDomainService` を例に課題を説明している。
...
書き込み側は単一集約の正確な状態変更記録に、読み取り側は横断検索や集計に最適化したい。このズレを解決するのがクエリモデルである。

### 21-8-3 CQRS

CQRSは、書き込みと読み取りの責務を分離するパターンである。イベントソーシングと組み合わせると、整合性を維持しつつ、後から新しいクエリモデルを構築できる。
...
クエリサイドのデータストアは、用途に応じて選べる。代表例は次のとおりである。
