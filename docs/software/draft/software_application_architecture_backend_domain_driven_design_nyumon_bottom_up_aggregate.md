...
次のisFull関数のように、CircleがMembersの数を検証するべきである。

リスト12.7：デメテルの法則にしたがい、オブジェクトへふるまいを追加する
...
識別子にはドメインルールを持たせないほうが良いため、ドメインルール上の識別子（例：書籍のISBN）はエンティティの識別子にしない方がいい。
...
isFull関数は、『サークルに所属するユーザの最大数はサークルのオーナーとなるユーザを含めて30名まで』というドメインルールを表現したものである。
