...
オブジェクトAやBは、中核にあるオブジェクトCへ依存しているとする。
...
ServiceLocatorオブジェクトでは、オブジェクトの呼び出しを動的に切り替える。

そのため、ビルドの段階ではエラーにならない。

一方で、IoCコンテナオブジェクトではUserApplicationServiceのコンストラクタにIUserRepositoryを渡すように実装しないといけない。
