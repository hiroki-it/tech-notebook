...
これにより、フロントエンドが存在しない新しい API にリクエストを送信してしまうようなミスを防げる。
...
フロントエンドの送信元に CDN がある場合、フロントエンドのリリース後に CDN のキャッシュを削除する必要がある。
...
- 汎用的なロジック (例：アプリケーションのロガー、データポイント記録パッケージ、スパン記録パッケージ)
- API 仕様書
- 開発環境ツール
- CI/CD 設定
- IaC 設定など
