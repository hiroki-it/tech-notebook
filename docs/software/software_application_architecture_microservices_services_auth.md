...
認証サービスはトークンベースの認証情報を使用し、トークンを`Cookie`ヘッダーや`Authorization`ヘッダーで伝播する。
...
この認証サービスは、セッションベースの認証情報を使用し、セッションを`Cookie`ヘッダーで伝播する。

セッションデータを再利用するために、ブラウザのCookieやストレージ (例：Redis) に保存する。
...
トークンを再利用するために、ブラウザのCookie、SessionStorage、およびLocalStorageに保存する。
