...
ダウンロードのためのユーティリティとしては、`curl` コマンドよりも `wget` コマンドのほうが良い。
...
ドメインとIPアドレスを紐付け、指定した名前解決をしつつ、`curl` コマンドを実行する。
...
`grep` コマンドを忘れると、後続のコマンドに取得内容をパラメーターとして渡すことになる。このため、問題が起こることがある。
...
文字列を置換する。また、`-i` オプションで元のファイルを上書きする。`find` コマンドと組み合わせて、特定のファイルのみで置換できるようにすると良い。複数の置換をする場合は、`-e` オプションを並べる。
