...
> ↪️ 参考：https://kazmax.zpp.jp/linux_beginner/chmod.html
...
| 数字モード | シンボルモード | 所有者 | グループ | その他 | 特徴                                                  |
| :--------: | -------------- | :----- | :------- | :----- | ----------------------------------------------------- |
|   `500`    | 記入中...      | r-x    | ---      | ---    | 所有者以外に全権限なし。所有者は、ReadとExecuteのみ。 |
...
| 数字モード | シンボルモード | 所有者 | グループ | その他 | 特徴                                     |
| :--------: | ---------- | :----- | :------- | :----- | ---------------------------------------- |
|   `555`    | `a=+rx`      | r-x    | r-x      | r-x    | 全てにWrite権限なし。ReadとExecuteのみ。 |
|   `666`    | `a=+rw`      | rw-    | rw-      | rw-    | 全てにExecute権限なし。ReadとWriteのみ。 |
|   `777`    | `a=rwx`         | rwx    | rwx      | rwx    | 全てに全権限あり。                       |
...
| 数字モード | シンボルモード | 所有者 | グループ | その他 | 特徴                                             |
| :--------: | -------------- | :----- | :------- | :----- | ------------------------------------------------ |
|   `644`    | 記入中...      | rw-    | r--      | r--    | 所有者以外にWriteとExecute権限なしで、Readのみ。 |
|   `755`    | 記入中...      | rwx    | r-x      | r-x    | 所有者以外にWrite権限なしで、ReadとExecuteのみ。 |
...
あらかじめ、ジョブ (定期的に実行するように設定されたバッチ処理) を登録しておき、指定したスケジュールに従って、ジョブを実行する。
...
| -------- | ------------------------------------------------------------------------------ |
| `uid`    | ユーザー番号を表す。                                                           |
| `gid`    | ユーザー名を表す。Rootユーザーの場合は、`(root)`がつく。                       |
| `groups` | プライマリグループのグループ番号を表す。Rootユーザーの場合は、`(root)`がつく。 |
| `番号`   | プライマリグループ名を表す。                                                   |

> ↪️ 参考：https://kcfran.com/2022/04/06/linux-command-id/
...
ストレージ上にある、物理ボリューム、パーティション、論理ボリューム、を取得する。
...
リモートサーバーへの接続に踏み台サーバーで`ssh`コマンドを実行する必要がある場合に、踏み台サーバー上での`ssh`コマンドをローカルマシン上で行う。
