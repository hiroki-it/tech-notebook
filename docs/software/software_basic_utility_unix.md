...
なお、TCPハンドシェイクを実行する場合は`nc`コマンドを使用する。
...
指定したIPアドレス/ドメインに、TCPハンドシェイクを実行する。

`curl`コマンドではHTTPプロトコルしか扱えず、TCPハンドシェイクを検証したい場合に役立つ。
...
リモートサーバーへの接続に踏み台サーバーで`ssh`コマンドを実行する必要がある場合に、踏み台サーバー上での`ssh`コマンドをローカルマシン上で実行する。
