...
Entityは，ミュータブルであるため，Setterと```__construct```メソッドの両方を持てる．ValueObjectは，イミュータブルのため，```__construct```メソッドしか持つことができない．
...
波括弧を用いると，明示的に変数として扱える．これによって，変数の前後に半角スペースを配置しなくとも，変数は展開される．
