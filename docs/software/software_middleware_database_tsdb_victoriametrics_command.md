...
データポイントを重複排除することにより、特定の期間のなかで最新のデータポイントを残す。

重複排除のタイミングは、収集ツールの収集間隔と同じ値にするとよい。

冗長化されたデータポイント収集ツールのインスタンスが単一の VictoriaMetrics へメトリクスを送信する場合、特定の期間には冗長化されたインスタンスによる同じデータポイント送信が発生する。
...
インバウンド通信を待ち受ける IP アドレスとポート番号を設定する。
...
DB への書き込みの同時実行時に、キューで待機する最大時間を設定する。
...
DB への書き込みの最大同時実行数を設定する。

各 CPU で 1 つずつ実行するため、 設定値はホストマシン (仮想サーバー、コンテナなど) の CPU のコア数に応じて自動的に設定される。

最大同時実行数を制限することで、CPU やメモリのスパイクと、それに伴うクラッシュを防げる。
...
VictoriaMetrics では、デフォルトで `30` 個しかラベルをつけられず、それ以上のラベルは切り捨てる。
...
VictoriaMetrics が使用できるメモリサイズを設定する。
...
VictoriaMetrics が使用できるメモリサイズのうちで、許容するメモリ使用率を設定する。

これが小さいと、VictoriaMetrics が OOM キラーで停止してしまう。
...
設定したディレクトリ配下に `data` ディレクトリを作成し、その配下にメトリクスを保管する。
...
JSON、CSV、バイナリ、Prometheus の形式を選べる。
...
Prometheus の HTTP サーバーとおおよそ同じ読み出しエンドポイントを持つ。
...
Prometheus の HTTP サーバーとおおよそ同じ書き込みエンドポイントを持つ。
...
VictoriaMetrics の保管するデータサイズを表す。
...
VictoriaMetrics の読み出しのデータサイズを表す。
...
VictoriaMetrics が処理したリクエストのエラー数を表す。
