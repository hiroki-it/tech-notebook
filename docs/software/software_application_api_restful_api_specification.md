...
なお、OpenAPI Generator は OpenAPI 仕様からコードを作成できる。
...
ここでは、コードの自動作成に OpenAPI Generator を使用するものとする。

`(1)`

: OpenAPI に基づいて API 仕様を定義する。
...
: OpenAPI Generator を使用して、API 仕様から静的ファイルを自動的に作成する。
...
: OpenAPI Generator を使って、API 仕様からコードを自動的に作成する。
...
: 自動的に作成されたファイル (`api_product.go`) に実際の API を追加実装していく。
...
例えば、API がレスポンス時に以下のような JSON 型データを返信する例を考える。
...
ここで、API スキーマを以下のように定義しておき、API からデータをレスポンスするときのバリデーションを実行する。
