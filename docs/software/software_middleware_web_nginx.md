...
Nginxの起動時、最初にマスタープロセスが実行され、Nginxに設定を適用する。
...
この時、Radix Treeアルゴリズムに基づいて、IPアドレスを高速で照合している。
...
静的モジュールは、ビルド後にNginxのバイナリに組み込む必要がある。
...
以降に同じ静的ファイル (例：`html` ファイル、`css` ファイル、画像、動画、メールなど) に関するインバウンド通信があった場合、NginxはAppサーバーへルーティングせず、保管したキャッシュを取得し、レスポンスとして返信する。
...
ただし、HTTPSリクエストを受信するために、Nginxにサーバー証明書を設定する必要がある。
