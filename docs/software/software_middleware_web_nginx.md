...
また、App サーバーからのレスポンスの返信データが静的ファイルであった場合、そのキャッシュをプロキシキャッシュストレージに保管する。
...
サービスメッシュツール (例：Istio) を使用すれば、そのサイドカープロキシが宛先のコンテナにトレースコンテキストを伝播してくれるが、Nginx であれば自前で実装する必要がある。
