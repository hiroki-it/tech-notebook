...
同じ境界付けられたコンテキスト内で`1`個の意味合い/定義付けを持った『名詞』や『動詞』のこと。
...
1. 受注管理する人は、商品ドメインモデルが持つ注文内容/値段に関するデータに関心がある。
2. 在庫管理する人は、商品ドメインモデルが持つ在庫数に関するデータに関心がある。
3. 発送管理する人は、商品ドメインモデルが持つ発送状況に関するデータに関心がある。
4. 請求管理する人は、商品ドメインモデルが持つ請求状況に関するデータに関心がある。
5. 仕入れ管理する人は、商品ドメインモデルが持つ仕入れ状況に関するデータに関心がある。

商品 (Item) というオブジェクトがあった時に、業務フローの中で商品を扱う人の文脈 (コンテキスト) によって、商品ドメインモデルの持つデータへの関心が変わる。

ここでいうデータとは、具体的に商品ドメインモデルを実装した時にプロパティとしてオブジェクトに持たせているデータである。

上記の1-4は、同じ商品ドメインモデルに対して関心の内容が異なるので、境界付けられたコンテキストが異なるとみなした方が良い。

別々のマイクロサービスとして分割し、それぞれのマイクロサービス上で異なる商品ドメインモデルを定義し、異なるデータを持たせることになる。

反対にモノリスだと、一つの商品ドメインモデルにコンテキスト関係なく全ての種類のデータを持たせて、全てのコンテキストで使い回す。
...
レイヤードアーキテクチャのインフラストラクチャ層に対して、依存性逆転を組み込むかどうはには言及していない。
...
アプリケーション間通信がリクエストリプライ方式の場合に適する。
...
そのため、ステートソーシングではドメインモデルの最新の状態しか永続化できない一方で、イベントソーシング方式はドメインモデルの状態の履歴が残る。
