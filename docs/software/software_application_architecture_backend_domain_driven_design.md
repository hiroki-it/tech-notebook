...
: アーキテクチャ (レイヤード型、ヘキサゴナル型、オニオン型、クリーンアーキテクチャ) を決め、クラス図を元にエンティティ層を実装する。
...
Modelの肥大化は、Modelが持つビジネスロジックをエンティティ層、またCRUD処理をインフラ層として分割することによって、対処された。
...
エンティティ層のオブジェクトは、エンティティ層の他のオブジェクトに依存する以外、何のオブジェクトにも外部パッケージにも依存しない。

逆に考えれば、これらに依存するものはエンティティ層に配置する必要はないと判断できる。
