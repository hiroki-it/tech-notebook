...
ドメイン駆動設計を採用は必須ではなく、オブジェクト指向分析設計を行っても問題はない。
...
1. Amazonで商品を買う人は、商品オブジェクトが持つ機能/値段/発送状況に関するデータに関心がある。
2. 受注管理する人は、商品オブジェクトが持つ注文内容/値段に関するデータに関心がある。
3. 在庫管理する人は、商品オブジェクトが持つ在庫数に関するデータに関心がある。
4. 発送管理する人は、商品オブジェクトが持つ発送状況に関するデータに関心がある。

商品 (Item) というオブジェクトがあった時に、業務フローの中で商品を扱う人の文脈 (コンテキスト) によって、商品オブジェクトの持つデータへの関心が変わる。

ここでいうデータとは、具体的に商品オブジェクトを実装した時にプロパティとしてオブジェクトに持たせているデータである。

上記の1-4は、同じ商品オブジェクトに対して関心の内容が異なるので、境界付けられたコンテキストが異なるとみなした方が良い。

別々のマイクロサービスとして分割し、それぞれのマイクロサービス上で異なる商品オブジェクトを定義し、異なるデータを持たせることになる。

反対にモノリスだと、一つの商品オブジェクトにコンテキスト関係なく全ての種類のデータを持たせて、全てのコンテキストで使い回す。
