...
DBレコードの書き込み/読み出しのトランザクション処理をルートエンティティ単位で実行するため、マイクロサービスの単位がわかりやすい。
...
通知処理は外部APIとの通信を伴うため失敗しやすく、非同期処理が適する。
...
ドメイン駆動設計では、DBレコードの書き込み/読み出しのトランザクション処理をルートエンティティ単位で実行する。
