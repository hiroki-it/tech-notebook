...
| 関心事           | 目的                                 | デザインパターン名<br>・：共存関係<br>／：競合関係                                                                                                             |
| ---------------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| フロントエンド   | フロントエンド分割方法               | モノリスフロントエンド／マイクロフロントエンド                                                                                                                 |
|                  | フロントエンドレンダリング方法       | CSR／SSR／SSG／ISR                                                                                                                                             |
| APIゲートウェイ  | APIゲートウェイ分割方法              | 中央集約ゲートウェイ／BFF                                                                                                                                      |
| マイクロサービス | マイクロサービス間通信方法           | ・リクエスト−レスポンス<br>・パブリッシュ−サブスクライブ                                                                                                       |
|                  | マイクロサービス分割方法             | ・ドメイン駆動設計に基づく境界<br>・永続データの機密性の差異                                                                                                   |
|                  | マイクロサービス設計方法             | ・エンティティや値オブジェクトなどのドメイン駆動設計デザインパターン<br>・レイヤードアーキテクチャやクリーンアーキテクチャなどのアプリケーションアーキテクチャ |
|                  | ドメインモデリング方法               | ステートソーシング／イベントソーシング                                                                                                                         |
|                  | トランザクション管理方法             | オーケストレーションサーガ／コレオグラフィサーガ／２フェーズコミット                                                                                           |
|                  | 認証                                 | セッション／SSO／JWTとAPIゲートウェイの組み合わせ                                                                                                              |
|                  | 認可                                 | 中央集中管理／分散管理                                                                                                                                         |
| ストレージ       | 永続データ管理方法                   | ・DB per service／Shared DB<br>・Polyglot Persistence／Multi-model DB                                                                                          |
|                  | 揮発性データ管理                     | 中央集中管理／分散管理                                                                                                                                         |
|                  | 静的ファイル管理                     | 中央集中管理／分散管理                                                                                                                                         |
| ネットワーク     | L4/L7トラフィック管理方法            | ・サービス検出<br>・ロードバランシング                                                                                                                         |
...
| 信頼性           | 各種インスタンスの回復性管理方法     | ・ヘルスチェック<br>・リトライ<br>・タイムアウト<br>・バルクヘッド<br>・サーキットブレイカー<br>・レートリミット                                               |
...
|                  | リリース方法                         | ・インプレースデプロイメント<br>・ローリングデプロイメント<br>・カナリアリリース<br>・ブルーグリーンデプロイメント                                             |
|                  | CI/CD                                | CIOps／GitOps                                                                                                                                                  |
|                  | 汎用的ロジック配布方法               | ・Externalized configuration<br>・Microservice chassis<br>・サービステンプレート<br>・**サービスメッシュ**                                                     |
| 監視             | ログ／メトリクス／トレースの監視方法 | 中央集中管理／分散管理                                                                                                                                         |
|                  | インシデント対処方法                 | 中央集中管理／分散管理                                                                                                                                         |
| 開発体制         | 組織編成方法                         | Collective ownership／Strong ownership                                                                                                                         |
|                  | リポジトリ構成方法                   | モノレポ／ポリレポ                                                                                                                                             |
