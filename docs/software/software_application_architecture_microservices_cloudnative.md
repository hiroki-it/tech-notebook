...
|                  | マイクロサービスの分割         | ・ドメイン駆動設計に基づく境界<br>・認証／認可サービス<br>・オーケストレーションサービス<br>・外部API接続サービス<br>・非同期的な機能サービス (例：メール／プッシュ通知の機能) <br>・リアルタイム処理の機能サービス<br>・永続データの機密度 (例：換金性の高い情報) <br>・永続データの所有組織の差異 |
...
代表的なものには、ドメイン駆動設計に基づく境界、認証／認可サービス、オーケストレーションサービス、外部API接続サービス、非同期的な機能サービス (例：メール／プッシュ通知機能など) 、リアルタイム処理の機能サービス、永続データの機密度 (例：換金性の高い情報) 、永続データの所有組織があります。
...
flowchart LR
  ...
  マイクロサービス分割方法の基点 --- ドメイン駆動設計に基づく境界([ドメイン駆動設計に基づく境界])
  ドメイン駆動設計に基づく境界 --- 境界づけられたコンテキスト[境界づけられた<br>コンテキスト]
  ドメイン駆動設計に基づく境界 --- 1つ以上のルートエンティティ群[1つ以上の集約]
  マイクロサービス分割方法の基点 ---- 認証／認可サービス[認証／認可]
  マイクロサービス分割方法の基点 ---- オーケストレーションサービス[オーケストレーションサービス]
  マイクロサービス分割方法の基点 ---- 外部API接続サービス[外部API接続サービス]
  マイクロサービス分割方法の基点 ---- 非同期的な機能サービス[非同期的な機能サービス]
  マイクロサービス分割方法の基点 ---- 非同期的な機能サービス[リアルタイム処理の機能サービス]
