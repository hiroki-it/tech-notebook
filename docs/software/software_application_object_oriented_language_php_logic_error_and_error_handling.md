---
title: 【知見を書きなぐるサイト】エラーとエラーハンドリング@PHP
---
...
例外をtry-catch文でキャッチした後，別の新しい例外をスローしてもよい．その場合は，例外のコンストラクタの第三引数（```previous```）を用いて，キャッチされていた元の例外も検知できるようにする．ちなみに，この例外をロギングする場合，スタックトレースログとして出力される．
