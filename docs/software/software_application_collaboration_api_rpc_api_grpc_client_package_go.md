...
gRPCクライアント側とgRPCサーバー側の両方で、サービス定義ファイル (`proto`ファイル) を作成する。
...
gRPCクライアント側とgRPCサーバー側の両方で、自動作成のために、`protoc`コマンドをインストールする。
...
このファイルには、gRPCクライアント側とgRPCサーバー側の両方が参照するための実装が定義されており、開発者はそのまま使用すれば良い。
...
`pb.go`ファイルには、gRPCクライアント側とgRPCサーバー側の両方が参照するための構造体や関数が定義されており、ユーザーはこのファイルをそのまま使用すれば良い。
