...
オーケストレーターは、現在のイベントをDBやログで管理する。

管理している現在のイベントの状態に応じて、次のイベントを実行する。
...
オーケストレーターがイベントのパブリッシュとサブスクライブの責務を持つ。

オーケストレーターは、メッセージングキューに対してパブリッシュとサブスクライブを実行する。

サブスクライブしたイベントに応じて、次のイベントをパブリッシュする。
