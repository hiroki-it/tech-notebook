...
各マイクロサービス間の通信方式は、リクエスト/レスポンスパターンまたはパブリッシャー/サブスクライバーパターンのどちらでもよい。
...
### 実装パターン (リクエスト/レスポンスパターン、パブリッシャー/サブスクライバーパターン)
...
各マイクロサービス間の通信方式は、パブリッシャー/サブスクライバーパターンにする必要がある。
...
- AWS Lambda、マイクロサービス間のパブリッシャー/サブスクライバーAWSリソース (例：AWS EventBridge、AWS SQS、AWS SNS）
- Google Cloud Function、マイクロサービス間のパブリッシャー/サブスクライバーGoogle Cloudリソース (例：Google Eventarc）
