...
クリーンアーキテクチャ原著の序文にて、著者は「私は、今まで色々な種類のシステムを作ってきたが、どのシステムでも、アーキテクチャのルールは同じだった」と述べている。
...
金額データの計算をInteractor内処理やエンティティ内関数で行うのではなく、金額計算を行う値オブジェクトの関数として分割する。
...
所要時間データの計算をInteractorクラス内処理やエンティティ内関数で行うのではなく、所要時間計算を行う値オブジェクトの関数として分割する。
...
その時は、保持している属性を反復的に検証できるように実装するとよい。
...
フレームワークの機能に依存することになるため、実装の詳細をインフラストラクチャ層に置く。
...
より上位レイヤーでは、そのレイヤーに合った例外に詰め替えてスローする。
