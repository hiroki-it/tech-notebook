...
jQueryでは，パッケージの読み出し宣言時に，『Jquery』という名前の代わりにドルマークを用いる仕様になってる．これと混乱しないように注意する．
...
順序を保ったまま配列を走査し，値を取得する．オブジェクトに対して```entires```メソッドを用いて，一度配列に変換すれば，オブジェクトでも```for  of```を用いることができる．```for  in```を用いるより，こちらを用いた方が良い．
...
オブジェクトに対して```entires```メソッドを実行し，一度連想配列に変換すれば，オブジェクトでも```for ... of```を用いることができる．
