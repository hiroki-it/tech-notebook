...
|          | 関数名                  | 引数型                                | 返却値型                                          | 処理内容                                                                                                                               |
|----------| ----------------------- | ------------------------------------- |-----------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------|
| 閲覧系 (単一) | `getFoo`                | `getFooRequest`                       | `getFooResponse`または`null`                     | リポジトリの`findOne`関数をコールし、ドメインオブジェクトを取得する。                                                                                            |
| 閲覧系 (複数) | `listFoos`              | `listFoosRequest`                     | 空配列を含む`listFoosResponse`                      | リポジトリの`findAll`関数をコールし、ドメインオブジェクトを取得する。                                                                                            |
| 登録系      | `registerFoo`           | `registerFooRequest`                  | `registerFooResponse`または`<例外オブジェクト>`                    | リポジトリの`findOne`関数をコールし、重複確認を実施する。リポジトリの`create`関数をコールし、ドメインオブジェクトを新規登録する。                                                          |
| 変更系      | `changeFoo`             | `changeFooRequest`                    | `changeFooResponse` または`<例外オブジェクト>`                     | リポジトリの`findOne`関数と`update`関数をコールし、既存のドメインオブジェクトを更新する。                                                                              |
| 変更系      | `saveFoo` (`upsertFoo`) | `saveFooRequest` (`upsertFooRequest`) | `saveFooResponse` (`upsertFooResponse`) または`<例外オブジェクト>` | リポジトリの`findOne`関数をコールし、存在確認を実施する。その結果に応じて`create`関数または`update`関数を実行する。`<br>`https://github.com/little-hands/ddd-q-and-a/issues/241 |
| 削除系      | `deleteFoo`             | `deleteFooRequest`                    | `deleteFooResponse`または`<例外オブジェクト>`            | リポジトリの`findOne`関数と`delete`関数をコールし、既存のドメインオブジェクトを削除する。                                                                              |
