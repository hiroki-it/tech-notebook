...
|          | 関数名                  | 引数型                                | 返却値型                                | 処理内容                                                                                                                                          |
|----------| ----------------------- | ------------------------------------- | --------------------------------------- |-----------------------------------------------------------------------------------------------------------------------------------------------|
| 閲覧系 (単一) | `getFoo`                | `getFooRequest`                       | `getFooResponse`                        |                                                                                                                                               |
| 閲覧系 (複数) | `listFoos`              | `listFoosRequest`                     | `listFoosResponse`                      |                                                                                                                                               |
| 登録系      | `registerFoo`           | `registerFooRequest`                  | `registerFooResponse`                   | リポジトリの`create`関数をコールし、ドメインオブジェクトを新規登録する。                                                                                  |
| 変更系      | `changeFoo`             | `changeFooRequest`                    | `changeFooResponse`                     | リポジトリの`update`関数をコールし、既存のドメインオブジェクトを更新する。                                                                                  |
| 変更系      | `saveFoo` (`upsertFoo`) | `saveFooRequest` (`upsertFooRequest`) | `saveFooResponse` (`upsertFooResponse`) | リポジトリの`find`関数をコールし、ドメインオブジェクトを取得する。これを使用して重複確認を実行し、その結果に応じてCREATEまたはUPDATEを実行する。`<br>`https://github.com/little-hands/ddd-q-and-a/issues/241 |
| 削除系      | `deleteFoo`             | `deleteFooRequest`                    | `deleteFooResponse`                     | リポジトリの`delete`関数をコールし、既存のドメインオブジェクトを削除する。                                                                                  |
