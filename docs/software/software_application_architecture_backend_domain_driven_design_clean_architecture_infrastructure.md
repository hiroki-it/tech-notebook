...
| `findWith<異なる境界づけられたコンテキストにあるほかの集約名>()` | 異なる境界づけられたコンテキストにあるほかの集約 | ドメインモデル型           | 異なる境界づけられたコンテキストにある集約を特定のアプリケーションで扱うことはよくないが、扱わないといけない場合に使用する。異なる境界づけられたコンテキストにあるほかの集約を含むルートエンティティのドメインモデルを取得する。特定の集約 Aに紐づくリポジトリでほかの集約 Bを取得したい場合、集約 Bを直接取得するのはよくない。集約 Aと B間のリレーションを使用し取得することで、取得する集約 Aの主体はそのままで、関連する集約 Bも一緒に取得できる。取得した集約 Aから集約 Bを取り出し、必要なデータを操作する。 |
...
| ---------------- | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Active Record    | DB 駆動      | ・非常に強い。`<br>`・手順としてテーブル設計が先にあり、`1` 個のドメインモデルが `1` 個のテーブルに対応している。`<br>`・テーブル間のリレーションシップによって、ドメインモデル間の依存関係が決まる。 | ・Eloquent (PHP)`<br>`・Active Record (Ruby) `<br>`・Hibernate (Java) | ビジネスロジックが複雑でないアプリケーション `<br>`https://www.informit.com/articles/article.aspx?p=1398618&seqNum=3 | DataMapper パターンと同じく、ORM の実装方法の 1つである。`<br>`https://culttt.com/2014/06/18/whats-difference-active-record-data-mapper/   |
| Data Mapper      | ドメイン駆動 | ・弱い `<br>`・Entity マネージャを使用して、ドメインモデルを DB に永続化する。                                                                                                                        | Doctrine                                                              | ビジネスロジックが複雑なアプリケーション `<br>`https://www.informit.com/articles/article.aspx?p=1398618&seqNum=3     | ActiveRecord パターンと同じく、ORM の実装方法の 1つである。`<br>`https://culttt.com/2014/06/18/whats-difference-active-record-data-mapper/ |
