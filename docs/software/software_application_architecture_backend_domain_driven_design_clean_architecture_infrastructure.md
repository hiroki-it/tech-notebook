...
| `findWith<関連するほかの集約名>()` | 関連するほかの集約                   | ドメインモデル型           | ほかの集約を含むルートエンティティのドメインモデルを取得する。特定の集約Aに紐づくリポジトリでほかの集約Bを取得したい場合、集約Bを直接取得するのはよくない。集約AとB間のリレーションを使用し取得することで、取得する集約Aの主体はそのままで、関連する集約Bも一緒に取得できる。取得した集約Aから集約Bを取り出し、必要なデータを操作する。 |
