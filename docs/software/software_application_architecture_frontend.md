...
CSRでは、ブラウザ上のJavaScriptがバックエンドからデータを取得し、JavaScriptの作成する静的ファイルに出力する。
...
SSGでは、フロントエンドアプリケーションへのリクエスト時にバックエンドからデータを取得し、それを静的ファイルに出力した上でブラウザに返信する。
...
SSGでは、フロントエンドアプリケーションのビルド時にバックエンドからデータを取得し、それを静的ファイルに出力した上でブラウザに返信する。
