...
| AWS ALB                            | AWS Certificate Managerの管理するSSL証明書を、AWS ALBに紐づける。ユーザーがAWS EKSクラスターにリクエストを送信する時に、通常のTLSを実行できるようになる。なお、ここでSSL/TLS終端とし、アップストリームのIstio IngressGatewayでSSL/TLSを再開する。 |
| AWS Aurora                         | SSL証明書をAWS Auroraに紐づける。マイクロサービスがAWS Auroraにトランザクションを実行する時に、通常のTLSを実行できるようになる。                                                                                                                  |
