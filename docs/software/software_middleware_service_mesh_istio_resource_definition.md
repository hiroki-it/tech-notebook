...
リクエストヘッダーにあるJWTトークンの発行元認証局が適切な場合に、認可を実施するように設定する。
...
HTTP/2の場合は、TCPスリーウェイハンドシェイク上で単一のストリームを確立し、ストリーム内で単一または複数のリクエスト−レスポンスを並行的に送受信する。
...
JWTトークンが失効していたり、不正な場合に、認証処理を失敗として`401`ステータスを返信する。
