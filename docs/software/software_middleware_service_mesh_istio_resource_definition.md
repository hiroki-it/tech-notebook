...
リクエスト時に、`istio-proxy`コンテナが宛先アプリコンテナに接続できた場合は以下である。

| 設定                     | 理由                                                                                                                                                       |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cancelled`              | リクエスト時に、`istio-proxy`コンテナが宛先アプリコンテナに接続できた。しかし、レスポンスでgRPCステータスコードが`Cancelled`であった。                     |
| `connect-failure`        | リクエスト時に、`istio-proxy`コンテナが宛先アプリコンテナに接続できた。しかし、レスポンスで読み取りタイムアウトが起こった。                                |
| `deadline-exceeded`      | リクエスト時に、`istio-proxy`コンテナが宛先アプリコンテナに接続できた。しかし、レスポンスでgRPCステータスコードが`DeadlineExceeded`であった。              |
| `gateway-error`          | リクエスト時に、 `istio-proxy`コンテナが宛先アプリコンテナに接続できた。しかし、レスポンスでGateway系ステータスコード (`502`、`503`、`504`) が返信された。 |
| `refused-stream`         | リクエスト時に、`istio-proxy`コンテナが宛先アプリコンテナに接続できた。しかし、レスポンスでgRPCステータスコードが`Unavailable`であった。                   |
| `reset`                  | リクエスト時に、`istio-proxy`コンテナが宛先アプリコンテナに接続できた。しかし、レスポンスで通信の切断／リセット／タイムアウト (特に) が起こった。          |
| `resource-exhausted`     | リクエスト時に、`istio-proxy`コンテナが宛先アプリコンテナに接続できた。しかし、レスポンスでgRPCステータスコードが`ResourceExhausted`であった。             |
| `retriable-status-codes` | リクエスト時に、`istio-proxy`コンテナが宛先アプリコンテナに接続できた。しかし、レスポンスでHTTPステータスコードが`503`ステータスであった。                 |
| `unavailable`            | リクエスト時に、`istio-proxy`コンテナが宛先アプリコンテナに接続できた。しかし、レスポンスでgRPCステータスコードが`Unavailable`であった。                   |

レスポンス時に、`istio-proxy`コンテナが宛先アプリコンテナに接続できなかった場合は以下である。

| 設定                   | 理由                                                                                                                                                               |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `reset-before-request` | リクエスト時に、`istio-proxy`コンテナが宛先アプリコンテナに接続できた。しかし、宛先アプリコンテナから`istio-proxy`コンテナに返信できず、通信のリセットが起こった。 |
