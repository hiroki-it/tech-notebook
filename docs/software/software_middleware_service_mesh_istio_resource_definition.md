...
レスポンス時に、宛先アプリコンテナが`istio-proxy`コンテナに返信できた場合は以下である。

| 設定                     | 理由                                                                                                                                             |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `cancelled`              | レスポンス時に、宛先アプリコンテナから`istio-proxy`コンテナに返信できた。しかし、gRPCステータスコードが`Cancelled`であった。                     |
| `connect-failure`        | レスポンス時に、宛先アプリコンテナから`istio-proxy`コンテナに返信できた。しかし、読み取りタイムアウトが起こった。                                |
| `deadline-exceeded`      | レスポンス時に、宛先アプリコンテナから`istio-proxy`コンテナに返信できた。しかし、gRPCステータスコードが`DeadlineExceeded`であった。              |
| `gateway-error`          | レスポンス時に、 宛先アプリコンテナから`istio-proxy`コンテナに返信できた。しかし、Gateway系ステータスコード (`502`、`503`、`504`) が返信された。 |
| `refused-stream`         | レスポンス時に、宛先アプリコンテナから`istio-proxy`コンテナに返信できた。しかし、gRPCステータスコードが`Unavailable`であった。                   |
| `reset`                  | レスポンス時に、宛先アプリコンテナから`istio-proxy`コンテナに返信できた。しかし、通信の切断／リセット／タイムアウト (特に) が起こった。          |
| `resource-exhausted`     | レスポンス時に、宛先アプリコンテナから`istio-proxy`コンテナに返信できた。しかし、gRPCステータスコードが`ResourceExhausted`であった。             |
| `retriable-status-codes` | レスポンス時に、宛先アプリコンテナから`istio-proxy`コンテナに返信できた。しかし、HTTPステータスコードが`503`ステータスであった。                 |
| `unavailable`            | レスポンス時に、宛先アプリコンテナから`istio-proxy`コンテナに返信できた。しかし、gRPCステータスコードが`Unavailable`であった。                   |

レスポンス時に、宛先アプリコンテナが`istio-proxy`コンテナに返信できなかった場合は以下である。
