...
認可スコープで、実施条件 (例：いずれのKubernetesリソース、HTTPリクエストのメソッド、JWTトークンの発行元認証局の識別子) を設定する。
...
リクエストヘッダーにあるJWTトークンの発行元認証局が適切な場合、認可を実施するように設定する。
...
GatewayでHTTPSリクエストを受信した場合、SSL証明書を検証をせずに、HTTPSをそのまま通過させる。
...
Bearer認証で使用するJWTトークンの発行元認証局を設定する。
...
`Cookie`ヘッダーを使用して資格情報を運ぶ場合 (例：フロントエンドアプリケーションがCSRやSSR) に役立つ。
...
`Authorization`ヘッダーを使用して資格情報を運ぶ場合 (例：フロントエンドアプリケーションがCSR) に役立つ。
