...
サイドカーモデルは、サイドカープロキシ型のサービスメッシュを実装したものである。
...
サイドカーモデルは、データプレーン、Isiodコントロールプレーン、といったコンポーネントから構成される。
...
アンビエントメッシュは、カーネルモデルのサービスメッシュを実装したものである。
...
サイドカーモデルを将来的に廃止するということはなく、好きな方を選べるようにするらしい。

ztunnelのPodを経由した段階でHTTPSプロトコルになる。

ハードウェアリソースの消費量の少ない`L4`プロトコルと、消費量の多い`L7`プロトコルのプロコトルの処理の責務が分離されているため、サイドカーモデルと比較して、`L4`プロトコルのみを処理する場合に、Nodeのハードウェアリソース消費量を節約できる。
