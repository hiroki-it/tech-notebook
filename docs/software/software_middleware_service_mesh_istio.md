...
Istioでは、外れ値の排出率を`100`%とすることで、ステータスコードもサーキットブレイカーの条件にできる。
...
Istio IngressGatewayを経由せずにサービスメッシュ外からインバウンド通信 (例えば、Prometheusやkubeletの`/metrics`スクレイピング) がくると、ラベル値が`unknown`になってしまう。
