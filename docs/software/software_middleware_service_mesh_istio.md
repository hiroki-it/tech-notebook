...
ハードウェアリソースの消費量の少ない`L4`プロトコルと、消費量の多い`L7`プロトコルのプロコトルの処理の責務が分離されているため、サイドカーモードと比較して、`L4`プロトコルのみを処理する場合に、Nodeのハードウェアリソース消費量を節約できる。
...
ztunnel Podが`L4` (トランスポート層) のプロトコル (例：TCP、UDPなど) を処理できる。
