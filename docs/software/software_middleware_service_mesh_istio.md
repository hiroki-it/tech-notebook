...
サイドカー型は、サイドカープロキシ型のサービスメッシュを実装したものである。
...
サイドカー型は、データプレーン、Isiodコントロールプレーン、といったコンポーネントから構成される。
...
アンビエントメッシュは、サイドカーレス型のサービスメッシュを実装したものである。
...
サイドカー型を将来的に廃止するということはなく、好きな方を選べるようにするらしい。

ztunnelのPodを経由した段階でHTTPSプロトコルになる。

ハードウェアリソースの消費量の少ない`L4`プロトコルと、消費量の多い`L7`プロトコルのプロコトルの処理の責務が分離されているため、サイドカー型と比較して、`L4`プロトコルのみを処理する場合に、Nodeのハードウェアリソース消費量を節約できる。
...
コントロールプレーン側では〇〇 (AWS ECSを認識するためのリソースが必要だがわからず...) の作成、クラウド上のコンテナのホストマシンではztunnelデーモンあるいはztunnelコンテナの実行が必要である。
