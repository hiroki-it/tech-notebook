...
サーバー側に、IDプロバイダーを認証マイクロサービスとして、SSOを実行する。
...
サーバー側に、セッションデータを作成する認証マイクロサービス (例：自前、Keycloak、など) を`1`個だけ配置し、認証処理を実行する。
...
サーバー側に、JWTを作成する認証マイクロサービス (例：自前、Keycloak、など) を`1`個だけ配置し、認証処理を実行する。
...
サーバー側に、JWTを作成する認証マイクロサービス (例：自前、Keycloak、など) を`1`個だけ配置し、一方でクライアント側ではOpaqueトークンを保存し、認証処理を実行する。
