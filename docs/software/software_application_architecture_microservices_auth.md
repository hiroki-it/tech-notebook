...
この認証サービスは、アカウント情報を永続化するためのDB、またはセッションを保存するためのストレージを持つ。
...
この認証サービスは、アカウント情報を永続化するためのDBを持つ。
...
この認証サービスは、セッションデータを保存するためのストレージを持つ。
...
一方でクライアント側ではOpaqueトークンを保存し、認証処理を実行する。
