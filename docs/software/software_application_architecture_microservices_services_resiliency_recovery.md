...
処理（例：通信、DBクエリ）が失敗した場合に、一時的な問題に起因している可能性を考慮して、再実行によって処理の成功を試みるデザインパターンである。
...
タイムアウトは、時間のかかる処理（例：通信、計算など）に制限時間を設けるデザインパターンである。これにより、時間のかかる処理がハードウェアリソースを使用し続けることを防ぎ、他の処理にハードウェアリソースを譲る。
...
サーキットブレイカーパターンは、マイクロサービス間の通信方式がリクエスト−レスポンスパターンの場合に、リクエストの場合には連鎖的な障害が起こるため、これに対処する必要がある。

宛先マイクロサービスからのレスポンスに外れ値 (`500`系ステータス率、Gateway系ステータス率、接続プール上限など) の閾値を設定しておく。
...
3. 障害の起こったマイクロサービスを指定された期間排出する。
4. 障害が回復次第、ルーティングを再開する。
...
フォールバックパターンは、リトライの失敗やサーキットブレイカーで宛先マイクロサービスからデータを取得できなかった場合に、代わりにデータを返信する。
