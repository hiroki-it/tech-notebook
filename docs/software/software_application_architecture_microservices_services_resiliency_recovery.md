...
タイムアウトは、時間のかかる処理（例：`for`文、HTTPリクエスト、ループ、ポーリング、DB操作、ファイル操作、計算、暗号化、圧縮解凍など）に制限時間を設けるデザインパターンである。
...
通信処理において、TCP接続が確立した後、宛先へリクエストが到達してから、レスポンスの返信を待機するまでの最大待機時間である。
...
スリーウェイハンドシェイクのTCP接続中に無通信状態 (パケットの送受信がない状態) を猶予する最大時間である。
...
フォールバックパターンは、エラーハンドリングの方法の1つである。
...
そのため、例外スローを別の識別子 (例：boolean値) に変えて関数コールしたクラスに持ち上げ、最終的には、これをポップアップなどでわかりやすく通知する必要がある。
