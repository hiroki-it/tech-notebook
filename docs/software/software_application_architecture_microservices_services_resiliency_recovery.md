...
サーキットブレイカーは、レスポンスを対象とした外れ値（例：500系ステータスコード率、Gateway系ステータス率、接続プール上限）が閾値を超過した場合に、宛先を指定した期間だけ排出（Eject）し、回復を待機するデザインパターンである。
...
2. 宛先マイクロサービスを排出し、特定のエラーステータス (`503`など) でレスポンスを返信する。
