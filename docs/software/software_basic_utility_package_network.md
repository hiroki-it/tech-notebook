...
正引き/逆引きによる名前解決をし、この時に指定したDNSレコードタイプのレコード値を返却させる。

**＊例＊**

名前解決をし、NSレコード値を返却させる。
...
指定したネットワークインターフェースにて、パケットを取得する。`-i` オプションを使用しない場合、全てのネットワークインターフェースが扱うパケットを取得することになる。
