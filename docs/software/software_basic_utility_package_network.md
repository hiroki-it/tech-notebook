...
もしドメイン名に複数の IP アドレスが割り当てられている場合、正引きを実行すると、すべての IP アドレスが返却される。
...
正引き/逆引きによる名前解決をし、このときに指定した DNS レコードタイプのレコード値を返却させる。

**＊例＊**

名前解決をし、NS レコード値を返却させる。
...
オプション (`-p`、`-l`、`-u`、`-n`、`-t`) の組み合わせ。各プロセスが開放しているポート番号、ポート番号で受信するプロトコル、接続状態などの一覧を取得する。
...
パケットの送信元と宛先から、送信元からのリクエスト (`ping`、`nc`、`curl` など) が届いているかを確認する。
...
スリーウェイハンドシェイクによって接続を確率するまでの仕組みである。

(1) 送信元から宛先に SYN パケットを送信する。
...
(2) 宛先から送信元に ACK パケットと SYN パケットを返信する。
...
`awk` コマンドや `grep` コマンドと組み合わせると、特定の IP アドレスを送信元/宛先としたパケットがあるか否かを検出できる。
...
指定したネットワークインターフェースにて、パケットを取得する。`-i` オプションを使用しない場合、すべてのネットワークインターフェースが扱うパケットを取得することになる。
...
UDP プロトコルを使用して、パケットを送信する。

もし、`traceroute` コマンドが終了すれば、すべてのルーターを経由できていることを表す。
...
ICMP プロトコルを使用して、Ping (ICMP エコーリクエスト) を送信する。
...
IP アドレスの名前解決するせずに、ルーターの送信元 IP アドレスをそのまま取得する。
...
宛先に TCP スリーウェイハンドシェイクを実行したり、HTTP や HTTPS でパケットを送信する。

その後、通信の送信元から宛先までに通過するルーターの送信元 IP アドレスを取得する。

`traceroute` コマンドでは UDP プロトコルで送信するため、ネットワークが正常でもそれ以外 (ファイアウォールなど) のところで通信できない場合がある。
