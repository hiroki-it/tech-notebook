...
4. ユースケース図に登場する名詞に基づき、ドメインオブジェクト図 (ドメインモデルを具体化する) やドメインモデル図 (ドメインモデルを抽象化する) を作成する。
...
オーケストレーションベースのように中央集中的に各マイクロサービスを操作するのではなく、特定のマイクロサービスが他のマイクロサービスを操作する方式である。
