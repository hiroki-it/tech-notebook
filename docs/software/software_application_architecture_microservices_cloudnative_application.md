...
クリーンアーキテクチャは、４つのアプリアーキテクチャ層 (インフラ層、インターフェース層、ユースケース層、エンティティ層)
...
| エンティティ層               | エンティティ、値オブジェクト、タイプコード、ドメインサービス、ドメインイベント、検索条件など | -                                                                                                                      |
...
| 図中のコンポーネント                        | 責務                                                                                                                                                                                                                                              |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| マイクロサービス (Temporalクライアント)     | エンティティ層には、ワークフローの定義ロジックを実装する。インフラ層に、クライアントからの同期リクエストを受信するためのAPIとTemporal SDKクライアントを実装する。                                                                                 |
| ワークフローステータスチェッカー            | エンティティ層には、取得するワークフローステータスのロジックを実装する。インフラ層にTemporal SDKのロジックを実装する。インフラ層に、クライアントからの非同期リクエスト (ポーリング処理) を受信するためのAPIとTemporal SDKクライアントを実装する。 |
...
14. マイクロサービスは、エンティティ層の認可を実行する。認可スコープに応じたCRUDをDBに実行する。
...
sequenceDiagram
    ...
    IDプロバイダー (Keycloak、Google) -->> マイクロサービス : レスポンス<br>(トークン検証結果)
    マイクロサービス ->> マイクロサービス: 認可実行<br>(エンティティ層)
...
認可ロジックはエンティティ層と似たような責務を持つため、分散パターンとして各マイクロサービスで実装するとよいです。
