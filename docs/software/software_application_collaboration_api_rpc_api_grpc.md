...
gRPCでは、クライアントとサーバーの間の通信方式に種類がある。通信方式は、```.proto```ファイルで定義する。
...
gRPCにおけるAPI仕様の実装であり、実装によりAPI仕様を説明する。クライアント側とサーバー側の両方で作成する必要がある。サービス定義ファイルにインターフェースとメッセージ構造を実装し、このファイルから```pb.*```ファイルを自動作成する。
