...
プル型のサブスクライブの場合、サブスクライバーはRabbitMQにポーリングを実行し、メッセージの取得を待機する（サブスクライブによる購読予約はない）。

注意点として、Kafkaのプル型はRabbitMQと仕組みが異なり、サブスクライブによる購読予約をKafkaに実行して予約し、その上でKafkaにポーリングを実行する必要がある。
