...
| `URX` | `UPSTREAM_RETRY_LIMIT_EXCEEDED`   |       なし       | アップストリームに対するリトライの上限数の超過で、Envoyはアップストリームに接続拒否されてしまった。 |

ダウンストリームが原因のメッセージは以下の通りである。

| 名前  | 正式名称                            | ステータスコード | 説明                                                                                      |
| ----- | ----------------------------------- | :--------------: | ----------------------------------------------------------------------------------------- |
| `DC`  | `DOWNSTREAM_CONNECTION_TERMINATION` |       なし       | Envoyのアップストリームへのリクエストが中断され、Envoyはレスポンスを受信できなかった。    |
