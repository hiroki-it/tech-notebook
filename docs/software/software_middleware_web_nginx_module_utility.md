...
Nginx では、モジュールを事前にビルドし、バイナリに組み込む必要がある。
...
gRPC Exporter を使用するために、gRPC パッケージが必要である。
...
トレースコンテキスト仕様として、W3C Trace Context を設定する。

執筆時点 (2024/05/02) では、他のトレースコンテキスト仕様 (例：X-Ray など) を伝播できない。
