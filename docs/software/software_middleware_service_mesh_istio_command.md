...
Namespace を指定しつつ、`analyze` コマンドを実行する。
...
IstioOperator のマニフェストを送信し、Kubernetes リソースを作成する。
...
IstioOperator を使用して、アクセスログの出力先を標準出力に変更する。
...
カナリアアップグレード時に使用するが、このオプションを使用しないと Kubernetes リソース名にリビジョンがつかないため、インストール時にも使用したほうがよい。
...
指定したマニフェストの Pod に istio-proxy をインジェクションする。
...
IstioOperator を `istio-system` に作成する。
...
カナリアアップグレード前に、現在のバージョンのエイリアスとリビジョン (現在の Istio のバージョンタグ) を確認するために使用する。
...
: `default` タグを持つ MutatingWebhookConfiguration を確認する。
...
: すると、既存の MutatingWebhookConfiguration を残して、新しい MutatingWebhookConfiguration を作成する。
