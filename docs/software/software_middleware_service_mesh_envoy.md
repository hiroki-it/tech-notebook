...
一方で、マイクロサービスにおける Envoy では、Envoy のコントロールプレーンが通信の送信元と宛先の両方のリバースプロキシを一緒に管理する。
...
コントロールプレーンは、データプレーンの Envoy を管理する。サービス検出のための API (XDS-API) を持つ。
...
単一のエンドポイントを提供し、適切な順番で各 XDS-API から宛先情報を取得できる。

各 XDS-API から宛先情報を取得してもよいが、ADS-API で一括して取得することできる。
...
データプレーンでは、Envoy が稼働し、通信を宛先にルーティングする。
...
: Envoy は、起動時にリスナーとクラスターを XDS-API から取得する。

     取得した宛先情報を自身に設定する。

`(2)`

: Envoy は、リスナーに紐付ける必要のあるルートを特定する。

`(3)`

: Envoy は、クラスターに紐付ける必要のあるエンドポイントを特定する。

`(4)`

: Envoy は、ルートとエンドポイントを XDS-API から取得する。
...
リスナーでは、Envoy に対する通信を IP アドレスとポートで待ち受ける。
...
`envoy.yaml` ファイルで `listeners` キーを設定することで、Envoy にリスナーを静的に設定できる。
...
Envoy は、起動時にコントロールプレーンの LDS-API にリモートプロシージャーコールを単方向/双方向で実行し、宛先のリスナーを取得する。

また、Envoy は宛先のリスナーを自身に動的に設定する。
...
Istio を使用して、`envoy` コンテナを稼働させるとする。
...
各フィルターは、ReadFilter と WriteFilter に分類できる。
...
Envoy は、起動時にコントロールプレーンの RDS-API にリモートプロシージャーコールを単方向/双方向で実行し、宛先のルートを取得する。

また、Envoy は宛先のルートを自身に動的に設定する。
...
クラスターでは、ルートからルーティングされた通信を受け取り、いずれかのエンドポイントの IP アドレスとポートにロードバランシングする。
...
`envoy.yaml` ファイルで `clusters` キーを設定することで、Envoy にクラスターを静的に設定できる。
...
Envoy は、起動時にコントロールプレーンの CDS-API にリモートプロシージャーコールを単方向/双方向で実行し、宛先のクラスターを取得する。

また、Envoy は宛先のクラスター設定を自身に動的に設定する。
...
Envoy はマルチスレッドでパケットを処理する。

メモリ上のプロセスで、メインスレッド、ワーカースレッド、ファイルフラッシュスレッドを持つ。
...
メモリ上のプロセス内の各ワーカースレッドが独立してリクエストを待ち受ける。
...
ファイル (例：アクセスログ) は一度メモリバッファー上に移動され、その後にフラッシュされる。
...
このとき、Envoy は `http://localhost:<ポート番号>` で待ち受け、一方で外部が指定する URL も `http://localhost:<ポート番号>` である。

サービスメッシュ外や他の Pod からリクエストを受信するために使用する。
...
このとき、Envoy は `http://0.0.0.0:<ポート番号>` で待ち受け、一方でローカルホストにあるマイクロサービスが指定する URL は `http://<マイクロサービスのホスト>:<ポート番号>` である。

サービスメッシュ外や他の Pod にリクエストを送信するために使用する。
...
Envoy は、リバースプロキシとして、ローカルホスト外にあるマイクロサービスへ通信を送信する。
...
なお、Envoy は一部のプロトコル (例：FastCGI プロトコル) をサポートしていない。
...
Envoy の文脈では、ロードバランサーとしての使い方を『フロントプロキシ』『エッジプロキシ』とも呼んでいる。

Istio Ingress Gateway で Envoy を使用するユースケースは、これに属する。
