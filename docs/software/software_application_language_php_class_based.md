...
Composerのオートロード機能を有効化した上で、外部ファイルのクラスの```namespace```を```use```で指定するだけで、そのクラスのみを読み込める。非クラスを読み込むことはできない。動的な値は持たず、静的に読み込むことに注意。ただし、チームの各エンジニアが好きな物を読み込んでいたら、スパゲッティコードになりかねない。そこで、チームでの開発では、記述ルールを設けて、```use```で読み込んで良いものを決めておくと良い。注意点として、composerのオートロードを使用しない場合、```require```関数と```use```の両方が必要である。
