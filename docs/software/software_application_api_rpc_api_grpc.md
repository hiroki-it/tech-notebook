...
RPCフレームワークの1つで、Protocol Bufferを使用してRPC (リモートプロシージャーコール) を実行する。
...
各マイクロサービスのリポジトリでは、アプリケーションのインフラストラクチャ層にgRPCクライアントとgRPCサーバーの定義を配置する。
