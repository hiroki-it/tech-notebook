...
クエリの実行時、オプティマイザーは統計情報を参照し、以下を判断しながらDBインデックスを使用する稼働かを判断する。

- 統計情報の

統計情報が古いと、テーブルのDBインデックスを使用するべきところを使用しなくて良いと、オプティマイザーが誤って判断してしまうことがある。

そのため、定期的に`ANALYZE TABLE <テーブル>`を実行し、テーブルの統計情報を更新しておくと良い。

<br>

### innodb_table_stats

各テーブルの統計情報（`n_rows`、`clustered_index_size`、`sum_of_other_index_sizes`）を管理する。
