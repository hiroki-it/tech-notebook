...
送信元マイクロサービスはメッセージブローカーまたはメッセージキューにイベントをパブリッシュする。

そして、メッセージブローカーまたはメッセージキューは、イベントを宛先マイクロサービスにサブスクリプションする。

サブスクライブではなく、宛先マイクロサービスがメッセージブローカーまたはメッセージキューにポーリングする方法もある。
...
マイクロサービス間の通信方式がリクエストロプライ方式の場合の障害対策である。
