...
送信元マイクロサービスはメッセージブローカーやメッセージキューにイベントをパブリッシュする。

そして、メッセージブローカーやメッセージキューは、イベントを宛先マイクロサービスにサブスクリプションする。

サブスクライブではなく、宛先マイクロサービスがメッセージブローカーやメッセージキューにポーリングする方法もある。
