...
取得されたコレクション型データの1つ目の要素の値を取得する．ユニーク制約の課せられたカラムを```where```メソッドの対象とする場合，コレクションとして取得されるが，コレクションが持つEloquentモデルは1つである．foreachを使用してコレクションからEloquentモデルを取り出してもよいが，無駄が多い．そこで，```first```メソッドを使用して，Eloquentモデルを直接取得する．
