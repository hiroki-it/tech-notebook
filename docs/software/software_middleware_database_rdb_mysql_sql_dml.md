...
設定した`SELECT`句が使用するテーブル名が表示される。

#### ▼ `type`

設定した`SELECT`句がテーブルに使用する時に、どの程度の数のカラムを検索するのかが表示される。
...
| range  | ・セカンダリーDBインデックスを使用している。<br>・`WHERE`句に重複したレコード値、`IN`句、`BETWEEN`句を使用している。 | 特定の複数カラム           |                                                                                            |
| ref    | ・セカンダリーDBインデックスを使用している。<br>・`WHERE`句に重複しないレコード値                                    | 特定の複数カラム           |                                                                                            |
