...
障害によって，トランザクションの終了後に一連のステートメントの更新結果がディスクに反映されなかった場合，ログファイルの更新後ログを使用して，ディスク上のデータファイルに更新結果を反映させる．

**＊例＊**

『a』の値を更新するステートメントを含むトランザクションの後に，ソフトウェアが異常終了した場合，ログファイルの更新後ログ『a = 5』を使用して，ディスク上のデータファイルに更新結果を反映させる．（ロールフォワード）
...
DBサーバーのハードウェア障害のこと．例えば，ハードディスクの障害がある．ディスクを初期化/交換した後，バックアップファイルからDBを修復し，ログファイルの更新後ログ『a = 5』『b = 1』を使用して，修復できる限りロールフォワードを行う．
