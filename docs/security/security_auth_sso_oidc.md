...
フロントエンドのブラウザが、IDプロバイダーのログアウトエンドポイントにPOSTリクエストを送信し、全てのアプリケーションからログアウトする。
...
IDプロバイダーが、バックエンドのいずれかのアプリケーションのログアウトエンドポイント (`<アプリケーションのドメイン>/k_logout`) にPOSTリクエストを送信し、そのアプリケーションでログアウト処理を実行する。

セッションの有効期限が失効し、バックエンドのアプリケーションが起点となってログアウト処理を始める場合、それはBack-Channelである。

また、フロントエンドのブラウザがバックエンドのアプリケーションにログアウトリクエストを送信し、これが起点となる場合もBack-Channelである。
