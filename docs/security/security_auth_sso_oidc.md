...
アクセストークンの発行前に、IDトークンの情報 (署名部分、有効期限、発行元認証局など) を検証する。
...
発行後のアクセストークンの署名部分の情報 (署名部分、有効期限、発行元認証局など) を検証する。

以下のいずれかの方法で検証できる。

- 認可サーバーのイントロスペクションエンドポイントにリクエストする。
- 認可サーバーから取得した公開鍵を使用して、アクセストークンの情報 (署名部分、有効期限、発行元認証局など) を検証する。
