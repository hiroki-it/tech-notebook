...
アクセストークンの発行前に、IDトークンの署名部分のデータ (有効期限、ユーザー識別子など) を検証する。
...
発行後のアクセストークンの署名部分のデータ (有効期限、ユーザー識別子など) を定期的に検証する。

以下のいずれかの方法で検証できる。

- 認可サーバーのイントロスペクションエンドポイントにリクエストする
- 認可サーバーから取得した公開鍵を使用して、アクセストークンの署名部分のデータ (有効期限、ユーザー識別子など) を検証する
