...
送信元マシンから宛先マシン上Webアプリケーションに対してコピーアンドペーストができない場合がある。
...
| ゲートウェイマシン | 宛先マシンで稼働するWebアプリのクライアントであり、ブラウザが稼働している。 |                            | ✅ (Guacamoleサーバー + guard) |      ✅       |
...
仮想ホストを使用してサーバーに複数のドメインを設定していても、サーバー証明書は一つになってしまう。
...
Chromeでは、HTTPSリクエストの使用時にサーバー証明書に不備がある (例：自己署名サーバー証明書を使用している) と、以下のような警告が表示される。
