...
2. クライアントがTLSバージョンや暗号スイートを提示し、サーバーがこれらを決定する。
3. 証明書ベース認証（サーバー認証、クライアント認証、相互TLS認証）を開始する。
   1. 証明書ベース認証の種類に応じて、クライアントとサーバー間で証明書を送受信する。
   2. 証明書の受信側は証明書の署名を検証する。
4. 証明書ベース認証完了する。
5. 鍵交換アルゴリズムに基づいて、共通鍵（セッション鍵）をクライアントとサーバー間で共有する。
6. TLSハンドシェイクが完了する。
7. HTTPSリクエスト／レスポンスを送受信する。クライアントとサーバー間で、共通鍵を使用してパケットのアプリケーションデータを暗号化／復号する。
