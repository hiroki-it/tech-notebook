...
宛先のサーバーに複数のSSLサーバー証明書を配置できるようになる。

従来のSSL/TLSでは、宛先のサーバーに単一のSSLサーバー証明書しか配置できなかった。

仮想ホストを使用してサーバーに複数のドメインを設定していても、SSLサーバー証明書は一つになってしまう。
...
Chromeでは、HTTPSリクエストの使用時にSSLサーバー証明書に不備がある (例：自己署名SSLサーバー証明書を使用している) と、以下のような警告が表示される。
...
サーバー認証では、HTTPSリクエストの宛先のみSSLサーバー証明書を設定すればよい。
