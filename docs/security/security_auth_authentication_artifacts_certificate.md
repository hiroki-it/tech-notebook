...
サーバー証明書をサーバー側へ配置したうえで、クライアント証明書を持つクライアントだけがHTTPSリクエストでサーバー側へ通信できるようにする。
...
送信されたXMLデータが改ざんされていないことを検証するための証明書。

HTTPSでXMLデータを送信する場合、サーバー証明書とXML署名要証明書の両方が必要である。
...
レスポンスのXMLデータにXML署名を付与し、クライアントが発行した署名用証明書の情報をレスポンスに含めて返信している。

クライアントはレスポンスのXML署名を検証することで、XMLデータが改ざんされていないことを確認する。
