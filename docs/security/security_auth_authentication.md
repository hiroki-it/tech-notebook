...
レンダリング後にブラウザから取得したトークンを設定する必要があるため、フロントエンドアーキテクチャがCSRの場合に採用できる。

SSRでは、レンダリング後の処理に関与できないため、ブラウザのトークンをSSRのアプリケーションに渡せない。
...
### クライアントPCの`Cookie`ディレクトリ

#### ▼ `Cookie`ディレクトリとは

クライアントPCのストレージ機能であり、明示的に削除しない限りは保存し続ける。
...
ブラウザは、クライアントPCの`Cookie`ディレクトリにトークンを保管する。

ブラウザを閉じても、ブラウザはクライアントPCのディレクトリのトークンを破棄せず、認証の成功状態を維持できる。
