...
セッションデータで認証情報を伝播する認証方法 (例：セッションIDを使用したForm認証、など) である。
...
いずれかのコンポーネントでセッションデータが消失しても復元できるように、セッションストレージを使用する必要がある。
...
トークン (例：アクセストークン、IDトークン、など) で認証情報を伝播する認証方法 (例：トークンを使用したForm認証、APIキー認証、PAT、OIDC、など) である。
