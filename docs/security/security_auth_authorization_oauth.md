...
OAuthには、仕組み別に『認可コードフロー』『暗黙的フロー』『リソースオーナー・パスワード・資格情報ズフロー (ダイレクト・アクセス・グラント) 』などがある。
...
OAuthでは、認証スキーマとしてBearer認証の採用例が多い。

一方で、AWSやGitHubでは独自の認証スキームの採用例もある。
...
: Facebookは認証を完了させ、Facebookの資格情報を送信する。

`(11)`

: 免許証作成サイトは認可を実施し、Facebookの資格情報の権限に応じた処理をクライアントに提供する。
