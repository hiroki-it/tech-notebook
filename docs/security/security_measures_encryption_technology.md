...
デジタル証明書をSSLに使用する場合、特にSSL証明書という。対になる秘密鍵と組み合わせて、ドメインの認証に使用される。
...
クライアント側に配置する証明書である。必須ではないが、SSL証明書のみを使用する場合よりも安全性が高くなる。対になる秘密鍵もクライアント側に配置することになるため、クライアント側にはクライアント証明書と秘密鍵の両方を配置することになる。
