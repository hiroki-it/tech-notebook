...
:    秘密鍵から、証明書署名要求 (```.csr```ファイル) を作成する。対話形式で入力を求められるため、『Common Name』に、Webサイトで使用する完全修飾ドメイン名を入力する以外は、何も入力せずにエンターとする。
...
:    中間認証局を『自分』として、秘密鍵と証明書署名要求に基づいて、SSL証明書 (```.crt```ファイル) を作成する。有効期限は```10```年 (```3650```日) とする。
...
:    秘密鍵 (```.key```ファイル) 、SSL証明書 (```.crt```ファイル) 、を該当の箇所に設定する。例えば、Nginxの設定ファイルなら、以下の通りとなる。
