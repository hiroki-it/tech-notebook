...
: ルート認証局は証明書署名要求からハッシュ値を算出し、ルート認証局のもつ秘密鍵を使用してハッシュ値を暗号化する。もとの証明書署名要求とハッシュ値を組み合わせたものを証明書とする。S/MIMEで使用する場合には、『S/MIME証明書』、SSL/TLSで使用する場合には、『SSL証明書』という。

`(4)`

: サーバーに署名済み証明書 (S/MIME証明書、SSL証明書) をインストールする。
...
1. クライアントで共通鍵を作成する。公開鍵で暗号化し、サーバーに送信する。
2. サーバーは秘密鍵でこれを復号し、共通鍵を取得する。
3. 送信元と宛先の間で共通鍵の交換が完了する。
