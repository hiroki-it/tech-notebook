...
: ルート認証局は証明書署名要求からハッシュ値を算出し、ルート認証局のもつ秘密鍵を使用してハッシュ値を暗号化する。もとの証明書署名要求とハッシュ値を組み合わせたものを証明書とする。S/MIMEで使用する場合には、『S/MIME証明書』、SSL/TLSで使用する場合には、『SSLサーバー証明書』という。

`(4)`

: サーバーに署名済み証明書 (S/MIME証明書、SSLサーバー証明書) をインストールする。
...
証明書 (S/MIME証明書、SSLサーバー証明書) を発行する中間認証局そのものが、なりすましの可能性がある。
...
もしルート認証局に署名されていない中間認証局でSSLサーバー証明書を署名すると、『このWebサイトで提示されたセキュリティ証明書は、信頼された証明機関から発行されたものではありません』といったような警告文が出る。
...
認証局によってSSLサーバー証明書の発行方法は異なり、単体あるいはセットで発行する場合がある。

ルート認証局と中間認証局のSSLサーバー証明書がセットになったファイルを証明書バンドルという。
...
SSLサーバー証明書をサーバー側に配置した上で、クライアント証明書を持つクライアントのみがHTTPSリクエストでサーバー側に通信できるようにする。
...
自己署名したオレオレのルート認証局で署名して、オレオレのSSLサーバー証明書やクライアント証明書を作成することもできる。
...
例えばNginxならば、`/etc/nginx/ssl`ディレクトリ配下にSSLサーバー証明書 (`.crt`) と`/etc/ssl/private`ディレクトリ配下に秘密鍵 (`.key`) の両方を配置する。
...
例えばUbuntuならば、`/etc/ssl/certs`ディレクトリ配下にSSLサーバー証明書 (`.crt`) を、`/etc/ssl/private`ディレクトリ配下に秘密鍵 (`.key`) を配置する。
...
### 自己署名SSLサーバー証明書 (オレオレSSLサーバー証明書) の作成

#### ▼ SSLサーバー証明書の作成に必要なもの

| ファイル          | 説明                                                                        | 拡張子                                |
| ----------------- | --------------------------------------------------------------------------- | ------------------------------------- |
| 秘密鍵            | SSLサーバー証明書とペアになる秘密鍵として動作する。                         | `pem`、`.key`、`.txt`                 |
| 証明書署名要求    | 秘密鍵から作成され、公開鍵であるSSLサーバー証明書を作成するために使用する。 | `.csr`、`.txt`                        |
| SSLサーバー証明書 | 秘密鍵とペアになる公開鍵として動作する。                                    | `pem`、`.crt`、`.cert`、`.ca`、`.txt` |
...
以下のコマンドをサーバー側で実施し、秘密鍵 (後にSSLサーバー証明書とペアになる) 、証明書署名要求、SSLサーバー証明書を作成する。

この時のルート認証局は『自分』である。

`(1)`

: SSLサーバー証明書の有効期限が切れてしまい、HTTPSリクエストを送信できなくなってしまったとする。
...
: 自己署名のSSLサーバー証明書を作成する。

     認証局はサーバー自身なため、サーバーの秘密鍵 (`.key`ファイル) と証明書署名要求 (`.csr`ファイル) による署名で、SSLサーバー証明書 (`.crt`ファイル) を作成する。
...
: SSLサーバー証明書 (`.crt`ファイル) 、SSLサーバー証明書とペアになる秘密鍵 (`.key`ファイル) を該当の箇所に設定する。
...
#-------------------------------------
server {
    # 443番ポートで待ち受けるようにし、SSLサーバー証明書を使用する。
    # listen 80;
    listen 443 ssl;
...
*  start date: Dec 21 02:08:29 2022 GMT # SSLサーバー証明書の開始日
*  expire date: Jan 22 02:08:28 2024 GMT # SSLサーバー証明書の失効日
...
`openssl`コマンド以外にも、SSLサーバー証明書を作成できるツールがある。
