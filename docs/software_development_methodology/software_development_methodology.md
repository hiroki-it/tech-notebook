---
title: 【IT技術の知見】開発手法論＠開発手法
description: 開発手法論＠開発手法の知見を記録しています。
---
...
- バックエンドロジックがなくてもレンダリングできる固定値のJavaScriptやTypeScriptを実装する。これは、フレームワークを使用していても可能。
- HTMLをそのまま実装する。
...
- AWS Lambdaなどのインフラストラクチャが抽象化されたツールを使用し、その上で簡易的なアプリケーションを稼働させる。
