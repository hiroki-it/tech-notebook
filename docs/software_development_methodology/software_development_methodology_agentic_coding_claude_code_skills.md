---
title: 【IT技術の知見】スキル＠Claude Code
description: スキル＠Claude Codeの知見を記録しています。
---
...
ユーザーがフロントエンドとバックエンドの両方を含む新機能のアーキテクチャ設計を依頼したら、以下の手順で進める。

このスキルはレイヤー、ファイル、関数だけを設計するものであり、具体的な実装は機能要件や非機能要件に応じて調整する必要がある。

### 全体像

1.  `./.claude/rules/application-architecture-design.md` から、レイヤードアーキテクチャにおけるフロントエンドとバックエンドの各層がどのディレクトリに相当するのかを確認する。
2.  まずはフロントエンドの設計に進む。
...
2. `./app/usecases`ディレクトリに`ui<機能名>Usecase.ts`を作成し、機能要件のユースケース（閲覧、登録、更新、削除）に応じた関数を実装する。
   ...
   - 更新系：`changeXxx`
   - 削除系：`deleteXxx`
3. `./app/presentation/routes`ディレクトリにある閲覧系ユースケースに対応したuiルートを確認する。このuiルートの`loader`で、`ui<機能名>Usecase.ts`にある閲覧系
4. ユースケースの関数を呼び出す。
5. `./app/presentation/routes`ディレクトリにある登録・更新・削除系ユースケースに対応したuiルートを確認する。このuiルートの`action`で、`ui<機能名>Usecase.ts`にある登録・更新・削除系ユースケースの関数を呼び出す。
6. ドメイン層の設計に進む。
...
2. 新しいORMのDBモデルが必要であれば、`./prisma/schema.prisma`にDBモデルを実装する。
3. `./app/infrastructure/models`ディレクトリに`<DBモデル名>.server.ts`を作成する。ここに登録・更新・削除系ユースケースに応じたCRUD関数を実装する。
   - READ系：`getXxx`
   - CREATE系：`createXxx`
   - UPDATE系：`updateXxx`
   - DELETE系：`deleteXxx`
   - なんらかの理由でCREATEとUPDATEをかねたい場合：`saveXxx`
4. `./app/usecases`ディレクトリにある`ui<機能名>Usecase.ts`を確認する。各ユースケースの関数で、`<DBモデル名>.server.ts`にあるCRUD関数を呼び出す。

### セルフレビュー

成果物が `./.claude/rules/application-architecture-design.md` に記載の規約に準拠しているかを確認する。準拠していなければ修正する。
```
