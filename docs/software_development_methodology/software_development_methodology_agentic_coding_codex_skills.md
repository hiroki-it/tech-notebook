...
2. `./app/usecases`ディレクトリに`ui<機能名>Usecase.ts`を作成し、機能要件のユースケース（閲覧、登録、更新、削除）に応じた関数を実装する。
   - 下書き系：いくつかの`getDraftXxx` (デフォルト値を含むドメインオブジェクト)
...
3. `./app/presentation/routes`ディレクトリにある下書き系または閲覧系ユースケースに対応したuiルートを確認する。このuiルートの`loader`で、`ui<機能名>Usecase.ts`にある閲覧系ユースケースの関数を呼び出す。
