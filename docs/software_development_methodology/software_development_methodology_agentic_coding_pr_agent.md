...
メンションに応じて自動でPRの内容を読み取り、レビューを実施する。
...
1. PR AgentをCIに導入する
2. 開発者はPR上でPR-Agentにコメントをもらい、レビューをお願いする前に、ある程度問題点を修正しておく

<br>

### レビュー依頼後

1. レビュアーは手元で生成AIに git diff develop をさせて、作業ブランチとdevelopブランチの差分を読み取らせる
2. 生成AIに次を読み取らせ、レビューしてもらう

- 保守性や可読性などのルールをまとめたオリジナルのドキュメント
- すでに作成いただいている仕様書

3. レビュアーは自身の観点と生成AIの観点を合わせて、必要であればコメントをする
