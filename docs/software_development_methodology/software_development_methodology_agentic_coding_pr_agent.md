...
1. PR Agent を CI に導入する
2. 開発者は PR 上で PR-Agent にコメントをもらい、レビューをお願いする前に、ある程度問題点を修正しておく

<br>

### レビュー依頼後

1. レビュアーは手元で生成 AI に git diff develop をさせて、作業ブランチと develop ブランチの差分を読み取らせる
2. 生成 AI に次を読み取らせ、レビューしてもらう

- 保守性や可読性などのルールをまとめたオリジナルのドキュメント
- すでに作成いただいている仕様書

3. レビュアーは自身の観点と生成 AI の観点を合わせて、必要であればコメントをする
...
プッシュのたびに PR-Agent を実行させるとレビューへの修正負荷が高いため、PR-Agent の発火は Webhook で制御したほうがよい。

<br>

### /describe

変更内容から、自動的に MR の種別や変更内容などを description に記載する。

### /review

変更したコードの問題点やセキュリティ的な脆弱性などをレビューし、PR にコメントする。
...
処理に失敗した際の fallback で使用する model を設定する。
