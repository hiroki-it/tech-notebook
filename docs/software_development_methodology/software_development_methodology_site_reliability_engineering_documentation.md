...
|                  | Terraform   | 独自の```module```ブロックを採用している場合、外部の仕様書自動作成ツール（例：terraform-docs）を使用して、```variable```ブロックや```output```ブロックに基づいて、```module```ブロックの仕様書を作成しておく。 | ℹ️ 参考：https://qiita.com/yutachaos/items/1a7f5a93ceaf972c76c6 |
