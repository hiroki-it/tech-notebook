...
任意のツールによる標準出力/標準エラー出力の結果をレビューコメントとして、GitHub に POST リクエストを送信する。
...
./github-comment exec -var "TestName:pluto" -var "Description:指定したK8sバージョン (${K8S_CURRENT_VERSION}) で使用できるK8sリソースやカスタムリソースのスキーマに基づいて、マニフェストのAPI仕様違反を検証する" \
  -- /pluto detect -t k8s=v${K8S_CURRENT_VERSION} manifest.yaml -o wide \
  || true

echo Test "Next K8s Version: ${K8S_NEXT_VERSION}"
./github-comment exec -var "TestName:pluto" -var "Description:指定したK8sバージョン (${K8S_NEXT_VERSION}) で使用できるK8sリソースやCRDのスキーマに基づいて、マニフェストのAPI仕様違反を検証する" \
...
echo Test "Next K8s Version: ${K8S_NEXT_VERSION}"
./github-comment exec -var "TestName:kubeconform" -var "Description:指定したK8sバージョン (${K8S_NEXT_VERSION}) で使用できるK8sリソースやカスタムリソースのスキーマに基づいて、マニフェストのAPI仕様違反を検証する" \
...
GitHub に送信するレビューコメントのテンプレートを設定する。
...
テンプレートが 1 つの場合は、`when` キーを `true` とする
