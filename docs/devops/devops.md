...
| ホワイトボックステスト | Unitテスト、Functionalテスト     |       ⭕️       | CIツールとテストフレームワークで自動化できる。   |
...
全ての一般ユーザーがルーティングされるデプロイ手法（例：カナリアリリース以外）と組み合わせる。ただ、機能をこっそりリリースしておく。を使用したいユーザーだけがトグルなどで有効化できるようにしておき、本番環境で実地的に検証する。
