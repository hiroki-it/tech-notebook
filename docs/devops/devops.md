...
CIツールを用いてCIパイプラインを，またCDツールを用いてCDパイプラインを行う手法のこと．
...
特にアプリケーションが冗長化されている場合に用いるデプロイ手法．旧環境と新環境のインスタンスの合計数を維持しながら，新環境インスタンスを段階的にデプロイする．新環境インスタンスが起動が完了したことをヘルスチェックなどで確認し，起動が完了したことを確認できれば，社外を含む全てのアクセスのルーティング先を新環境インスタンスに自動的に切り替えていく．アクセス可能な新環境インスタンスを増やしながら，旧環境インスタンスを少しずつ削除していく．この時，マネージドなデプロイツールを用いると，ルーティング先の切り替え作業がより簡単になる．
...
旧環境（Prodブルー）を残したまま，新環境（Testグリーン）をデプロイする．特定のポート番号からのみ新環境にアクセスできるようにし，新環境の動作を開発者の目で確認する．新環境の動作に問題がなければ，社外を含む全てのアクセスのルーティング先を，新環境に手動で切り替える．新環境への切り替えが完全に完了した後，新環境から旧環境にロールバックを行う場合に備えて，旧環境は削除せずに残しておく．何を基点にしてルーティング先を切り替えるかによって，具体的な方法が大きく異なり，ロードバランサーを基点とする場合が多い．この時，マネージドなデプロイツールを用いると，ルーティング先の切り替え作業がより簡単になる．
...
旧環境を残したまま，新環境をデプロイする．一部のクライアントのリクエストのみを新環境にルーティングし，その後，新環境にルーティングされるクライアントを，手動で段階的に増やしていく．新環境に問題があれば，ロールバックとして旧環境への重み付けを```100```%とする．新環境への重み付けが```100```%になった後，旧環境を削除する．この時，マネージドなデプロイツールを用いると，ルーティング先の重み付け値の変更作業がより簡単になる．
