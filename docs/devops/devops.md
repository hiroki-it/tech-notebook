...
KubernetesのCICDパイプラインにCIOpsを採用する場合、リポジトリ側にkubeconfigファイルを置く必要がある。kubeconfigファイルは機密性が高く、漏洩させたくない。ただ、どうしてもCIOpsを採用したいのであれば、sops暗号化キー（例：AWS KMS、GCP KMS、など）でkubeconfigファイルを暗号化しておき、これをCIパイプライン内に出力する。
