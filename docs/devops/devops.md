...
システムには、例えば以下の品質の特性がある（ISOの規格の場合）。これらの項目に関して、一定水準以上の品質を維持しやすくするためには、DevOpsを採用すると良い。
...
開発者と運用者が協調して開発と運用を行い、システムを継続的に改善する手法論のこと。DevOpsの実現方法には、CIOpsまたはGitOpsがある。
