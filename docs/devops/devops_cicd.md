...
2. テスト
3. 承認
4. DBマイグレーション（アプリケーションを起動する前に実行する必要がある）
5. アプリケーションのデプロイ
...
執筆時点 (2022/11/09) では、どのようなユースケースでもArgoCDがよいかもしれない。
...
この場合、リポジトリで同じ設定ファイルを横展開するよりも、CIツールの設定ファイルの共有部分は特定のリポジトリで中央集中的に管理するとよい。
...
1. 承認
2. DBマイグレーション（アプリケーションを起動する前に実行する必要がある）
3. アプリケーションのデプロイ

DBマイグレーションを実行するタイミングとしては、次がある。

- CDツール（例：ArgoCD）でアプリケーションのデプロイ直前に実行する
- Dockerfileで、アプリケーションの起動コマンドの直前にマイグレーションコマンドを実行する
- CIツールともCDツールとも違う別のツールで実行する
