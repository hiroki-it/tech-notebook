...
2. テスト
3. 承認
4. DBマイグレーション（アプリを起動する前に実行する必要がある）
5. デプロイ
...
| --------------------------------- | ------------------------------------------------------------------------------ | :----------: | ------------------------ |
| DBマイグレーション                | ステージング環境のDBに対するデプロイ。アプリを起動する前に実行する必要がある。 |     `⭕️`     | CDツールで自動化できる。 |
...
| DBマイグレーション        | 本番環境のDBに対するデプロイ。アプリを起動する前に実行する必要がある。 |     `⭕️`     | CDツールで自動化できる。 |
...
1. 承認
2. デプロイ
3. DBマイグレーション（アプリを起動する前に実行する必要がある）
