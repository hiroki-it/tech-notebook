...
AWSの資格情報は、CircleCIのデフォルト名と同じ環境変数名で登録しておけば、オプションで渡さなくとも、自動的に入力してくれる。
...
aws-cliコマンドのインストールと、Credentials情報の設定する。
...
ただし、資格情報ファイルの作成では、orbsを使用しないほうがより簡潔に条件分岐を実装できるかもしれない。
