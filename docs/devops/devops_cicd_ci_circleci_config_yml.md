...
公式リファレンスには載っていないため、この方法は非推奨。

`parameter` を渡したい `executor` を使いまわしたいときに使用する。
...
| 粒度   | 説明                                                           | 備考                                                            |
| ------ | -------------------------------------------------------------- | --------------------------------------------------------------- |
| build  | プログラムの実行環境を作成する。                               | build と test を分割しにくい場合は、同じ job で定義してもよい。 |
...
これを使用しない場合、例えば、CircleCI コンテナで `composer install` コマンドを実行することで、毎回の workflow で同じパッケージがインストールされる。
...
`workspace` キーで、コールされる `jobs` キーの引数として設定することで、その `jobs` キー内の最初と最後に、`steps` キーを追加できる。
...
`ignore` キーですべてのブランチを指定することで、マージによる発火を防げる。
...
Container レベルより参照範囲が大きく、プロジェクト内、つまりリポジトリ内のみで参照できる。
...
Project レベルより参照範囲が大きく、異なるプロジェクト間、つまりリポジトリ間で参照できる。
