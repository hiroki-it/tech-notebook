...
そのため、これらのツールを使用せずSecretにデータを注入する場合、プラグインは採用しなくともよい。
...
ArgoCDのビルトインのKustomizeのバージョンの場合、Kustomizeをインストールする必要はない。
...
マニフェストを復号するツールとクラウド暗号化キーを使用している場合、暗号化キーがHTTPSを要求することがある。

この時、サーバー証明書をサイドカーに設定する必要がある。
