...
ただし、CodeDeployの様にロードバランサ－を使用し、現環境と新環境のルーティングを切り替えるようにすると、ダウンタイムを防げる。
...
確認できれば、社外を含む全てのアクセスのルーティング先を、新環境インスタンスに自動的に切り替えていく。
...
`2` 個の環境に名前をつけるときに、優劣の印象がないように色が選ばれ、色の中でも無難な青と緑になった。
...
現環境と新環境を用意したうえで、一部のユーザー (由来のカナリアに相当) の手を借りて新環境を実地的にテストし (例：該当のエラーメトリクスが基準値を満たすか) 、新環境に問題が起こらなければ全てのユーザーを新環境にルーティングする。
