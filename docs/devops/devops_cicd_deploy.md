...
最初に旧アプリケーションを停止し、サーバーの OS とミドルウェアの構成はそのままで、アプリケーションのみを上書きする。
...
ヘルスチェックなどで新環境インスタンスの起動完了を確認する。

確認できれば、社外を含むすべてのアクセスのルーティング先を、新環境インスタンスに自動的に切り替えていく。
...
ただし、新環境の起動しか自動的に確認されないため、具体的な動作が正常か否かをリリース作業後に確認する必要がある。
...
`2` 個の環境に名前をつけるときに、優劣の印象がないように色が選ばれ、色のなかでも無難な青と緑になった。
...
: 現環境 (Prod ブルー) を残したまま、新環境 (Test グリーン) をデプロイする。
...
: 新環境の動作に問題が起こらなければ、社外を含むすべてのアクセスのルーティング先を、新環境に手動で切り替える。

`(4)`

: 新環境への切り替えが完全に完了した後に備える。

新環境から現環境にロールバックする場合、現環境は削除せずに残しておく。
...
一方でブルー/グリーンデプロイメントでは、現環境はリリース後も削除せず稼働させたままにしておく。
...
現環境と新環境を用意したうえで、一部のユーザー (由来のカナリアに相当) の手を借りて新環境を実地的にテストし (例：該当のエラーメトリクスが基準値を満たすか) 、新環境に問題が起こらなければすべてのユーザーを新環境にルーティングする。
...
: 一般ユーザーからの一部リクエストのみを新環境にルーティングし、実際のユーザー (由来のカナリアに相当) の手を借りて、本番環境で実地的に検証する (例：該当のエラーメトリクスが基準値を満たすか) 。

     この実環境でのテストで新環境に問題が起こらなければ、ルーティングされるユーザーを手動で段階的に増やしていく。
...
- AWS (API Gateway、Route53 や AWS ALB による重み付けルーティング)
- ArgoCD
...
すべての一般ユーザーがルーティングされるデプロイ手法 (例：カナリアリリース以外) と組み合わせる。
