...
アプリのデプロイまでの各ステップ (ビルド、ホワイトボックステスト、デプロイ) を自動化する。
...
アプリのデプロイの粒度を小さくし、小さなデプロイを頻繁に実行する。
...
最初に旧アプリを停止し、サーバーのOSとミドルウェアの構成はそのままで、アプリのみを上書きする。
...
旧アプリの停止から新アプリの再起動まで、に相当する時間のダウンタイムが発生する。
...
新アプリの起動を確認してから、旧アプリを停止するため、ダウンタイムが発生しない。
